...
import { collectPreparedPrepackErrors, shouldSkipPrepack } from "../scripts/civitas-prepack.ts";

describe("shouldSkipPrepack", () => {
  it("treats unset and explicit false values as disabled", () => {
    expect(shouldSkipPrepack({})).toBe(false);
    expect(shouldSkipPrepack({ CIVITAS_PREPACK_PREPARED: "0" })).toBe(false);
    expect(shouldSkipPrepack({ CIVITAS_PREPACK_PREPARED: "false" })).toBe(false);
  });

  it("treats non-false values as enabled", () => {
    expect(shouldSkipPrepack({ CIVITAS_PREPACK_PREPARED: "1" })).toBe(true);
    expect(shouldSkipPrepack({ CIVITAS_PREPACK_PREPARED: "true" })).toBe(true);
  });
});
