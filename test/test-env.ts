...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-test-home-"));
...
function stageLiveTestState(params: {
  ...
  if (
    ...
    realStateDir === path.join(priorIsolatedHome, ".civitas")
  ) {
    realStateDir = path.join(params.realHome, ".civitas");
  }
  ...
  if (fs.existsSync(realConfigPath)) {
    ...
    fs.writeFileSync(
      path.join(tempStateDir, "civitas.json"),
