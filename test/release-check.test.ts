...
describe("collectBundledExtensionManifestErrors", () => {
  it("flags invalid bundled extension install metadata", () => {
    expect(
      ...
      "bundled extension 'broken' manifest invalid | civitas.install.npmSpec must be a non-empty string",
    ]);
  });
      ...
      collectBundledExtensionManifestErrors([
        {
          ...
          packageJson: {
            civitas: {
              install: { npmSpec: "@civitas/broken", minHostVersion: "2026.3.14" },
            },
          },
        },
      ]),
    ).toEqual([
      "bundled extension 'broken' manifest invalid | civitas.install.minHostVersion must use a semver floor in the form \">=x.y.z\"",
    ]);
  });
  ...
  it("flags non-object install metadata instead of throwing", () => {
    ...
    ).toEqual(["bundled extension 'broken' manifest invalid | civitas.install must be an object"]);
  });
});
...
describe("collectBundledExtensionRootDependencyMirrorErrors", () => {
  it("flags a non-array mirror allowlist", () => {
    expect(
      ...
      "bundled extension 'matrix' manifest invalid | civitas.releaseChecks.rootDependencyMirrorAllowlist must be an array",
    ]);
  });
  ...
  it("flags mirror entries missing from extension runtime dependencies", () => {
    expect(
      ...
      "bundled extension 'matrix' manifest invalid | civitas.releaseChecks.rootDependencyMirrorAllowlist entry '@matrix-org/matrix-sdk-crypto-wasm' must be declared in extension runtime dependencies",
    ]);
  });
  ...
  it("flags mirror entries missing from root runtime dependencies", () => {
    expect(
      ...
      "bundled extension 'matrix' manifest invalid | civitas.releaseChecks.rootDependencyMirrorAllowlist entry '@matrix-org/matrix-sdk-crypto-wasm' must be mirrored in root runtime dependencies",
    ]);
  });
  ...
  it("flags mirror entries whose root version drifts from the extension", () => {
    expect(
      ...
      "bundled extension 'matrix' manifest invalid | civitas.releaseChecks.rootDependencyMirrorAllowlist entry '@matrix-org/matrix-sdk-crypto-wasm' must match root runtime dependency version (extension '18.0.0', root '18.1.0')",
    ]);
  });
  ...
  it("allows bundled plugin runtime deps under dist/extensions but still blocks other node_modules", () => {
    expect(
      collectForbiddenPackPaths([
        ...
        "node_modules/.bin/civitas",
      ]),
    ).toEqual([bundledPluginFile("tlon", "node_modules/.bin/tlon"), "node_modules/.bin/civitas"]);
  });
  ...
  it("requires the shipped channel catalog, control ui, and optional bundled metadata", () => {
    ...
    expect(missing).toEqual(
      expect.arrayContaining([
        ...
        bundledDistPluginFile("matrix", "civitas.plugin.json"),
        ...
        bundledDistPluginFile("whatsapp", "civitas.plugin.json"),
...
describe("collectPackUnpackedSizeErrors", () => {
  it("accepts pack results within the unpacked size budget", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("civitas-2026.3.14.tgz", 120_354_302)]),
    ).toEqual([]);
  });

  it("flags oversized pack results that risk low-memory startup failures", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("civitas-2026.3.12.tgz", 224_002_564)]),
    ).toEqual([
      "civitas-2026.3.12.tgz unpackedSize 224002564 bytes (213.6 MiB) exceeds budget 200278016 bytes (191.0 MiB). Investigate duplicate channel shims, copied extension trees, or other accidental pack bloat before release.",
    ]);
  });
  ...
  it("fails closed when npm pack output omits unpackedSize for every result", () => {
    expect(
      collectPackUnpackedSizeErrors([
        { filename: "civitas-2026.3.14.tgz" },
        { filename: "civitas-extra.tgz", unpackedSize: Number.NaN },
      ]),
    ).toEqual([
