...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `civitas-e2e-${name}-`));
  const configDir = path.join(homeDir, ".civitas");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          CIVITAS_SKIP_PROVIDERS: "1",
          CIVITAS_SKIP_GMAIL_WATCHER: "1",
          CIVITAS_SKIP_CRON: "1",
          CIVITAS_SKIP_BROWSER_CONTROL_SERVER: "1",
          CIVITAS_SKIP_CANVAS_HOST: "1",
          CIVITAS_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
