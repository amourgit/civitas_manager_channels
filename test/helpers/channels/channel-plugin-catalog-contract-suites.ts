...
function writeDiscoveredChannelPlugin(params: {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "civitas.plugin.json"),
    JSON.stringify({
      id: params.pluginId,
      configSchema: {},
    }),
    ...
    it.each([
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-catalog-"));
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@civitas/demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(os.tmpdir(), "civitas-channel-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-catalog-state-"));
          ...
          return {
            ...
            env: {
              ...process.env,
              CIVITAS_STATE_DIR: stateDir,
              CHANNELDBOT_STATE_DIR: undefined,
              CIVITAS_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
            },
            expected: {
    ...
    it.each([
      {
        ...
        setup: () => {
          const home = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-catalog-home-"));
          const catalogPath = path.join(home, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@civitas/env-demo-channel",
          ...
          return {
            env: {
              ...process.env,
              CIVITAS_PLUGIN_CATALOG_PATHS: "~/catalog.json",
              CIVITAS_HOME: home,
              HOME: home,
            },
      ...
      },
      {
        ...
        setup: () => {
          ...
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@civitas/default-env-demo",
