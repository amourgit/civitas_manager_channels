...
function installDiscoveryHooks(state: DiscoveryState) {
  beforeEach(async () => {
    vi.resetModules();
    vi.doMock("civitas/plugin-sdk/agent-runtime", async () => {
    ...
    vi.doMock("civitas/plugin-sdk/provider-auth", async () => {
      const actual = await vi.importActual<object>("civitas/plugin-sdk/provider-auth");
      return {
        ...actual,
        ensureAuthProfileStore: ensureAuthProfileStoreMock,
        listProfilesForProvider: listProfilesForProviderMock,
