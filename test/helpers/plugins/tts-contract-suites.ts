...
function asLegacyTtsConfig(value: unknown): CIVITASConfig {
  return value as CIVITASConfig;
}

function asLegacyCIVITASConfig(value: Record<string, unknown>): CIVITASConfig {
  return value as unknown as CIVITASConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): CIVITASConfig {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    const baseCfg: CIVITASConfig = asLegacyCIVITASConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.CIVITAS_TTS_PREFS;
      process.env.CIVITAS_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.CIVITAS_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: CIVITASConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
