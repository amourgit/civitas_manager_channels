...
describe("loadVitestExperimentalConfig", () => {
  ...
  it("enables the filesystem module cache explicitly", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          CIVITAS_VITEST_FS_MODULE_CACHE: "1",
        },
        "linux",
      ),
    ).toEqual({
  ...
  it("passes through the filesystem module cache path when provided", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          CIVITAS_VITEST_FS_MODULE_CACHE_PATH: "/tmp/civitas-vitest-cache",
        },
        "linux",
      ),
    ).toEqual({
      experimental: {
        fsModuleCache: true,
        fsModuleCachePath: "/tmp/civitas-vitest-cache",
      },
    });
  });
  ...
  it("allows disabling the filesystem module cache explicitly", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          CIVITAS_VITEST_FS_MODULE_CACHE: "0",
        },
        "linux",
      ),
    ).toEqual({});
  ...
  it("enables import timing output and import breakdown reporting", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          CIVITAS_VITEST_IMPORT_DURATIONS: "true",
          CIVITAS_VITEST_PRINT_IMPORT_BREAKDOWN: "1",
        },
        "linux",
      ),
    ).toEqual({
