...
describe("stageBundledPluginRuntimeDeps", () => {
  function createBundledPluginFixture(params: {
    ...
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-runtime-deps-"));
  ...
  it("skips restaging when runtime deps stamp matches the sanitized manifest", () => {
    const { pluginDir, repoRoot } = createBundledPluginFixture({
      packageJson: {
        name: "@civitas/fixture-plugin",
        version: "1.0.0",
        dependencies: { "left-pad": "1.3.0" },
        peerDependencies: { civitas: "^1.0.0" },
        peerDependenciesMeta: { civitas: { optional: true } },
        devDependencies: { civitas: "^1.0.0" },
        civitas: { bundle: { stageRuntimeDependencies: true } },
      },
    });
    ...
    stageBundledPluginRuntimeDeps({
      ...
      installPluginRuntimeDepsImpl: ({ fingerprint }: { fingerprint: string }) => {
        installCount += 1;
        fs.writeFileSync(
          path.join(pluginDir, ".civitas-runtime-deps-stamp.json"),
  ...
  it("stages runtime deps from the root node_modules when already installed", () => {
    ...
    expect(fs.existsSync(path.join(pluginDir, ".civitas-runtime-deps-stamp.json"))).toBe(true);
  });
