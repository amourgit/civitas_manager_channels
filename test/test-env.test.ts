...
function createTempHome(): string {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-test-env-real-home-"));
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-civitas.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.civitas/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.civitas/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".civitas", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".civitas", "agents", "main", "agent", "auth-profiles.json"),
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".civitas", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".civitas", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
