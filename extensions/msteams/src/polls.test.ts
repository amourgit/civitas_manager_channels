...
describe("msteams polls", () => {
  ...
  it("stores and records poll votes", async () => {
    const home = await fs.promises.mkdtemp(path.join(os.tmpdir(), "civitas-msteams-polls-"));
...
const createFsStore = async () => {
  const stateDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "civitas-msteams-polls-"));
