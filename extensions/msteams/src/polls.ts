...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["civitasPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["civitas", "pollId"]) ??
    readNestedString(value, ["civitas", "poll", "id"]) ??
    readNestedString(value, ["data", "civitasPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "civitas", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          civitasPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "civitas poll vote",
          displayText: "Vote recorded",
          value: { civitasPollId: pollId, pollId },
        },
      },
    ],
  };
