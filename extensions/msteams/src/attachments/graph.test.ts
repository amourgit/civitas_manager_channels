...
describe("downloadMSTeamsGraphMedia hosted content $value fallback", () => {
  ...
  it("adds the CIVITAS User-Agent to guarded Graph attachment fetches", async () => {
    ...
    for (const [call] of guardCalls) {
      ...
      expect((headers as Headers).get("User-Agent")).toMatch(
        /^teams\.ts\[apps\]\/.+ CIVITAS\/.+$/,
      );
    }
  });

  it("adds the CIVITAS User-Agent to Graph shares downloads for reference attachments", async () => {
    vi.mocked(fetchWithSsrFGuard).mockImplementation(async (params: { url: string }) => {
      const url = params.url;
    ...
    expect(headers.get("User-Agent")).toMatch(/^teams\.ts\[apps\]\/.+ CIVITAS\/.+$/);
  });
});
