import { withFetchPreconnect } from "civitas/plugin-sdk/testing";
...
describe("graph upload helpers", () => {
  ...
  it("uploads to OneDrive with the personal drive path", async () => {
    ...
    expect(fetchFn).toHaveBeenCalledWith(
      "https://graph.microsoft.com/v1.0/me/drive/root:/CIVITASShared/a.txt:/content",
      expect.objectContaining({
        method: "PUT",
        headers: expect.objectContaining({
          Authorization: "Bearer graph-token",
          "Content-Type": "application/octet-stream",
          "User-Agent": expect.stringMatching(/^teams\.ts\[apps\]\/.+ CIVITAS\/.+$/),
        }),
      }),
    );
  ...
  it("uploads to SharePoint with the site drive path", async () => {
    ...
    expect(fetchFn).toHaveBeenCalledWith(
      "https://graph.microsoft.com/v1.0/sites/site-123/drive/root:/CIVITASShared/b.txt:/content",
      expect.objectContaining({
        method: "PUT",
        headers: expect.objectContaining({
          Authorization: "Bearer graph-token",
