...
type MemoryStorageModule = typeof import("civitas/plugin-sdk/memory-core-host-engine-storage");
...
describe("memory vector dedupe", () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    ({ buildFileEntry } = await import("civitas/plugin-sdk/memory-core-host-engine-storage"));
    ({ createMemoryManagerOrThrow } = await import("./test-manager.js"));
    ({ closeAllMemorySearchManagers } = await import("./index.js"));
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-mem-"));
