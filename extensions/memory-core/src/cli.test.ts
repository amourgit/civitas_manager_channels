...
let defaultRuntime: typeof import("civitas/plugin-sdk/memory-core-host-runtime-cli").defaultRuntime;
let isVerbose: typeof import("civitas/plugin-sdk/memory-core-host-runtime-cli").isVerbose;
let setVerbose: typeof import("civitas/plugin-sdk/memory-core-host-runtime-cli").setVerbose;

beforeAll(async () => {
  ({ registerMemoryCli } = await import("./cli.js"));
  ({ defaultRuntime, isVerbose, setVerbose } =
    await import("civitas/plugin-sdk/memory-core-host-runtime-cli"));
});
...
describe("memory cli", () => {
  ...
  it("documents memory help examples", () => {
    ...
    expect(helpText).toContain("civitas memory status --fix");
    expect(helpText).toContain("Repair stale recall locks and normalize promotion metadata.");
    expect(helpText).toContain("civitas memory status --deep");
    expect(helpText).toContain("Probe embedding provider readiness.");
    expect(helpText).toContain('civitas memory search "meeting notes"');
    expect(helpText).toContain("Quick search using positional query.");
    expect(helpText).toContain('civitas memory search --query "deployment" --max-results 20');
    expect(helpText).toContain("Limit results for focused troubleshooting.");
    expect(helpText).toContain("civitas memory promote --apply");
  ...
  it("shows the fix hint only before --fix has been run", async () => {
    ...
    try {
      ...
      await runMemoryCli(["status"]);
      expect(log).toHaveBeenCalledWith(
        expect.stringContaining("Fix: civitas memory status --fix --agent main"),
      );
