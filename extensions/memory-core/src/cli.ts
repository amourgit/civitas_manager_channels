...
import {
  ...
  formatHelpExamples,
  theme,
} from "civitas/plugin-sdk/memory-core-host-runtime-cli";
import type {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["civitas memory status", "Show index and provider status."],
          [
            "civitas memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["civitas memory status --deep", "Probe embedding provider readiness."],
          ["civitas memory index --force", "Force a full reindex."],
          ['civitas memory search "meeting notes"', "Quick search using positional query."],
          [
            'civitas memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `civitas memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
            "Review weighted short-term candidates for long-term memory.",
          ],
          [
            "civitas memory promote --apply",
            "Append top-ranked short-term candidates into MEMORY.md.",
          ],
          ["civitas memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.civitas.ai/cli/memory")}\n`,
    );
