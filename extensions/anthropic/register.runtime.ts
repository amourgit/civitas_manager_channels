...
import { cloneFirstTemplateModel } from "civitas/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "civitas/plugin-sdk/provider-usage";
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("civitas doctor --yes")}"`,
  ].join("\n");
}
