import type { CIVITASConfig, ProviderAuthResult } from "civitas/plugin-sdk/provider-auth";
import { readClaudeCliCredentialsCached } from "civitas/plugin-sdk/provider-auth";

const DEFAULT_CLAUDE_CLI_MODEL = "claude-cli/claude-sonnet-4-6";
type AgentDefaultsModel = NonNullable<NonNullable<CIVITASConfig["agents"]>["defaults"]>["model"];
type AgentDefaultsModels = NonNullable<NonNullable<CIVITASConfig["agents"]>["defaults"]>["models"];
...
export function buildAnthropicCliMigrationResult(config: CIVITASConfig): ProviderAuthResult {
