...
const hoisted = vi.hoisted(() => ({
  listWhatsAppAccountIds: vi.fn((cfg: CIVITASConfig) => {
  ...
  resolveWhatsAppAccount: vi.fn(
    ({ cfg, accountId }: { cfg: CIVITASConfig; accountId?: string | null }) => ({
      enabled:
...
describe("whatsapp channel action helpers", () => {
  ...
  it("returns null when WhatsApp is not configured", () => {
    expect(
      describeWhatsAppMessageActions({ cfg: {} as CIVITASConfig, accountId: "default" }),
    ).toBeNull();
  });
