...
const CONFIG_KEY = Symbol.for("civitas:testConfigMock");
...
vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/civitas-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("civitas:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("civitas:lastSocket")];
  if (typeof getter === "function") {
