...
import { danger, info, success } from "civitas/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "civitas/plugin-sdk/runtime-env";
import { logInfo } from "civitas/plugin-sdk/text-runtime";
...
export async function loginWeb(
  ...
  try {
    ...
    if (code === LOGGED_OUT_STATUS) {
      ...
      console.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("civitas channels login")} and scan the QR again.`,
        ),
      );
