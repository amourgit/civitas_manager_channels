...
vi.mock("civitas/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("civitas/plugin-sdk/media-runtime")>(
    "civitas/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `civitas-inbound-media-${crypto.randomUUID()}`);
