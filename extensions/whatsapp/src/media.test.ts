...
import { optimizeImageToPng } from "civitas/plugin-sdk/media-runtime";
...
import { captureEnv } from "civitas/plugin-sdk/testing";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredCIVITASTmpDir(), "civitas-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub CIVITAS_STATE_DIR.
    // Also keep it outside the CIVITAS temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["CIVITAS_STATE_DIR"]);
    process.env.CIVITAS_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "civitas-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredCIVITASTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default CIVITAS state workspace and sandbox roots", async () => {
  ...
  it("rejects default CIVITAS state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));

    await expect(
      loadWebMedia(path.join(stateDir, "workspace-CIVITAS Channeldy", "tmp", "render.bin"), {
        maxBytes: 1024 * 1024,
        readFile,
      }),
    ).rejects.toMatchObject({ code: "path-not-allowed" });
  ...
  it("allows per-agent workspace-* paths with explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
    const agentWorkspaceDir = path.join(stateDir, "workspace-CIVITAS Channeldy");
