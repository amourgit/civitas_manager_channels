...
beforeEach(async () => {
  sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-group-gating-"));
...
const makeConfig = (overrides: Record<string, unknown>) =>
  ({
    ...
    ...overrides,
  }) as unknown as ReturnType<typeof import("civitas/plugin-sdk/config-runtime").loadConfig>;

function runGroupGating(params: {
  cfg: ReturnType<typeof import("civitas/plugin-sdk/config-runtime").loadConfig>;
...
function makeInboundCfg(messagePrefix = "") {
  return {
    agents: { defaults: { workspace: "/tmp/civitas" } },
...
describe("applyGroupGating", () => {
  ...
  it("allows group messages when whatsapp groups default disables mention gating", () => {
    const cfg = makeConfig({
      ...
      messages: { groupChat: { mentionPatterns: ["@civitas"] } },
    });
