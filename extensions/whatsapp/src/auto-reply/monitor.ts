import { resolveInboundDebounceMs } from "civitas/plugin-sdk/channel-inbound";
import { formatCliCommand } from "civitas/plugin-sdk/cli-runtime";
import { waitForever } from "civitas/plugin-sdk/cli-runtime";
...
import { DEFAULT_GROUP_HISTORY_LIMIT } from "civitas/plugin-sdk/reply-history";
...
import { getChildLogger } from "civitas/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  type RuntimeEnv,
} from "civitas/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      ...
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("civitas channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("civitas channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
