...
let resetLogger: typeof import("civitas/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("civitas/plugin-sdk/runtime-env").setLoggerOverride;
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("civitas/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `civitas-outbound-${crypto.randomUUID()}.log`);
