...
const WHATSAPP_ACTIVE_LISTENER_STATE_KEY = Symbol.for("civitas.whatsapp.activeListenerState");
...
export function requireActiveWebListener(accountId?: string | null): {
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${id}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`civitas channels login --channel whatsapp --account ${id}`)}.`,
    );
  }
