...
import { VERSION } from "civitas/plugin-sdk/cli-runtime";
import { danger, success } from "civitas/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "civitas/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "civitas/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("civitas channels login")}`,
              ),
            );
          }
        }
