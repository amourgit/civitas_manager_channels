...
const CONFIG_DIR = path.join(process.env.HOME ?? os.tmpdir(), ".civitas");
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("civitas-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("civitas-lid-a-", async (first) => {
      await withTempDir("civitas-lid-b-", (second) => {
