...
import { info, success } from "civitas/plugin-sdk/runtime-env";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("civitas channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
