    ...
    collectIssues: ({ account, accountId, issues }) => {
      ...
      if (!linked) {
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("civitas channels login")} (scan QR on the gateway host).`,
        });
        return;
      }
      ...
      if (healthState === "stale") {
        ...
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("civitas doctor")} (or restart the gateway). If it persists, relink via channels login and check logs.`,
        });
        return;
      }
