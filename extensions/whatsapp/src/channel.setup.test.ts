...
const hoisted = vi.hoisted(() => ({
  ...
  resolveWhatsAppAuthDir: vi.fn(() => ({
    authDir: "/tmp/civitas-whatsapp-test",
  })),
}));
...
vi.mock("civitas/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("civitas/plugin-sdk/setup")>(
    "civitas/plugin-sdk/setup",
  );
...
describe("whatsapp setup wizard", () => {
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/civitas-whatsapp-test" });
  });
  ...
  it("skips relink note when already linked and relink is declined", async () => {
    ...
    expect(harness.note).not.toHaveBeenCalledWith(
      expect.stringContaining("civitas channels login"),
      "WhatsApp",
    );
  });
