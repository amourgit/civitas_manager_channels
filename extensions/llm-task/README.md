...
`CIVITAS.invoke --each`) without adding new CIVITAS code per workflow.
...
This extension depends on CIVITAS internal modules (the embedded agent runner).
It is intended to ship as a **bundled** CIVITAS extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.CIVITAS/extensions` as a standalone plugin directory.
