...
describe("DiffArtifactStore", () => {
  ...
  beforeEach(async () => {
    ...
    } = await createDiffStoreHarness("civitas-diffs-store-"));
  });
...
describe("createDiffsHttpHandler", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("civitas-diffs-http-"));
  });
  ...
  it("serves the shared viewer runtime asset", async () => {
    ...
    expect(handled).toBe(true);
    expect(res.statusCode).toBe(200);
    expect(String(res.body)).toContain("civitasDiffsReady");
  });
