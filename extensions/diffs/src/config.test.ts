...
function compileManifestConfigSchema() {
  const manifest = JSON.parse(
    fs.readFileSync(new URL("../civitas.plugin.json", import.meta.url), "utf8"),
...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/civitas/",
      }),
    ).toBe("https://example.com/civitas");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/civitas?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/civitas#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/civitas/" })).toBe(true);
  });
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/civitas",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/civitas/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("civitasDiffsReady");
