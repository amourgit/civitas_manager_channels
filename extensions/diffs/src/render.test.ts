...
describe("renderDiffDocument", () => {
  ...
  it("renders before/after input into a complete viewer document", async () => {
    ...
    expect(rendered.html).toContain("data-civitas-diff-root");
  ...
  it("resolves viewer assets under an optional base path", async () => {
    ...
    expect(
      new URL(loaderSrc ?? "", "https://example.com/civitas/plugins/diffs/view/id/token").pathname,
    ).toBe("/civitas/plugins/diffs/assets/viewer.js");
  });
  ...
  it("downgrades invalid language hints to plain text", async () => {
    ...
    const payloads = [...html.matchAll(/data-civitas-diff-payload>(.*?)<\/script>/g)].map(
