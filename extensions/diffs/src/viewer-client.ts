...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-civitas-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-civitas-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.civitasDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.civitasDiffsError = "true";
