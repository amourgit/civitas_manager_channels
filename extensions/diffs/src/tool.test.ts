...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("civitas-diffs-tool-"));
  });
  ...
  it("uses configured viewerBaseUrl when tool input omits baseUrl", async () => {
    ...
    expect(readTextContent(result, 0)).toContain(
      "https://example.com/civitas/plugins/diffs/view/",
    );
    expect((result?.details as Record<string, unknown>).viewerUrl).toEqual(
      expect.stringContaining("https://example.com/civitas/plugins/diffs/view/"),
    );
  });
