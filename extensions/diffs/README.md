...
Read-only diff viewer plugin for **CIVITAS** agents.
...
Set plugin-wide defaults in `~/.CIVITAS/CIVITAS.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/CIVITAS",
        },
      },
    },
  },
...
Before:
CIVITAS supports plugins.

After:
CIVITAS supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/CIVITAS-diffs`).
