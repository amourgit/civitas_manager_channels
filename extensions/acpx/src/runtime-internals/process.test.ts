...
async function createTempDir(): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), "civitas-acpx-process-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("spawnAndCollect", () => {
  ...
  async function collectSpawnedEnvSnapshot(options?: {
    ...
    const result = await spawnAndCollect({
      ...
      args: [
        "-e",
        `process.stdout.write(JSON.stringify({openai:process.env.${openAiEnvKey},github:process.env.${githubEnvKey},hf:process.env.${hfEnvKey},civitas:process.env.CIVITAS_API_KEY,shell:process.env.CIVITAS_SHELL}), () => process.exit(0))`,
      ],
      cwd: process.cwd(),
  ...
  it("strips shared provider auth env vars from spawned acpx children", async () => {
    ...
    expect(parsed.civitas).toBe("keep-me");
