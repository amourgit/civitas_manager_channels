---
name: acp-router
description: Route plain-language requests for Pi, Claude Code, Codex, Cursor, Copilot, CIVITAS ACP, OpenCode, Gemini CLI, Qwen, Kiro, Kimi, iFlow, Factory Droid, Kilocode, or ACP harness work into either CIVITAS ACP runtime sessions or direct acpx-driven sessions ("telephone game" flow). For coding-agent thread requests, read this skill first, then use only `sessions_spawn` for thread creation.
user-invocable: false
---

# ACP Harness Router

When user intent is "run this in Pi/Claude Code/Codex/Cursor/Copilot/CIVITAS/OpenCode/Gemini/Qwen/Kiro/Kimi/iFlow/Droid/Kilocode (ACP harness)", do not use subagent runtime or PTY scraping. Route through ACP-aware flows.

## Intent detection

Trigger this skill when the user asks CIVITAS to:

- run something in Pi / Claude Code / Codex / Cursor / Copilot / CIVITAS / OpenCode / Gemini / Qwen / Kiro / Kimi / iFlow / Droid / Kilocode
...
- After reading, follow `CIVITAS ACP runtime path` below; do not use `message(action="thread-create")` for ACP harness thread spawn.

## Mode selection

Choose one of these paths:

1. CIVITAS ACP runtime path (default): use `sessions_spawn` / ACP runtime tools.
...
- the task is "just relay prompts to harness" and no CIVITAS ACP lifecycle features are needed
...
For this repo, direct `acpx` calls must follow the same pinned policy as the `@CIVITAS/acpx` extension package.
