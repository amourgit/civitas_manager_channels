  ...
  it("uses interactions.callbackBaseUrl when configured", () => {
    const url = resolveInteractionCallbackUrl("default", {
      channels: {
        mattermost: {
          interactions: {
            callbackBaseUrl: "https://gateway.example.com/civitas",
          },
        },
      },
    });
    expect(url).toBe("https://gateway.example.com/civitas/mattermost/interactions/default");
  });
...
describe("createMattermostInteractionHandler", () => {
  ...
  function createActionContext(actionId = "approve", channelId = "chan-1") {
    const context = { action_id: actionId, __civitas_channel_id: channelId };
