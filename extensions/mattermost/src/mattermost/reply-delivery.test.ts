...
import type { ChunkMode } from "civitas/plugin-sdk/reply-runtime";
...
describe("deliverMattermostReplyPayload", () => {
  it("passes agent-scoped mediaLocalRoots when sending media paths", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-mm-state-"));
    ...
    try {
      ...
      const cfg = {} satisfies CIVITASConfig;
