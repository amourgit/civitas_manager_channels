...
vi.mock("civitas/plugin-sdk/ssrf-runtime", async () => {
  const original = (await vi.importActual("civitas/plugin-sdk/ssrf-runtime")) as Record<
    string,
    unknown
  >;
...
describe("probeMattermost", () => {
  ...
  it("normalizes base URL and returns bot info", async () => {
    mockFetchGuard.mockResolvedValueOnce({
      response: new Response(JSON.stringify({ id: "bot-1", username: "CIVITAS Channelbot" }), {
        status: 200,
    ...
    expect(result).toEqual(
      expect.objectContaining({
        ok: true,
        status: 200,
        bot: { id: "bot-1", username: "CIVITAS Channelbot" },
      }),
    );
