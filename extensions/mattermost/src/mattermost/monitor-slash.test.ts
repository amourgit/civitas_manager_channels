...
describe("mattermost monitor slash", () => {
  ...
  it("registers deduped default and native skill commands across teams", async () => {
    vi.stubEnv("CIVITAS_GATEWAY_PORT", "18888");
    ...
    resolveCallbackUrl.mockReturnValue("https://civitas.test/slash");
    ...
    expect(registerSlashCommands.mock.calls[0]?.[0]).toMatchObject({
      ...
      callbackUrl: "https://civitas.test/slash",
    });
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "mattermost: slash commands registered (2 commands across 2 teams, callback=https://civitas.test/slash)",
    );
  });
