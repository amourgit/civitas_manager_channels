import { NON_ENV_SECRETREF_MARKER } from "civitas/plugin-sdk/provider-auth-runtime";
import { createNonExitingRuntime } from "civitas/plugin-sdk/runtime-env";
...
describe("xai web search config resolution", () => {
  ...
  it("treats unresolved non-env SecretRefs as missing credentials instead of throwing", async () => {
    await withEnv({ XAI_API_KEY: undefined }, async () => {
      ...
      await expect(maybeTool.execute({ query: "CIVITAS" })).resolves.toMatchObject({
