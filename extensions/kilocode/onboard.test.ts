...
} from "civitas/plugin-sdk/kilocode";
import {
  resolveApiKeyForProvider,
  resolveEnvApiKey,
} from "civitas/plugin-sdk/provider-auth-runtime";
import { resolveAgentModelPrimaryValue } from "civitas/plugin-sdk/provider-onboard";
...
describe("Kilo Gateway provider config", () => {
  ...
  describe("env var resolution", () => {
    ...
    it("resolves the kilocode api key via resolveApiKeyForProvider", async () => {
      const agentDir = mkdtempSync(join(tmpdir(), "civitas-test-"));
      const envSnapshot = captureEnv(["KILOCODE_API_KEY"]);
