    ...
    if (parts.length === 0) {
      await prompter.note(
        [
          ...
          `Tip: use \`${formatCliCommand("civitas directory peers list --channel zalouser")}\` to look up people after onboarding.`,
        ].join("\n"),
      ...
      if (entries.length === 0) {
        await prompter.note(
          [
            ...
            `Tip: use \`${formatCliCommand("civitas directory groups list --channel zalouser")}\` after onboarding to find group IDs.`,
