...
function resolveQianfanPreset(cfg: CIVITASConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: CIVITASConfig) => {
...
export function applyQianfanProviderConfig(cfg: CIVITASConfig): CIVITASConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: CIVITASConfig): CIVITASConfig {
