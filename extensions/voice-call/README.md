...
Official Voice Call plugin for **CIVITAS**.
...
Docs: `https://docs.CIVITAS.ai/plugins/voice-call`
Plugin system: `https://docs.CIVITAS.ai/plugin`

## Install (local dev)

### Option A: install via CIVITAS (recommended)

```bash
CIVITAS plugins install @CIVITAS/voice-call
```
...
```bash
PLUGIN_HOME=~/.CIVITAS/extensions
mkdir -p "$PLUGIN_HOME"
...
- If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `CIVITAS doctor --fix` to rewrite them.
- advanced webhook, streaming, and tunnel notes: `https://docs.CIVITAS.ai/plugins/voice-call`
...
`https://docs.CIVITAS.ai/plugins/voice-call#stale-call-reaper`

## TTS for calls

Voice Call uses the core `messages.tts` configuration for
streaming speech on calls. Override examples and provider caveats live here:
`https://docs.CIVITAS.ai/plugins/voice-call#tts-for-calls`

## CLI

```bash
CIVITAS voicecall call --to "+15555550123" --message "Hello from CIVITAS"
CIVITAS voicecall continue --call-id <id> --message "Any questions?"
CIVITAS voicecall speak --call-id <id> --message "One moment"
CIVITAS voicecall end --call-id <id>
CIVITAS voicecall status --call-id <id>
CIVITAS voicecall tail
CIVITAS voicecall expose --mode funnel
```
