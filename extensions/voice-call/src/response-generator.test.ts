...
function createAgentRuntime(payloads: Array<Record<string, unknown>>) {
  ...
  const runtime = {
    ...
    resolveAgentDir: () => "/tmp/civitas/agents/main",
    resolveAgentWorkspaceDir: () => "/tmp/civitas/workspace/main",
    ...
    runEmbeddedPiAgent,
    session: {
      resolveStorePath: () => "/tmp/civitas/sessions.json",
      loadSessionStore: () => ({}),
      saveSessionStore: async () => {},
      resolveSessionFilePath: () => "/tmp/civitas/sessions/session.jsonl",
    },
