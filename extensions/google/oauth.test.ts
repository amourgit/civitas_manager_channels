...
describe("loginGeminiCliOAuth", () => {
  ...
  beforeEach(() => {
    ...
    process.env.CIVITAS_GEMINI_OAUTH_CLIENT_ID = "test-client-id.apps.googleusercontent.com";
    process.env.CIVITAS_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret"; // pragma: allowlist secret
  ...
  it("falls back across loadCodeAssist endpoints with aligned headers and metadata", async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === USERINFO_URL) {
        return responseJson({ email: "lobster@civitas.ai" });
      }
      if (url === LOAD_PROD) {
