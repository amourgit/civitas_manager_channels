...
function normalizeDenyList(cfg: CIVITASPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: CIVITASPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: CIVITASPluginApi) {
    ...
    const timerService: CIVITASPluginService = {
