...
import type { DiscordGuildEntry } from "civitas/plugin-sdk/config-runtime";
import type { CIVITASConfig } from "civitas/plugin-sdk/config-runtime";
import type { ChannelSetupDmPolicy, ChannelSetupWizard } from "civitas/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "civitas/plugin-sdk/setup-runtime";
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: CIVITASConfig; accountId: string }) => {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: CIVITASConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: CIVITASConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: CIVITASConfig; accountId: string }) =>
