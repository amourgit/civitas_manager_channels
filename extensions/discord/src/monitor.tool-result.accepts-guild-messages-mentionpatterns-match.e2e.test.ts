  ...
  it("accepts guild messages when mentionPatterns match", async () => {
    const cfg = createMentionRequiredGuildConfig({
      messages: {
        ...
        groupChat: { mentionPatterns: ["\\bcivitas\\b"] },
      },
    } as Partial<Config>);
    ...
    await handler(createGuildMessageEvent({ messageId: "m2", content: "civitas: hello" }), client);
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "CIVITAS" },
          },
        },
      }),
      client,
