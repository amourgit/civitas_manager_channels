...
describe("DiscordMessageListener", () => {
  ...
  it("logs handler failures", async () => {
    const logger = {
      ...
      typeof import("civitas/plugin-sdk/logging-core").createSubsystemLogger
    >;
    const handler = vi.fn(async () => {
...
describe("discord allowlist helpers", () => {
  ...
  it("matches ids by default and names only when enabled", () => {
    ...
    expect(allowListMatches(allow, { name: "friends-of-civitas" })).toBe(false);
    expect(allowListMatches(allow, { name: "steipete" }, { allowNameMatching: true })).toBe(true);
    expect(
      allowListMatches(allow, { name: "friends-of-civitas" }, { allowNameMatching: true }),
    ).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    ...
    expect(resolved?.slug).toBe("friends-of-civitas");
  });
...
const channelRuntimeModule = await import("civitas/plugin-sdk/infra-runtime");
vi.spyOn(channelRuntimeModule, "enqueueSystemEvent").mockImplementation(enqueueSystemEventSpy);

const routingModule = await import("civitas/plugin-sdk/routing");
...
function makeReactionListenerParams(overrides?: {
  ...
  return {
    cfg: {} as ReturnType<typeof import("civitas/plugin-sdk/config-runtime").loadConfig>,
    accountId: "acc-1",
    runtime: {} as import("civitas/plugin-sdk/runtime-env").RuntimeEnv,
