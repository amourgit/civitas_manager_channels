...
const { STORE_PATH, mockSessionStoreEntries } = vi.hoisted(() => ({
  STORE_PATH: "/tmp/civitas-exec-approvals-test.json",
...
vi.mock("civitas/plugin-sdk/text-runtime", async () => {
  const actual = await vi.importActual<typeof import("civitas/plugin-sdk/text-runtime")>(
    "civitas/plugin-sdk/text-runtime",
  );
  return {
    ...actual,
    logDebug: vi.fn(),
