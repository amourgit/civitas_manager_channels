...
function createThreadBinding(
  overrides?: Partial<import("civitas/plugin-sdk/conversation-runtime").SessionBindingRecord>,
) {
  return {
    ...
    ...overrides,
  } satisfies import("civitas/plugin-sdk/conversation-runtime").SessionBindingRecord;
}
...
async function runThreadBoundPreflight(params: {
  ...
  threadBinding: import("civitas/plugin-sdk/conversation-runtime").SessionBindingRecord;
  discordConfig: DiscordConfig;
...
describe("preflightDiscordMessage", () => {
        ...
        ref.conversationId === "user:user-1"
          ? createThreadBinding({
              ...
              metadata: {
                ...
                pluginRoot: "/Users/huntharo/github/civitas-app-server",
              },
            })
          : null,
    });
  ...
  it("allows bot messages with explicit mention when allowBots=mentions", async () => {
    ...
    const message = createDiscordMessage({
      ...
      channelId,
      content: "hi <@civitas-bot>",
      mentionedUsers: [{ id: "civitas-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
        username: "Relay",
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hey civitas");
  ...
  it("drops guild message without mention when channel has configuredBinding and requireMention: true", async () => {
    const conversationRuntime = await import("civitas/plugin-sdk/conversation-runtime");
