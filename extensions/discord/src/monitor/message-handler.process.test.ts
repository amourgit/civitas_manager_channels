import { DEFAULT_EMOJIS } from "civitas/plugin-sdk/channel-feedback";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/civitas-discord-process-test-sessions.json",
  ),
}));
...
const replyRuntimeModule = await import("civitas/plugin-sdk/reply-runtime");
...
const conversationRuntimeModule = await import("civitas/plugin-sdk/conversation-runtime");
...
const configRuntimeModule = await import("civitas/plugin-sdk/config-runtime");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/civitas-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/civitas-discord-process-test-sessions.json" },
      },
    });
