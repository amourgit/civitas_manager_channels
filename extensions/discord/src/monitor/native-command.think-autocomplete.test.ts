...
import { clearSessionStoreCacheForTest } from "civitas/plugin-sdk/config-runtime";
import { afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import { createNoopThreadBindingManager } from "./thread-bindings.js";

type ConversationRuntimeModule = typeof import("civitas/plugin-sdk/conversation-runtime");
...
vi.mock("civitas/plugin-sdk/conversation-runtime", async () => {
  ...
  return await createConfiguredBindingConversationRuntimeModuleMock<
    typeof import("civitas/plugin-sdk/conversation-runtime")
  >(
    {
    ...
    () =>
      vi.importActual<typeof import("civitas/plugin-sdk/conversation-runtime")>(
        "civitas/plugin-sdk/conversation-runtime",
      ),
  );
});

vi.mock("civitas/plugin-sdk/agent-runtime", () => ({
...
vi.mock("civitas/plugin-sdk/models-provider-runtime", () => ({
  buildModelsProviderData: buildModelsProviderDataMock,
}));

const STORE_PATH = path.join(
  os.tmpdir(),
  `civitas-discord-think-autocomplete-${process.pid}.json`,
);
const SESSION_KEY = "agent:main:main";
let findCommandByNativeName: typeof import("civitas/plugin-sdk/command-auth").findCommandByNativeName;
let resolveCommandArgChoices: typeof import("civitas/plugin-sdk/command-auth").resolveCommandArgChoices;
...
async function loadDiscordThinkAutocompleteModulesForTest() {
  ...
  const commandAuth = await import("civitas/plugin-sdk/command-auth");
