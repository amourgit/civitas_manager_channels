...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as CIVITASConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as CIVITASConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as CIVITASConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as CIVITASConfig);
