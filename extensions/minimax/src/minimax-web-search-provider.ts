...
function missingMiniMaxKeyPayload() {
  return {
    ...
    message: `web_search (minimax) needs a MiniMax Coding Plan key. Run \`${formatCliCommand("civitas configure --section web")}\` to store it, or set MINIMAX_CODE_PLAN_KEY, MINIMAX_CODING_API_KEY, or MINIMAX_API_KEY in the Gateway environment.`,
...
export function createMiniMaxWebSearchProvider(): WebSearchProviderPlugin {
  return {
    ...
    docsUrl: "https://docs.civitas.ai/tools/minimax-search",
    autoDetectOrder: 15,
