...
function getChannelConfig(cfg: CIVITASConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: CIVITASConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: CIVITASConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: CIVITASConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL CIVITAS uses to send replies back to Chat.",
      ],
