...
describe("irc setup", () => {
  ...
  it("validates required input and applies normalized account config", () => {
    ...
    expect(
      validateInput!({
        input: { host: "irc.libera.chat", nick: "civitas" },
      } as never),
    ).toBeNull();
  ...
  it("configures host and nick via setup prompts", async () => {
    ...
    expect(result.cfg.channels?.irc?.nick).toBe("civitas-bot");
    expect(result.cfg.channels?.irc?.tls).toBe(true);
    expect(result.cfg.channels?.irc?.channels).toEqual(["#civitas", "#ops"]);
    expect(result.cfg.channels?.irc?.groupPolicy).toBe("allowlist");
    expect(Object.keys(result.cfg.channels?.irc?.groups ?? {})).toEqual(["#civitas", "#ops"]);
  });
