...
import type { ChannelSetupDmPolicy } from "civitas/plugin-sdk/setup";
import type { ChannelSetupWizard } from "civitas/plugin-sdk/setup";
import { formatDocsLink } from "civitas/plugin-sdk/setup";
import type { WizardPrompter } from "civitas/plugin-sdk/setup";
...
export const ircSetupWizard: ChannelSetupWizard = {
  ...
  textInputs: [
    ...
    },
    {
      ...
      initialValue: ({ cfg, accountId }) =>
        resolveIrcAccount({ cfg: cfg as CoreConfig, accountId }).config.realname || "CIVITAS",
      validate: ({ value }) => (String(value ?? "").trim() ? undefined : "Required"),
      normalizeValue: ({ value }) => String(value).trim(),
      applySet: async ({ cfg, accountId, value }) =>
        updateIrcAccountConfig(cfg as CoreConfig, accountId, {
  ...
  completionNote: {
    ...
    lines: [
      ...
      "Command: civitas channels status --probe",
      `Docs: ${formatDocsLink("/channels/irc", "channels/irc")}`,
    ],
  },
