...
const createBackupArchiveMock = vi.hoisted(() =>
  vi.fn(async (_params: unknown) => ({
    ...
    archiveRoot: "2026-03-17-civitas-backup",
...
describe("matrix client storage paths", () => {
  ...
  function setupStateDir(
    ...
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-matrix-storage-"));
    const stateDir = path.join(homeDir, ".civitas");
  ...
  function createMigrationEnv(stateDir: string): NodeJS.ProcessEnv {
    return {
      ...
      CIVITAS_HOME: path.dirname(stateDir),
