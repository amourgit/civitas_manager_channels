...
import { withFileLock } from "civitas/plugin-sdk/infra-runtime";
...
// `civitas matrix verify bootstrap`) can corrupt each other's state.
...
function resolveDefaultIdbSnapshotPath(): string {
  const stateDir =
    process.env.OPENCLAW_STATE_DIR || path.join(process.env.HOME || "/tmp", ".civitas");
