...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another CIVITAS runtime is using the same account. " +
    `Check 'civitas matrix verify status --verbose --account ${accountId}' and 'civitas matrix devices list --account ${accountId}'.`
  );
}
