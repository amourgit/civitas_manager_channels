...
const CIVITAS_PLUGIN_SDK_PREFIX = ["civitas", "plugin-sdk"].join("/");
...
function findCIVITASPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "civitas" && pkg.exports?.["./plugin-sdk"]) {
...
function buildPluginSdkAliasMap(moduleUrl) {
  const location = findCIVITASPackageRoot(path.dirname(fileURLToPath(moduleUrl)));
  if (!location) {
    return {};
  }
  ...
  if (rootAlias) {
    aliasMap[CIVITAS_PLUGIN_SDK_PREFIX] = rootAlias;
  }
  ...
  for (const exportKey of Object.keys(packageJson.exports ?? {})) {
    ...
    if (resolvedPath) {
      aliasMap[`${CIVITAS_PLUGIN_SDK_PREFIX}/${subpath}`] = resolvedPath;
    }
  }
  ...
  if (extensionApi) {
    aliasMap["civitas/extension-api"] = extensionApi;
  }
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findCIVITASPackageRoot(moduleDir);
  if (location) {
