...
function formatMatrixCliCommand(command: string, accountId?: string): string {
  ...
  return `civitas matrix ${command}${suffix}`;
}
...
export function registerMatrixCli(params: { program: Command }): void {
  const root = params.program
    ...
    .addHelpText("after", () => "\nDocs: https://docs.civitas.ai/channels/matrix\n");
      ...
      async (options: {
        ...
        await runMatrixCliCommand({
          ...
          onText: (result) => {
            ...
            const bindHint = `civitas agents bind --agent <id> --bind matrix:${result.accountId}`;
