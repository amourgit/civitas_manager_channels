...
import { writeJsonFileAtomically as writeJsonFileAtomicallyImpl } from "civitas/plugin-sdk/json-store";
    ...
    if (!stat.isDirectory()) {
      return {
        ...
        warning:
          ...
          "CIVITAS skipped automatic crypto migration for that path.",
      };
    }
  } catch (err) {
    return {
...
function resolveMatrixAccountIds(cfg: CIVITASConfig): string[] {
    ...
    if (!plan.deviceId) {
      warnings.push(
        `Legacy Matrix encrypted state detected at ${plan.legacyCryptoPath}, but no device ID was found for account "${plan.accountId}". ` +
          `CIVITAS will continue, but old encrypted history cannot be recovered automatically.`,
      );
      continue;
    }
    ...
    if (!summary.decryptionKeyBase64 && (summary.roomKeyCounts?.backedUp ?? 0) > 0) {
      warnings.push(
        `Legacy Matrix encrypted state for account "${plan.accountId}" has backed-up room keys, but no local backup decryption key was found. ` +
          `Ask the operator to run "civitas matrix verify backup restore --recovery-key <key>" after upgrade if they have the recovery key.`,
      );
    }
