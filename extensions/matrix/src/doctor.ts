...
} from "civitas/plugin-sdk/runtime";
import {
  ...
  for (const plan of detection.plans) {
    notes.push(
      [
        ...
        '- Run "civitas doctor --fix" to extract any saved backup key now. Backed-up room keys will restore automatically on next gateway start.',
      ].join("\n"),
    );
  }
  return notes;
...
export async function collectMatrixInstallPathWarnings(cfg: CIVITASConfig): Promise<string[]> {
  ...
  return formatPluginInstallPathIssue({
    issue,
    pluginLabel: "Matrix",
    defaultInstallCommand: "civitas plugins install @civitas/matrix",
  }).map((entry) => `- ${entry}`);
}
...
export async function applyMatrixDoctorRepair(params: {
  ...
  if (actionableMatrixMigration) {
    try {
      ...
      warnings.push(
        '- Skipping Matrix migration changes for now. Resolve the snapshot failure, then rerun "civitas doctor --fix".',
      );
    }
