...
import { formatZonedTimestamp } from "civitas/plugin-sdk/matrix-runtime-shared";
...
describe("matrix CLI verification commands", () => {
  ...
  it("lists matrix devices", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith("- A7hWrQ70ea (current, CIVITAS Gateway)");
    expect(console.log).toHaveBeenCalledWith("  Last IP: 127.0.0.1");
    expect(console.log).toHaveBeenCalledWith("- BritdXC6iL (CIVITAS Gateway)");
  });
  ...
  it("prunes stale matrix gateway devices", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith("Deleted stale CIVITAS devices: BritdXC6iL");
  ...
  it("adds a matrix account and prints a binding hint", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "Bind this account to an agent: civitas agents bind --agent <id> --bind matrix:ops",
    );
  });
  ...
  it("bootstraps verification for newly added encrypted accounts", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "Matrix device hygiene warning: stale CIVITAS devices detected (BritdXC6iL). Run 'civitas matrix devices prune-stale --account ops'.",
    );
  });
  ...
  it("uses --name as fallback account id and prints account-scoped config path", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "Bind this account to an agent: civitas agents bind --agent <id> --bind matrix:main-bot",
    );
  });
  ...
  it("shows explicit backup issue in default status output", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "- Backup key is not loaded on this device. Run 'civitas matrix verify backup restore' to load it and restore old room keys.",
    );
    expect(console.log).not.toHaveBeenCalledWith(
      "- Backup is present but not trusted for this device. Re-run 'civitas matrix verify device <key>'.",
    );
  });
  ...
  it("includes backup reset guidance when the backup key does not match this device", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "- If you want a fresh backup baseline and accept losing unrecoverable history, run 'civitas matrix verify backup reset --yes'. This may also repair secret storage so the new backup key can be loaded after restart.",
    );
  });
  ...
  it("prints resolved account-aware guidance when a named Matrix account is selected implicitly", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "- Run 'civitas matrix verify device <key> --account assistant' to verify this device.",
    );
    expect(console.log).toHaveBeenCalledWith(
      "- Run 'civitas matrix verify bootstrap --account assistant' to create a room key backup.",
    );
  });
