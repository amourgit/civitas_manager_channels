  ...
  if (!channel) {
    return {
      warning:
        ...
        'Configure Matrix, then rerun "civitas doctor --fix" or restart the gateway.',
    };
  }
  if (requiresExplicitMatrixDefaultAccount(params.cfg)) {
    return {
      warning:
        ...
        'Set "channels.matrix.defaultAccount" to the intended target account before rerunning "civitas doctor --fix" or restarting the gateway.',
    };
  }
  ...
  if (!target) {
    ...
    return {
      warning:
        ...
        'Start the gateway once with a working Matrix login, or rerun "civitas doctor --fix" after cached credentials are available.',
    };
  }
