...
import { encodePngRgba, fillPixel } from "civitas/plugin-sdk/media-runtime";
import type { ResolvedTtsConfig } from "civitas/plugin-sdk/speech-runtime";
...
const LIVE_MODEL_ID = process.env.CIVITAS_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.4-nano";
const LIVE_IMAGE_MODEL = process.env.CIVITAS_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-1";
const LIVE_VISION_MODEL = process.env.CIVITAS_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-4.1-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.CIVITAS_LIVE_TEST === "1";
...
function createLiveConfig(): CIVITASConfig {
...
describeLive("openai plugin live", () => {
  ...
  it("lists voices and synthesizes audio through the registered speech provider", async () => {
    ...
    const audioFile = await speechProvider.synthesize({
      text: "CIVITAS integration test OK.",
      cfg,
