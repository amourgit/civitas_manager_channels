...
import { buildOauthProviderAuthResult } from "civitas/plugin-sdk/provider-auth";
import { loginOpenAICodexOAuth } from "civitas/plugin-sdk/provider-auth-login";
import {
  DEFAULT_CONTEXT_TOKENS,
  normalizeModelCompat,
  normalizeProviderId,
  type ProviderPlugin,
} from "civitas/plugin-sdk/provider-model-shared";
import { buildProviderStreamFamilyHooks } from "civitas/plugin-sdk/provider-stream";
import { fetchCodexUsage } from "civitas/plugin-sdk/provider-usage";
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `civitas models auth login --provider openai-codex` or `civitas configure`.";
}
