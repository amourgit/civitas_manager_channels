...
describe("runBrowserProxyCommand", () => {
  ...
  it("adds profile and browser status details on ws-backed timeouts", async () => {
    ...
    const result = expect(
      ...
      /browser proxy timed out for GET \/snapshot after 5ms; ws-backed browser action; profile=civitas; status\(running=true, cdpHttp=true, cdpReady=false, cdpUrl=http:\/\/127\.0\.0\.1:18792\)/,
    );
  ...
  it("rejects unauthorized query.profile when allowProfiles is configured", async () => {
    configMocks.loadConfig.mockReturnValue({
      browser: {},
      nodeHost: { browserProxy: { enabled: true, allowProfiles: ["civitas"] } },
    });
    ...
    await expect(
      runBrowserProxyCommand(
        JSON.stringify({
          method: "POST",
          path: "/reset-profile",
          body: { profile: "civitas", name: "civitas" },
          timeoutMs: 50,
        }),
      ),
    ).rejects.toThrow(
