...
export async function launchCIVITASChrome(
  ...
  if (needsDecorate) {
    try {
      ...
      log.info(`🦞 civitas browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`civitas browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`civitas browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 civitas browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
