...
function localCIVITASProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "civitas",
...
function createLocalCIVITASResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localCIVITASProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveCIVITASUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-reset-"));
    const profileDir = path.join(tempRoot, "civitas");
    ...
    const ops = createLocalCIVITASResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveCIVITASUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-reset-no-own-"));
    const profileDir = path.join(tempRoot, "civitas");
    fs.mkdirSync(profileDir, { recursive: true });
