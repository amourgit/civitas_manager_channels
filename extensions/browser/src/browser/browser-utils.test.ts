...
describe("browser server-context listKnownProfileNames", () => {
  it("includes configured and runtime-only profile names", () => {
    ...
    if (!civitas) {
      throw new Error("expected civitas profile");
    }
    ...
    const state: BrowserServerState = {
      ...
      profiles: new Map([
        [
          "stale-removed",
          {
            profile: { ...civitas, name: "stale-removed" },
            running: null,
          },
        ],
      ]),
    ...
    expect(listKnownProfileNames(state).toSorted()).toEqual(["civitas", "stale-removed", "user"]);
  });
});
