...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("civitas");
    expect(profile?.driver).toBe("civitas");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const civitas = resolveProfile(resolved, "civitas");
    expect(civitas?.driver).toBe("civitas");
    expect(civitas?.cdpPort).toBe(18800);
    expect(civitas?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from CIVITAS_GATEWAY_PORT when unset", () => {
    withEnv({ CIVITAS_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const civitas = resolveProfile(resolved, "civitas");
      expect(civitas?.cdpPort).toBe(19012);
      expect(civitas?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ CIVITAS_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const civitas = resolveProfile(resolved, "civitas");
      expect(civitas?.cdpPort).toBe(19022);
      expect(civitas?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(civitas?.cdpPort).toBe(19000);
    expect(civitas?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "civitas");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "civitas")!;
  ...
  describe("default profile preference", () => {
    it("defaults to civitas profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("civitas");
    });

    it("keeps civitas default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("civitas");
    ...
    it("keeps civitas default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("civitas");
    ...
    it("keeps civitas default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
