...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".civitas-profile-decorated");
}
 ...
 */
export function decorateCIVITASProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_CIVITAS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_CIVITAS_BROWSER_COLOR).toUpperCase();
