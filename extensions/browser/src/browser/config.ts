import type { BrowserConfig, BrowserProfileConfig, CIVITASConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_CIVITAS_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  userDataDir?: string;
  color: string;
  driver: "civitas" | "existing-session";
  attachOnly: boolean;
};
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_CIVITAS_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "civitas" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_CIVITAS_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_CIVITAS_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  rootConfig?: CIVITASConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_CIVITAS_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_CIVITAS_BROWSER_PROFILE_NAME]
        ? DEFAULT_CIVITAS_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "civitas";
