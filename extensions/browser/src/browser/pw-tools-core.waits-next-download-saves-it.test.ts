...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredCIVITASTmpDir: vi.fn(() => "/tmp/civitas"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-civitas-dir.js");

describe("pw-tools-core", () => {
  beforeAll(async () => {
    vi.doMock("./pw-session.js", () => sessionMocks);
    vi.doMock("./chrome.js", () => chromeMocks);
    tmpDirModule = await import("../infra/tmp-civitas-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredCIVITASTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredCIVITASTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredCIVITASTmpDir.mockReturnValue("/tmp/civitas");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".civitas-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredCIVITASTmpDir.mockReturnValue("/tmp/civitas-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "civitas-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "civitas-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredCIVITASTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "civitas-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "civitas-preferred", "downloads")}${path.sep}`),
    );
  });
