...
import { buildCIVITASChromeLaunchArgs } from "./chrome.js";
...
describe("browser chrome launch args", () => {
  it("does not force an about:blank tab at startup", () => {
    const args = buildCIVITASChromeLaunchArgs({
      ...
      userDataDir: "/tmp/civitas-test-user-data",
    });

    expect(args).not.toContain("about:blank");
    expect(args).toContain("--remote-debugging-port=18800");
    expect(args).toContain("--user-data-dir=/tmp/civitas-test-user-data");
  });
});
