...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["civitas", "user"]);
    ...
    expect(stopMap.civitas).toHaveBeenCalledTimes(1);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("civitas browser stop failed: Error: oops");
  });
});
