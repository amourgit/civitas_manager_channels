...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-civitas-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-civitas-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-civitas-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function resolveBrowserFetchOperatorHint(url: string): string {
  ...
  return isLocal
    ? `Restart the CIVITAS gateway (CIVITAS.app menubar, or \`${formatCliCommand("civitas gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      appendBrowserToolModelHint(
        `Can't reach the CIVITAS browser control service (timed out after ${timeoutMs}ms). ${operatorHint}`,
      ),
    );
  }
  return new Error(
    appendBrowserToolModelHint(
      `Can't reach the CIVITAS browser control service. ${operatorHint} (${msg})`,
    ),
  );
}
