...
import { MEDIA_MAX_BYTES } from "civitas/plugin-sdk/media-runtime";
...
describe("persistBrowserProxyFiles", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("civitas-browser-proxy-files-");
  });
  ...
  it("persists browser proxy files under the shared media store", async () => {
    ...
    expect(path.normalize(savedPath ?? "")).toContain(
      `${path.sep}.civitas${path.sep}media${path.sep}browser${path.sep}`,
    );
  ...
  it("rejects browser proxy files that exceed the shared media size limit", async () => {
    ...
    await expect(
      fs.stat(path.join(tempHome.home, ".civitas", "media", "browser")),
    ).rejects.toThrow();
  });
});
