...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => CIVITASConfig>(),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<CIVITASConfig["gateway"]>["auth"]> | undefined;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: CIVITASConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
