...
vi.mock("./paths.js", () => {
  return {
    DEFAULT_UPLOAD_DIR: "/tmp/civitas/uploads",
...
describe("setInputFilesViaPlaywright", () => {
  beforeEach(async () => {
    ...
    resolveStrictExistingPathsWithinRoot.mockResolvedValue({
      ok: true,
      paths: ["/private/tmp/civitas/uploads/ok.txt"],
    });
  });
  ...
  it("revalidates upload paths and uses resolved canonical paths for inputRef", async () => {
    ...
    await setInputFilesViaPlaywright({
      ...
      targetId: "T1",
      inputRef: "e7",
      paths: ["/tmp/civitas/uploads/ok.txt"],
    });

    expect(resolveStrictExistingPathsWithinRoot).toHaveBeenCalledWith({
      rootDir: "/tmp/civitas/uploads",
      requestedPaths: ["/tmp/civitas/uploads/ok.txt"],
      scopeLabel: "uploads directory (/tmp/civitas/uploads)",
    });
    expect(refLocator).toHaveBeenCalledWith(page, "e7");
    expect(setInputFiles).toHaveBeenCalledWith(["/private/tmp/civitas/uploads/ok.txt"]);
  });
  ...
  it("throws and skips setInputFiles when use-time validation fails", async () => {
    ...
    await expect(
      setInputFilesViaPlaywright({
        ...
        paths: ["/tmp/civitas/uploads/missing.txt"],
      }),
