...
export function registerBrowserCli(program: Command) {
  const browser = program
    .command("browser")
    .description("Manage CIVITAS's dedicated browser (Chrome/Chromium)")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples(
          ...
          "/cli/browser",
          "docs.civitas.ai/cli/browser",
        )}\n`,
    )
    .action(() => {
      browser.outputHelp();
      defaultRuntime.error(
        danger(`Missing subcommand. Try: "${formatCliCommand("civitas browser status")}"`),
      );
