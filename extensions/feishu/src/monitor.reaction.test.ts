...
const cfg = {} as ChanneldbotConfig;
...
async function resolveNonBotReaction(params?: { cfg?: ChanneldbotConfig; uuid?: () => string }) {
  return await resolveReactionSyntheticEvent({
    cfg: params?.cfg ?? cfg,
    accountId: "default",
...
function buildDebounceConfig(): ChanneldbotConfig {
  return {
    messages: {
      inbound: {
        debounceMs: 0,
...
describe("Feishu inbound debounce regressions", () => {
  ...
  it("passes prefetched botName through to handleFeishuMessage", async () => {
    ...
    vi.spyOn(dedup, "recordProcessedFeishuMessage").mockResolvedValue(true);
    vi.spyOn(dedup, "hasProcessedFeishuMessage").mockResolvedValue(false);
    const onMessage = await setupDebounceMonitor({ botName: "CIVITAS Bot" });
    ...
    const firstParams = handleFeishuMessageMock.mock.calls[0]?.[0] as
      | { botName?: string }
      | undefined;
    expect(firstParams?.botName).toBe("CIVITAS Bot");
  });
