...
export async function maybeCreateDynamicAgent(params: {
  ...
  if (existingAgent) {
    ...
    const updatedCfg: CIVITASConfig = {
      ...cfg,
      bindings: [
        ...existingBindings,
        {
  ...
  const workspaceTemplate = dynamicCfg.workspaceTemplate ?? "~/.civitas/workspace-{agentId}";
  const agentDirTemplate = dynamicCfg.agentDirTemplate ?? "~/.civitas/agents/{agentId}/agent";
