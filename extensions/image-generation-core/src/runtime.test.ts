...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    getImageGenerationProvider: vi.fn<
      (providerId: string, config?: CIVITASConfig) => ImageGenerationProvider | undefined
    >(() => undefined),
    getProviderEnvVars: vi.fn<(providerId: string) => string[]>(() => []),
    isFailoverError: vi.fn<(err: unknown) => boolean>(() => false),
    listImageGenerationProviders: vi.fn<(config?: CIVITASConfig) => ImageGenerationProvider[]>(
      () => [],
    ),
...
describe("image-generation runtime", () => {
  ...
  it("lists runtime image-generation providers through the owner runtime", () => {
    ...
    expect(listRuntimeImageGenerationProviders({ config: {} as CIVITASConfig })).toEqual(
      providers,
    );
    expect(mocks.listImageGenerationProviders).toHaveBeenCalledWith({} as CIVITASConfig);
  });
  ...
  it("explains native image-generation config and provider auth when no model is configured", async () => {
    ...
    const promise = generateImage({ cfg: {} as CIVITASConfig, prompt: "draw a cat" });
  ...
  it("does not crash on prototype-like provider ids in auth hints", async () => {
    ...
    await expect(
      generateImage({ cfg: {} as CIVITASConfig, prompt: "draw a cat" }),
