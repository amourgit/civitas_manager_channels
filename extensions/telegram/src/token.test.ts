...
describe("resolveTelegramToken", () => {
  ...
  function createTempDir(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-telegram-token-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  function createUnknownAccountConfig(): CIVITASConfig {
    return {
      channels: {
        telegram: {
  ...
  function expectNoTokenForUnknownAccount(cfg: CIVITASConfig) {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked tokenFile paths", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile: tokenLink } } } as CIVITASConfig;
  ...
  // Regression: https://github.com/civitas/civitas/issues/53876
...
describe("telegram update offset store", () => {
  it("persists and reloads the last update id", async () => {
    await withStateDirEnv("civitas-telegram-", async () => {
