import type { TelegramNetworkConfig } from "civitas/plugin-sdk/config-runtime";
import { afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";

vi.mock("civitas/plugin-sdk/runtime-env", () => ({
  isTruthyEnvValue: (value: string | undefined) =>
    typeof value === "string" && /^(1|true|yes|on)$/i.test(value.trim()),
  isWSL2Sync: vi.fn(() => false),
}));

let isWSL2Sync: typeof import("civitas/plugin-sdk/runtime-env").isWSL2Sync;
...
async function loadModule() {
  ({ isWSL2Sync } = await import("civitas/plugin-sdk/runtime-env"));
  ({
  ...
  it.each([
    {
      ...
      env: {
        CIVITAS_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1",
        CIVITAS_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1",
      },
      expected: {
        value: true,
        source: "env:CIVITAS_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "uses env disable when set",
      env: { CIVITAS_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1" },
      expected: {
        value: false,
        source: "env:CIVITAS_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "prefers env enable over config",
      env: { CIVITAS_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1" },
  ...
  it.each([
    {
      ...
      env: { CIVITAS_TELEGRAM_DNS_RESULT_ORDER: "verbatim" },
      nodeMajor: 22,
      expected: {
        value: "verbatim",
        source: "env:CIVITAS_TELEGRAM_DNS_RESULT_ORDER",
      },
    },
    {
      name: "normalizes trimmed env values",
      env: { CIVITAS_TELEGRAM_DNS_RESULT_ORDER: "  IPV4FIRST  " },
      nodeMajor: 20,
      expected: {
        value: "ipv4first",
        source: "env:CIVITAS_TELEGRAM_DNS_RESULT_ORDER",
    ...
    },
    {
      ...
      env: { CIVITAS_TELEGRAM_DNS_RESULT_ORDER: "bogus" },
      network: { dnsResultOrder: "ipv4first" },
      nodeMajor: 20,
      expected: { value: "ipv4first", source: "config" },
    },
