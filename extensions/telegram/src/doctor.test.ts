...
vi.mock("civitas/plugin-sdk/runtime", async () => {
  const actual = await vi.importActual<typeof import("civitas/plugin-sdk/runtime")>(
    "civitas/plugin-sdk/runtime",
  );
  return {
    ...actual,
...
describe("telegram doctor", () => {
  ...
  it("formats username repair warnings", () => {
    ...
    expect(warnings[1]).toContain("civitas doctor --fix");
  });
});
