...
describe("telegramSetupWizard.finalize", () => {
  it("shows global config commands for the default account", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining('civitas config set channels.telegram.dmPolicy "allowlist"'),
      "Telegram DM access warning",
    );
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining(`civitas config set channels.telegram.allowFrom '["YOUR_USER_ID"]'`),
      "Telegram DM access warning",
    );
  });
  ...
  it("shows account-scoped config commands for named accounts", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining(
        'civitas config set channels.telegram.accounts.alerts.dmPolicy "allowlist"',
      ),
      "Telegram DM access warning",
    );
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining(
        `civitas config set channels.telegram.accounts.alerts.allowFrom '["YOUR_USER_ID"]'`,
      ),
      "Telegram DM access warning",
    );
  });
