...
type LoadConfigFn = typeof import("civitas/plugin-sdk/config-runtime").loadConfig;
type LoadSessionStoreFn = typeof import("civitas/plugin-sdk/config-runtime").loadSessionStore;
type ResolveStorePathFn = typeof import("civitas/plugin-sdk/config-runtime").resolveStorePath;
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("civitas/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/civitas-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("civitas/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("civitas/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
