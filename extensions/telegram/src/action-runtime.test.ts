...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): CIVITASConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as CIVITASConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): CIVITASConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as CIVITASConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: CIVITASConfig, accountId = "media") {
    await handleTelegramAction(
