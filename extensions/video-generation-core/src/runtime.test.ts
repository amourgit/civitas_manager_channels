...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    getVideoGenerationProvider: vi.fn<
      (providerId: string, config?: CIVITASConfig) => VideoGenerationProvider | undefined
    >(() => undefined),
    isFailoverError: vi.fn<(err: unknown) => boolean>(() => false),
    listVideoGenerationProviders: vi.fn<(config?: CIVITASConfig) => VideoGenerationProvider[]>(
      () => [],
    ),
...
describe("video-generation runtime", () => {
  ...
  it("lists runtime video-generation providers through the owner runtime", () => {
    ...
    expect(listRuntimeVideoGenerationProviders({ config: {} as CIVITASConfig })).toEqual(
      providers,
    );
    expect(mocks.listVideoGenerationProviders).toHaveBeenCalledWith({} as CIVITASConfig);
  });
  ...
  it("explains native video-generation config and provider auth when no model is configured", async () => {
    ...
    const promise = generateVideo({ cfg: {} as CIVITASConfig, prompt: "animate a cat" });
