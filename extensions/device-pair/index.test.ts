...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredCIVITASTmpDir: vi.fn(() => path.join(os.tmpdir(), "civitas-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredCIVITASTmpDir: pluginApiMocks.resolvePreferredCIVITASTmpDir,
...
function createApi(params?: {
  runtime?: CIVITASPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as CIVITASPluginApi["runtime"],
...
function createChannelRuntime(
  ...
  } as unknown as CIVITASPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredCIVITASTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredCIVITASTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the CIVITAS iOS app:");
    expect(text).toContain("![CIVITAS pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the CIVITAS iOS app:");
