...
async function writeQrPngTempFile(data: string): Promise<string> {
  const pngBase64 = await renderQrPngBase64(data);
  const tmpRoot = resolvePreferredCIVITASTmpDir();
...
function resolveAuthLabel(cfg: CIVITASPluginApi["config"]): ResolveAuthLabelResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    pickFirstDefined([process.env.CIVITAS_GATEWAY_TOKEN, cfg.gateway?.auth?.token]) ?? undefined;
  const password =
    pickFirstDefined([process.env.CIVITAS_GATEWAY_PASSWORD, cfg.gateway?.auth?.password]) ??
    undefined;
...
async function resolveGatewayUrl(api: CIVITASPluginApi): Promise<ResolveUrlResult> {
...
export default definePluginEntry({
  id: "device-pair",
  name: "Device Pair",
  description: "QR/bootstrap pairing helpers for CIVITAS devices",
      ...
      handler: async (ctx) => {
        ...
        if (action === "qr") {
          ...
          if (target && canSendQrPngToChannel(channel)) {
            ...
            try {
              ...
              const sent = await sendQrPngToSupportedChannel({
                ...
                ctx,
                target,
                caption: ["Scan this QR code with the CIVITAS iOS app:", "", ...infoLines].join(
                  "\n",
                ),
                qrFilePath,
              });
          ...
          if (channel === "webchat") {
            ...
            return {
              text: [
                "Scan this QR code with the CIVITAS iOS app:",
                "",
