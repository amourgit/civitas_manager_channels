import { MAX_IMAGE_BYTES } from "civitas/plugin-sdk/media-runtime";
...
describe("tlon monitor media", () => {
  async function loadMediaModule() {
    const mediaRuntime = await import("civitas/plugin-sdk/media-runtime");
  ...
  it("stores fetched media through the shared inbound media store with the image cap", async () => {
    ...
    saveMediaBuffer.mockResolvedValue({
      id: "photo---uuid.png",
      path: "/tmp/civitas/media/inbound/photo---uuid.png",
    ...
    expect(result).toEqual({
      localPath: "/tmp/civitas/media/inbound/photo---uuid.png",
      contentType: "image/png",
