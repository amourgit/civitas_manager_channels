...
type WebSearchConfig = NonNullable<CIVITASConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<CIVITASConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: CIVITASConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: CIVITASConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: CIVITASConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: CIVITASConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: CIVITASConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: CIVITASConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: CIVITASConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: CIVITASConfig, override?: number): number {
