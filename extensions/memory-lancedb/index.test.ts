...
const TEST_RUNTIME_MANIFEST = {
  name: "civitas-memory-lancedb-runtime",
...
function createRuntimeLoader(
  ...
  return createLanceDbRuntimeLoader({
    ...
    resolveStateDir: () => "/tmp/civitas-state",
...
describe("memory plugin e2e", () => {
  const { getDbPath } = installTmpDirHarness({ prefix: "civitas-memory-test-" });
  ...
  test("passes configured dimensions to OpenAI embeddings API", async () => {
    ...
    vi.resetModules();
    vi.doMock("civitas/plugin-sdk/runtime-env", () => ({
    ...
    try {
      ...
      vi.doUnmock("civitas/plugin-sdk/runtime-env");
      vi.doUnmock("openai");
...
describe("lancedb runtime loader", () => {
  test("uses the bundled module when it is already available", async () => {
    ...
    const installRuntime = vi.fn(async () => "/tmp/civitas-state/plugin-runtimes/lancedb.js");
  ...
  test("reuses an existing user runtime install before attempting a reinstall", async () => {
    ...
    const resolveRuntimeEntry = vi.fn(
      () => "/tmp/civitas-state/plugin-runtimes/memory-lancedb/runtime-entry.js",
    );
    const installRuntime = vi.fn(
      async () => "/tmp/civitas-state/plugin-runtimes/memory-lancedb/runtime-entry.js",
    );
    const loader = createRuntimeLoader({
      importResolved,
      resolveRuntimeEntry,
    ...
    expect(resolveRuntimeEntry).toHaveBeenCalledWith(
      expect.objectContaining({
        runtimeDir: "/tmp/civitas-state/plugin-runtimes/memory-lancedb/lancedb",
      }),
    );
    expect(installRuntime).not.toHaveBeenCalled();
  });
  ...
  test("installs LanceDB into user state when the bundled runtime is unavailable", async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "installing runtime deps under /tmp/civitas-state/plugin-runtimes/memory-lancedb/lancedb",
      ),
    );
  });
  ...
  test("fails fast in nix mode instead of attempting auto-install", async () => {
    ...
    const loader = createRuntimeLoader({
      env: { CIVITAS_NIX_MODE: "1" } as NodeJS.ProcessEnv,
      installRuntime,
    });
  ...
  test("clears the cached failure so later calls can retry the install", async () => {
    ...
    const installRuntime = vi
      ...
      .mockResolvedValueOnce(
        "/tmp/civitas-state/plugin-runtimes/memory-lancedb/lancedb/node_modules/@lancedb/lancedb/index.js",
      );
    const importResolved = vi.fn(async () => runtimeModule);
    const loader = createRuntimeLoader({
      installRuntime,
