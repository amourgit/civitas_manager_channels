...
// Prefix for CIVITAS-generated action IDs to scope our handler
const CIVITAS_ACTION_PREFIX = "civitas:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${CIVITAS_ACTION_PREFIX}`);

  // Handle CIVITAS modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
