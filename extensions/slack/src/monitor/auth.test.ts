...
describe("resolveSlackEffectiveAllowFrom", () => {
  const prevTtl = process.env.CIVITAS_SLACK_PAIRING_ALLOWFROM_CACHE_TTL_MS;
  ...
  beforeEach(() => {
    ...
    if (prevTtl === undefined) {
      delete process.env.CIVITAS_SLACK_PAIRING_ALLOWFROM_CACHE_TTL_MS;
    } else {
      process.env.CIVITAS_SLACK_PAIRING_ALLOWFROM_CACHE_TTL_MS = prevTtl;
    }
  });
  ...
  it("refreshes pairing-store allowFrom when cache TTL is zero", async () => {
    process.env.CIVITAS_SLACK_PAIRING_ALLOWFROM_CACHE_TTL_MS = "0";
    readStoreAllowFromForDmPolicyMock.mockResolvedValue(["u2"]);
    const ctx = makeSlackCtx(["u1"]);
