import type { SlackSlashCommandConfig } from "civitas/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "civitas");
  const name = normalizedName || "civitas";
  return {
