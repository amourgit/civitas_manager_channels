...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "civitas_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "civitas_cmdarg", "arg-menu options");
  });
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(testHarness.actions.has("civitas_cmdarg")).toBe(true);
    expect(testHarness.options.has("civitas_cmdarg")).toBe(true);
  ...
  it("falls back to static menus when app.options() throws during registration", async () => {
    ...
    expect(actions.has("civitas_cmdarg")).toBe(true);
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("civitas_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("civitas_cmdarg");
    expect(blockId).toContain("civitas_cmdarg_ext:");
    const token = (blockId ?? "").slice("civitas_cmdarg_ext:".length);
