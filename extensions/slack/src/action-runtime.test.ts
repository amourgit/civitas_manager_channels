...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): CIVITASConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as CIVITASConfig;
  ...
  async function resolveReadToken(cfg: CIVITASConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: CIVITASConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
