...
import { reduceInteractiveReply } from "civitas/plugin-sdk/interactive-runtime";
import type { InteractiveReply } from "civitas/plugin-sdk/interactive-runtime";
import { truncateSlackText } from "./truncate.js";

export const SLACK_REPLY_BUTTON_ACTION_ID = "civitas:reply_button";
export const SLACK_REPLY_SELECT_ACTION_ID = "civitas:reply_select";
...
export function buildSlackInteractiveBlocks(interactive?: InteractiveReply): SlackBlock[] {
  ...
  return reduceInteractiveReply(interactive, initialState, (state, block) => {
    ...
    if (block.type === "buttons") {
      ...
      state.blocks.push({
        type: "actions",
        block_id: `civitas_reply_buttons_${++state.buttonIndex}`,
    ...
    state.blocks.push({
      type: "actions",
      block_id: `civitas_reply_select_${++state.selectIndex}`,
      elements: [
        {
          type: "static_select",
