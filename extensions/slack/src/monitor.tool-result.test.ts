...
let resetInboundDedupe: typeof import("civitas/plugin-sdk/reply-runtime").resetInboundDedupe;
...
beforeAll(async () => {
  ({ resetInboundDedupe } = await import("civitas/plugin-sdk/reply-runtime"));
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\bcivitas\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("civitas: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("civitas: hello <@U2>");
  });
