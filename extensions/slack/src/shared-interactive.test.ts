...
describe("buildSlackInteractiveBlocks", () => {
  it("renders shared interactive blocks in authored order", () => {
    expect(
      ...
      expect.objectContaining({
        type: "actions",
        block_id: "civitas_reply_select_1",
      }),
      expect.objectContaining({
        type: "section",
      ...
      expect.objectContaining({
        type: "actions",
        block_id: "civitas_reply_buttons_1",
      }),
    ]);
  });
  ...
  it("preserves original callback payloads for round-tripping", () => {
    ...
    expect(buttonBlock.elements?.[0]?.action_id).toBe("civitas:reply_button:1:1");
    expect(buttonBlock.elements?.[0]?.value).toBe("pluginbind:approval-123:o");
    expect(selectBlock.elements?.[0]?.action_id).toBe("civitas:reply_select:1");
