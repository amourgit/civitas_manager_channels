...
- Extension production code should import from `CIVITAS/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `CIVITAS.plugin.json` and the package
  `CIVITAS` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `CIVITAS/plugin-sdk/*`.
