...
vi.mock("civitas/plugin-sdk/ssrf-runtime", () => ({
...
describe("ollama web search provider", () => {
  ...
  it("maps generic search args into the Ollama experimental search endpoint", async () => {
    ...
    const result = await tool.execute({ query: "civitas docs", count: 3 });
    ...
    expect(result).toMatchObject({
      ...
      provider: "ollama",
      count: 1,
      results: [{ url: "https://civitas.ai/docs" }],
    });
  ...
  it("surfaces Ollama signin guidance for 401 responses", async () => {
    ...
    await expect(runOllamaWebSearch({ query: "latest civitas release" })).rejects.toThrow(
      "ollama signin",
    );
  });
