...
RUN echo "civitas-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && OPENCLAW_OPENSHELL_E2E)(
    ...
    async () => {
      ...
      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-openshell-e2e-"));
      ...
      const gatewayName = `civitas-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `civitas-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/civitas-sandboxes"),
      ...
      try {
        ...
        expect(stdout).toContain("civitas-openshell-e2e");
