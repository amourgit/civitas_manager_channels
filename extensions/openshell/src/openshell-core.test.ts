...
describe("openshell backend manager", () => {
  ...
  it("checks runtime status with config override from CIVITAS config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "civitas-session-1234",
        backendId: "openshell",
        runtimeLabel: "civitas-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: expect.objectContaining({
        sandboxName: "civitas-session-1234",
        config: expect.objectContaining({
          from: "custom-source",
        }),
      }),
      args: ["sandbox", "get", "civitas-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "civitas-session-5678",
        backendId: "openshell",
        runtimeLabel: "civitas-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "civitas",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: expect.objectContaining({
        sandboxName: "civitas-session-5678",
        config: expect.objectContaining({
          command: "/usr/local/bin/openshell",
          gateway: "lab",
        }),
      }),
      args: ["sandbox", "delete", "civitas-session-5678"],
    });
  });
});
...
describe("openshell fs bridges", () => {
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("civitas-openshell-fs-");
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("civitas-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("civitas-openshell-agent-");
  ...
  it("writes, reads, renames, and removes files without local host paths", async () => {
    const workspaceDir = await makeTempDir("civitas-openshell-remote-local-");
    const remoteWorkspaceDir = await makeTempDir("civitas-openshell-remote-workspace-");
    const remoteAgentDir = await makeTempDir("civitas-openshell-remote-agent-");
