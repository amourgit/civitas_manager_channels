...
export async function startGateway(ctx: GatewayContext): Promise<void> {
  ...
  } else if (isGlobalTTSAvailable(cfg as CIVITASConfig)) {
    const globalProvider = (cfg as CIVITASConfig).messages?.tts?.provider ?? "auto";
    log?.info(
    ...
    try {
      ...
      const handleMessage = async (event: {
        ...
        const hasTTS =
          ...
          isGlobalTTSAvailable(cfg as CIVITASConfig);
