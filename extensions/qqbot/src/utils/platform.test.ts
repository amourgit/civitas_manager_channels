...
describe("qqbot local media path remapping", () => {
  ...
  it("remaps missing workspace media paths to the real media directory", () => {
    ...
    const civitasDir = path.join(actualHome, ".civitas");
    fs.mkdirSync(civitasDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(civitasDir, "qqbot-platform-test-"));
