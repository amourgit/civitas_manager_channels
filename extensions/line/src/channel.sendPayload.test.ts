...
import type { CIVITASConfig, PluginRuntime } from "../api.js";
...
function createRuntime(): { runtime: PluginRuntime; mocks: LineRuntimeMocks } {
  ...
  const resolveLineAccount = vi.fn(
    ({ cfg, accountId }: { cfg: CIVITASConfig; accountId?: string }) => {
...
describe("linePlugin outbound.sendPayload", () => {
  it("preserves resolved accountId when pairing notifications push directly", async () => {
    ...
    expect(mocks.pushMessageLine).toHaveBeenCalledWith(
      "line:user:1",
      "CIVITAS: your access has been approved.",
      {
        accountId: "primary",
        channelAccessToken: "token-primary",
      },
  ...
  it("sends flex message without dropping text", async () => {
    const { runtime, mocks } = createRuntime();
    setLineRuntime(runtime);
    const cfg = { channels: { line: {} } } as CIVITASConfig;
  ...
  it("uses configured text chunk limit for payloads", async () => {
    const { runtime, mocks } = createRuntime();
    setLineRuntime(runtime);
    const cfg = { channels: { line: { textChunkLimit: 123 } } } as CIVITASConfig;
