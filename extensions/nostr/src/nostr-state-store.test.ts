...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.CIVITAS_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-nostr-"));
  process.env.CIVITAS_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.CIVITAS_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".civitas");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.CIVITAS_STATE_DIR = previous;
    }
