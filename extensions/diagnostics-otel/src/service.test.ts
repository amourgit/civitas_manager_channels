...
import type { CIVITASPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/civitas-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): CIVITASPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("civitas.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("civitas.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("civitas.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("civitas.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("civitas.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("civitas.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("civitas.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("civitas.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("civitas.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("civitas.webhook.processed");
    expect(spanNames).toContain("civitas.message.processed");
    expect(spanNames).toContain("civitas.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["civitas.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("civitas.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "civitas.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["civitas.reason"]).toBe("string");
    expect(String(attrs?.["civitas.reason"])).not.toContain(
