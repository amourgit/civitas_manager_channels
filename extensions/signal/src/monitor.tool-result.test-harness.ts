import type { MockFn } from "civitas/plugin-sdk/testing";
...
vi.mock("civitas/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("civitas/plugin-sdk/config-runtime")>(
    "civitas/plugin-sdk/config-runtime",
  );
  return {
    ...actual,
    loadConfig: () => config,
    resolveStorePath: vi.fn(() => "/tmp/civitas-sessions.json"),
...
vi.mock("civitas/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("civitas/plugin-sdk/reply-runtime")>(
    "civitas/plugin-sdk/reply-runtime",
  );
  return {
    ...actual,
...
vi.mock("civitas/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("civitas/plugin-sdk/conversation-runtime")>(
    "civitas/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("civitas/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("civitas/plugin-sdk/security-runtime")>(
    "civitas/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("civitas/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("civitas/plugin-sdk/infra-runtime")>(
    "civitas/plugin-sdk/infra-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("civitas/plugin-sdk/reply-runtime"),
      import("civitas/plugin-sdk/infra-runtime"),
    ]);
    resetInboundDedupe();
    config = {
