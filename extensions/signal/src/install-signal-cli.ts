...
import { runPluginCommandWithTimeout } from "civitas/plugin-sdk/run-command";
import type { RuntimeEnv } from "civitas/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "civitas/plugin-sdk/setup-tools";
...
async function installSignalCliFromRelease(runtime: RuntimeEnv): Promise<SignalInstallResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-signal-"));
