...
vi.mock("civitas/plugin-sdk/fetch-runtime", () => ({
  resolveFetch: (...args: unknown[]) => resolveFetchMock(...args),
}));

vi.mock("civitas/plugin-sdk/core", async () => {
  const actual = await vi.importActual<typeof import("civitas/plugin-sdk/core")>(
    "civitas/plugin-sdk/core",
  );
  return {
    ...actual,
...
vi.mock("civitas/plugin-sdk/text-runtime", () => ({
