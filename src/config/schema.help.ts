...
export const FIELD_HELP: Record<string, string> = {
  meta: "Metadata fields automatically maintained by CIVITAS to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
  "meta.lastTouchedVersion": "Auto-set when CIVITAS writes the config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for CIVITAS. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  wizard:
  ...
  "wizard.lastRunVersion":
    "CIVITAS version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version setup changes.",
  ...
  update:
    "Update-channel and startup-check behavior for keeping CIVITAS runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  ...
  "agents.list[].runtime":
    "Optional runtime descriptor for this agent. Use embedded for default CIVITAS execution or acp for external ACP harness defaults.",
  "agents.list[].runtime.type":
    'Runtime type for this agent: "embedded" (default CIVITAS runtime) or "acp" (ACP harness defaults).',
  ...
  "agents.list[].runtime.acp.agent":
    "Optional ACP harness agent id to use for this CIVITAS agent (for example codex, claude, cursor, gemini, civitas).",
  ...
  "browser.profiles.*.driver":
    'Per-profile browser driver mode. Use "civitas" (or legacy "CIVITAS Channeld") for CDP-based profiles, or use "existing-session" for host-local Chrome DevTools MCP attachment.',
  ...
  "discovery.wideArea.domain":
    "Optional unicast DNS-SD domain for wide-area discovery, such as civitas.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
  "discovery.mdns":
  ...
  "agents.defaults.sandbox.browser.network":
    "Docker network for sandbox browser containers (default: civitas-sandbox-browser). Avoid bridge if you need stricter isolation.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /civitas).",
  ...
  "gateway.nodes.allowCommands":
    "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `civitas security audit`.",
  ...
  "nodeHost.browserProxy.allowProfiles":
    "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, CIVITAS enforces least-privilege profile access and blocks persistent profile create/delete through the proxy.",
  media:
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $CIVITAS_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "models.providers.*.injectNumCtxForOpenAICompat":
    "Controls whether CIVITAS injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
  ...
  "agents.defaults.memorySearch.store.path":
    "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.civitas/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
  ...
  "memory.backend":
    'Selects the global memory engine: "builtin" uses CIVITAS memory internals, while "qmd" uses the QMD sidecar pipeline. Keep "builtin" unless you intentionally operate QMD.',
  "memory.citations":
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `civitas plugins update` to locate install sources).",
  ...
  "agents.defaults.embeddedPi":
    "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in CIVITAS sessions.",
  ...
  "commands.mcp":
    "Allow /mcp chat command to manage CIVITAS MCP server config under mcp.servers (default: false).",
  "commands.plugins":
  ...
  mcp: "Global MCP server definitions managed by CIVITAS. Embedded Pi and other runtime adapters can consume these servers without storing them inside Pi-owned project settings.",
  "mcp.servers":
    "Named MCP server definitions. CIVITAS stores them in its own config and runtime adapters decide which transports are supported at execution time.",
  session:
  ...
  "session.parentForkMaxTokens":
    "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, CIVITAS starts a fresh thread session instead of forking; set 0 to disable this protection.",
  "session.mainKey":
  ...
  hooks:
    "Inbound webhook automation surface for mapping external events into wake or agent actions in CIVITAS. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
  "hooks.enabled":
