...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ CIVITAS_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-civitas",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-civitas#general");
  });
  ...
  it("derives session transcripts dir from CIVITAS_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { CIVITAS_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when CIVITAS_STATE_DIR differs from stored paths", () => {
  ...
  it("resolveSessionFilePathOptions keeps explicit agentId alongside absolute store path", () => {
    const storePath = "/tmp/civitas/agents/main/sessions/sessions.json";
