...
function collectConfigEnvVarsByTarget(cfg?: CIVITASConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: CIVITASConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: CIVITASConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: CIVITASConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.CIVITAS_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
