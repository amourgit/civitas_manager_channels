...
import { resetConfigRuntimeState, type CIVITASConfig } from "./config.js";

export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  resetConfigRuntimeState();
  try {
    return await withTempHomeBase(fn, { prefix: "civitas-config-" });
  } finally {
    resetConfigRuntimeState();
  }
}

export async function writeCIVITASConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.CIVITAS_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected CIVITAS_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeCIVITASConfig(home, config);
...
export function buildWebSearchProviderConfig(params: {
  provider: NonNullable<
    NonNullable<NonNullable<NonNullable<CIVITASConfig["tools"]>["web"]>["search"]>["provider"]
  >;
  enabled?: boolean;
