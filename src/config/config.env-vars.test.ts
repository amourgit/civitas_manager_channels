...
describe("config env vars", () => {
  it("applies env vars from env block when missing", async () => {
    await withEnvOverride({ OPENROUTER_API_KEY: undefined }, async () => {
      applyConfigEnvVars({ env: { vars: { OPENROUTER_API_KEY: "config-key" } } } as CIVITASConfig);
  ...
  it("applies env vars from env.vars when missing", async () => {
    await withEnvOverride({ GROQ_API_KEY: undefined }, async () => {
      applyConfigEnvVars({ env: { vars: { GROQ_API_KEY: "gsk-config" } } } as CIVITASConfig);
  ...
  it("blocks dangerous startup env vars from config env", async () => {
    await withEnvOverride(
      ...
      async () => {
        ...
        const entries = collectConfigRuntimeEnvVars(config as CIVITASConfig);
        ...
        applyConfigEnvVars(config as CIVITASConfig);
  ...
  it("loads ${VAR} substitutions from ~/.civitas/.env on repeated runtime loads", async () => {
    await withTempHome(async (_home) => {
      await withEnvOverride({ BRAVE_API_KEY: undefined }, async () => {
        const stateDir = process.env.CIVITAS_STATE_DIR?.trim();
        if (!stateDir) {
          throw new Error("Expected CIVITAS_STATE_DIR to be set by withTempHome");
        }
        ...
        const first = resolveConfigEnvVars(config, process.env) as CIVITASConfig;
        expect(first.tools?.web?.search?.apiKey).toBe("from-dotenv");

        delete process.env.BRAVE_API_KEY;
        loadDotEnv({ quiet: true });
        const second = resolveConfigEnvVars(config, process.env) as CIVITASConfig;
  ...
  it("respects CIVITAS_STATE_DIR when reading state-dir .env vars", async () => {
    await withTempHome(async (_home) => {
      const customStateDir = path.join(process.env.CIVITAS_STATE_DIR ?? "", "custom-state");
      await writeStateDirDotEnv("CUSTOM_KEY=from-override\n", {
        stateDir: customStateDir,
      });
      expect(
        readStateDirDotEnvVars({
          CIVITAS_STATE_DIR: customStateDir,
        }).CUSTOM_KEY,
      ).toBe("from-override");
    });
  });
