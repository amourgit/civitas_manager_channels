...
import type { ConfigFileSnapshot } from "./types.civitas.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/civitas/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/civitas/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/civitas/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/civitas/nickserv-password.txt");
