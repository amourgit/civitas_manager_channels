...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.CIVITAS_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveCIVITASPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    CIVITAS_STATE_DIR: path.join(os.tmpdir(), "civitas-config-doc-baseline-state"),
    CIVITAS_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
