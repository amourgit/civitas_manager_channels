...
async function expectNoNewerVersionWarning(touchedVersion: string) {
  await withTempHome(async (home) => {
    ...
    const configPath = path.join(configDir, "civitas.json");
    await fs.writeFile(
      configPath,
...
describe("config io paths", () => {
  it("uses ~/.civitas/civitas.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".civitas", 19001);
  ...
  it("defaults to ~/.civitas/civitas.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".civitas", "civitas.json"));
    });
  });
