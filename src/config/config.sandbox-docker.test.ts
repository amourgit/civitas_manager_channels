...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("civitas-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "civitas-sandbox-browser-global" },
      agentBrowser: { network: "civitas-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("civitas-sandbox-browser-agent");
  });
