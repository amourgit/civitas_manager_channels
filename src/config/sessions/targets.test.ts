...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): CIVITASConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: CIVITASConfig, env: NodeJS.ProcessEnv) =>
  ...
  it("resolves all configured agent stores", async () => {
    await withTempHome(async () => {
      const cfg: CIVITASConfig = {
        session: {
          store: "~/.civitas/agents/{agentId}/sessions/sessions.json",
        },
        agents: {
