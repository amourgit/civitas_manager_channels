...
// Keep integration tests deterministic: never read a real civitas.json.
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-pruning-integ-"));
  });
  ...
  it("does not archive external transcript paths when capping entries", async () => {
    ...
    const now = Date.now();
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-external-cap-"));
  ...
  it("never deletes transcripts outside the agent sessions directory during budget cleanup", async () => {
    ...
    const now = Date.now();
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-external-session-"));
    const externalTranscript = path.join(externalDir, "outside.jsonl");
