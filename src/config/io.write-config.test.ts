...
describe("config io write", () => {
  ...
  function createBlueBubblesManifestRecord(): PluginManifestRecord {
    return {
      ...
      source: "/virtual/plugins/bluebubbles/civitas.plugin.json",
      manifestPath: "/virtual/plugins/bluebubbles/civitas.plugin.json",
      channelCatalogMeta: {
        id: "bluebubbles",
        label: "BlueBubbles",
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-config-io-"));
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".civitas", "civitas.json");
  ...
  async function writeGatewayPatchAndReadLastAuditEntry(params: {
    ...
    const auditPath = path.join(params.home, ".civitas", "logs", "config-audit.jsonl");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withSuiteHome(async (home) => {
        const stateDir = path.join(home, ".civitas");
  ...
  it("keeps writes inside an CIVITAS_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "civitas.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "civitas.json"), "utf-8"),
      ) as {
  ...
  it('shows actionable guidance for dmPolicy="open" without wildcard allowFrom', async () => {
    await withSuiteHome(async (home) => {
      ...
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        'civitas config set channels.telegram.dmPolicy "pairing"',
      );
    });
  });
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    await withSuiteHome(async (home) => {
      ...
      expect(lines.length).toBeGreaterThan(0);
      await expect(
        fs.stat(path.join(home, ".civitas", "logs", "config-audit.jsonl")),
      ).resolves.toBeDefined();
      await expect(
        fs.stat(path.resolve("undefined", ".civitas", "logs", "config-audit.jsonl")),
      ).rejects.toThrow();
    });
  });
