...
export type WideAreaDiscoveryConfig = {
  enabled?: boolean;
  /** Optional unicast DNS-SD domain (e.g. "civitas.internal"). */
  domain?: string;
};
...
export type CanvasHostConfig = {
  enabled?: boolean;
  /** Directory to serve (default: ~/.civitas/workspace/canvas). */
  root?: string;
...
export type GatewayControlUiConfig = {
  ...
  enabled?: boolean;
  /** Optional base path prefix for the Control UI (e.g. "/civitas"). */
  basePath?: string;
 ...
 * Used when Channeldbot runs behind an identity-aware proxy (Pomerium, Caddy + OAuth, etc.)
...
export type GatewayReloadConfig = {
   ...
   * @see https://github.com/civitas/civitas/issues/47711
   */
