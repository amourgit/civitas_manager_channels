...
export function parseCIVITASVersion(raw: string | null | undefined): CIVITASVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeCIVITASVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseCIVITASVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameCIVITASStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseCIVITASVersion(a);
  const parsedB = parseCIVITASVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareCIVITASVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseCIVITASVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseCIVITASVersion(current);
  const parsedTouched = parseCIVITASVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameCIVITASStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareCIVITASVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: CIVITASVersion): number {
