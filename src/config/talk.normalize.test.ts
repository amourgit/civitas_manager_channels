...
async function withTempConfig(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-talk-"));
  const configPath = path.join(dir, "civitas.json");
...
describe("talk normalization", () => {
  it("maps legacy ElevenLabs fields into provider/providers", () => {
    const normalized = normalizeTalkSection({
      voiceId: "voice-123",
      voiceAliases: { Channeld: "EXAVITQu4vr4xnSDxMaL" }, // pragma: allowlist secret
      modelId: "eleven_v3",
    ...
    expect(normalized).toEqual({
      providers: {
        elevenlabs: {
          voiceId: "voice-123",
          voiceAliases: { Channeld: "EXAVITQu4vr4xnSDxMaL" },
          modelId: "eleven_v3",
          outputFormat: "pcm_44100",
          apiKey: "secret-key", // pragma: allowlist secret
        },
