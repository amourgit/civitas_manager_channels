...
const SAME_CHANNEL_SDK_GUARDS: GuardedSource[] = [
  {
    ...
    forbiddenPatterns: [/["']civitas\/plugin-sdk\/discord["']/, /plugin-sdk-internal\/discord/],
  },
  {
    path: bundledPluginFile("slack", "src/shared.ts"),
    forbiddenPatterns: [/["']civitas\/plugin-sdk\/slack["']/, /plugin-sdk-internal\/slack/],
  },
  {
    path: bundledPluginFile("telegram", "src/shared.ts"),
    forbiddenPatterns: [/["']civitas\/plugin-sdk\/telegram["']/, /plugin-sdk-internal\/telegram/],
  },
  {
    path: bundledPluginFile("telegram", "src/account-inspect.ts"),
    forbiddenPatterns: [/["']civitas\/plugin-sdk\/account-resolution["']/],
  },
  {
  ...
  },
  {
    ...
    forbiddenPatterns: [/["']civitas\/plugin-sdk\/imessage["']/, /plugin-sdk-internal\/imessage/],
  },
  {
    path: bundledPluginFile("whatsapp", "src/shared.ts"),
    forbiddenPatterns: [/["']civitas\/plugin-sdk\/whatsapp["']/, /plugin-sdk-internal\/whatsapp/],
  },
  {
    path: bundledPluginFile("signal", "src/shared.ts"),
    forbiddenPatterns: [/["']civitas\/plugin-sdk\/signal["']/, /plugin-sdk-internal\/signal/],
  },
  {
...
const CHANNEL_CONFIG_SCHEMA_GUARDS: GuardedSource[] = [
  {
    ...
    forbiddenPatterns: [/["']civitas\/plugin-sdk\/core["']/],
  },
];
...
function expectNoCrossPluginSdkFacadeImports(file: string, imports: string[]): void {
  ...
  for (const specifier of imports) {
    if (!specifier.startsWith("civitas/plugin-sdk/")) {
      continue;
    }
    const targetSubpath = specifier.slice("civitas/plugin-sdk/".length);
...
describe("channel import guardrails", () => {
  ...
  it("keeps bundled extension source files off root and compat plugin-sdk imports", () => {
    for (const file of collectExtensionSourceFiles()) {
      ...
      expect(text, `${file} should not import civitas/plugin-sdk root`).not.toMatch(
        /["']civitas\/plugin-sdk["']/,
      );
      expect(text, `${file} should not import civitas/plugin-sdk/compat`).not.toMatch(
        /["']civitas\/plugin-sdk\/compat["']/,
      );
    }
  });
  ...
  it("keeps internalized extension helper surfaces behind local api barrels", () => {
    for (const extensionId of LOCAL_EXTENSION_API_BARREL_GUARDS) {
      for (const file of collectExtensionFiles(extensionId)) {
        ...
        ).not.toMatch(new RegExp(`["']civitas/plugin-sdk/${extensionId}(?:["'/])`, "u"));
      }
    }
  });
});
