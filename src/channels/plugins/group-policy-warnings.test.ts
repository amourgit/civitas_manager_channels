...
import type { CIVITASConfig } from "../../config/types.civitas.js";
import {
...
describe("group policy warning builders", () => {
  ...
  it("projects cfg-only warning collector inputs", () => {
    const collect = projectConfigWarningCollector<{ cfg: CIVITASConfig; accountId: string }>(
      ({ cfg }) => [cfg.channels ? "configured" : "none"],
    );

    expect(
      collect({
        cfg: { channels: { slack: {} } } as CIVITASConfig,
        accountId: "acct-1",
      }),
    ).toEqual(["configured"]);
  });
  ...
  it("projects account+cfg warning collector inputs", () => {
    const collect = projectAccountConfigWarningCollector<
      ...
      { account: { accountId: string }; cfg: CIVITASConfig }
    >(
      (cfg: CIVITASConfig) => cfg.channels ?? {},
