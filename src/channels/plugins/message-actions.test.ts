...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(listChannelMessageCapabilities({} as CIVITASConfig).toSorted()).toEqual([
      "buttons",
      "cards",
      "interactive",
    ]);
    expect(channelSupportsMessageCapability({} as CIVITASConfig, "interactive")).toBe(true);
    expect(channelSupportsMessageCapability({} as CIVITASConfig, "buttons")).toBe(true);
    expect(channelSupportsMessageCapability({} as CIVITASConfig, "cards")).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as CIVITASConfig, channel: "demo-buttons" },
        "interactive",
      ),
    ).toBe(true);
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as CIVITASConfig, channel: "demo-cards" },
        "interactive",
      ),
    ).toBe(false);
    expect(
    ...
    expect(channelSupportsMessageCapabilityForChannel({ cfg: {} as CIVITASConfig }, "cards")).toBe(
      false,
    );
  });
  ...
  it("uses unified message tool discovery for actions, capabilities, and schema", () => {
    ...
    expect(listChannelMessageActions({} as CIVITASConfig)).toEqual(["send", "broadcast", "react"]);
    expect(listChannelMessageCapabilities({} as CIVITASConfig)).toEqual(["interactive"]);
    expect(
      resolveChannelMessageToolSchemaProperties({
        cfg: {} as CIVITASConfig,
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(listChannelMessageActions({} as CIVITASConfig)).toEqual(["send", "broadcast"]);
    expect(listChannelMessageCapabilities({} as CIVITASConfig)).toEqual([]);
