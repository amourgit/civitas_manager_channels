...
import { resolveCIVITASPackageRootSync } from "../../infra/civitas-root.js";
import { resolveBundledPluginsDir } from "../../plugins/bundled-dir.js";
import { discoverCIVITASPlugins } from "../../plugins/discovery.js";
import { loadPluginManifest } from "../../plugins/manifest.js";
import type { CIVITASPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, CIVITASPackageManifest>>;
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = [
    resolveCIVITASPackageRootSync({ cwd: process.cwd() }),
    resolveCIVITASPackageRootSync({ moduleUrl: import.meta.url }),
...
function toChannelMeta(params: {
  channel: NonNullable<CIVITASPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: CIVITASPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  origin?: PluginOrigin;
  workspaceDir?: string;
  packageManifest?: CIVITASPackageManifest;
...
function loadBundledMetadataCatalogEntries(options: CatalogOptions): ChannelPluginCatalogEntry[] {
  ...
  for (const dirent of fs.readdirSync(bundledDir, { withFileTypes: true })) {
    ...
    const entry = buildCatalogEntry({
      ...
      packageManifest: packageJson.civitas,
    });
    if (entry) {
      entries.push(entry);
    }
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverCIVITASPlugins({
    workspaceDir: options.workspaceDir,
    env: options.env,
  });
