...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): CIVITASConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as CIVITASConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: CIVITASConfig) => string[];
  input: CIVITASConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: CIVITASConfig, expected: string) {
