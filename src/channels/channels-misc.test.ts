...
describe("WA_WEB_AUTH_DIR", () => {
  ...
  it("resolves lazily and caches across the legacy and channels/web entrypoints", async () => {
    const resolveWebChannelAuthDir = vi.fn(() => "/tmp/civitas-whatsapp-auth");
    ...
    expect(String(channelWeb.WA_WEB_AUTH_DIR)).toBe("/tmp/civitas-whatsapp-auth");
    expect(String(webEntry.WA_WEB_AUTH_DIR)).toBe("/tmp/civitas-whatsapp-auth");
