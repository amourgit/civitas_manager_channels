...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["civitas update", "Update a source checkout (git)"],
        ["civitas update --channel beta", "Switch to beta channel (git + npm)"],
        ["civitas update --channel dev", "Switch to dev channel (git + npm)"],
        ["civitas update --tag beta", "One-off update to a dist-tag or version"],
        ["civitas update --tag main", "One-off package install from GitHub main"],
        ["civitas update --dry-run", "Preview actions without changing anything"],
        ["civitas update --no-restart", "Update without restarting the service"],
        ["civitas update --json", "Output result as JSON"],
        ["civitas update --yes", "Non-interactive (accept downgrade prompts)"],
        ["civitas update wizard", "Interactive update wizard"],
        ["civitas --update", "Shorthand for civitas update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run civitas update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.civitas.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["civitas update status", "Show channel + version status."],
          ["civitas update status --json", "JSON output."],
          ["civitas update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.civitas.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
