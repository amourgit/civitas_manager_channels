...
import type { ConfigFileSnapshot, CIVITASConfig } from "../config/types.js";
import { createCliRuntimeCapture, mockRuntimeModule } from "./test-runtime-capture.js";

/**
 * Test for issue #6070:
 * `civitas config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: CIVITASConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...
    writeConfigFile: (cfg: CIVITASConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: CIVITASConfig, config: CIVITASConfig) {
...
function withRuntimeDefaults(resolved: CIVITASConfig): CIVITASConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
function makeInvalidSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/custom-civitas.json",
    exists: true,
    raw: "{}",
    parsed: {},
    sourceConfig: {},
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: CIVITASConfig = {
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__CIVITAS_REDACTED__");
    });
  });
  ...
  describe("config validate", () => {
    ...
    it("returns machine-readable JSON with --json for invalid config", async () => {
      ...
      expect(payload.path).toBe("/tmp/custom-civitas.json");
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: CIVITASConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
    ...
    it("shows --strict-json and keeps --json as a legacy alias in help", async () => {
      ...
      expect(helpText).toContain("civitas config set gateway.port 19001 --strict-json");
      expect(helpText).toContain(
        "civitas config set channels.discord.token --ref-provider default --ref-source",
      );
      expect(helpText).toContain("--ref-id DISCORD_BOT_TOKEN");
      expect(helpText).toContain(
        "civitas config set --batch-file ./config-set.batch.json --dry-run",
      );
    });
  });
  ...
  describe("config set builders and dry-run", () => {
    ...
    it("rejects --allow-exec without --dry-run", async () => {
      const nonexistentBatchPath = path.join(
        os.tmpdir(),
        `civitas-config-batch-nonexistent-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      await expect(
    ...
    it("supports batch-file mode", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `civitas-config-batch-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
    ...
    it("rejects malformed batch-file payloads", async () => {
      const pathname = path.join(
        os.tmpdir(),
        `civitas-config-batch-invalid-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      fs.writeFileSync(pathname, '{"path":"gateway.auth.mode","value":"token"}', "utf8");
      try {
  ...
  describe("config file", () => {
    it("prints the active config file path", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/tmp/civitas.json");
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
    });
    ...
    it("handles config file path with home directory", async () => {
      ...
      snapshot.path = "/home/user/.civitas/civitas.json";
      mockReadConfigFileSnapshot.mockResolvedValueOnce(snapshot);

      await runConfigCommand(["config", "file"]);

      expect(mockLog).toHaveBeenCalledWith("/home/user/.civitas/civitas.json");
    });
  });
});
