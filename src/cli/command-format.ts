...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+civitas\b|^civitas\b/;
const CONTAINER_FLAG_RE = /(?:^|\s)--container(?:\s|=|$)/;
const PROFILE_FLAG_RE = /(?:^|\s)--profile(?:\s|=|$)/;
const DEV_FLAG_RE = /(?:^|\s)--dev(?:\s|$)/;
const UPDATE_COMMAND_RE =
  /^(?:pnpm|npm|bunx|npx)\s+civitas\b.*(?:^|\s)update(?:\s|$)|^civitas\b.*(?:^|\s)update(?:\s|$)/;
...
export function formatCliCommand(
  ...
  const container = env.CIVITAS_CONTAINER_HINT?.trim();
  const profile = normalizeProfileName(env.CIVITAS_PROFILE);
