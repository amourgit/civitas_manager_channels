...
describe("parseCliProfileArgs", () => {
  it("leaves gateway --dev for subcommands", () => {
    ...
    expect(res.argv).toEqual(["node", "civitas", "gateway", "--dev", "--allow-unconfigured"]);
  });
  ...
  it("still accepts global --dev before subcommand", () => {
    const res = parseCliProfileArgs(["node", "civitas", "--dev", "gateway"]);
    if (!res.ok) {
      throw new Error(res.error);
    }
    expect(res.profile).toBe("dev");
    expect(res.argv).toEqual(["node", "civitas", "gateway"]);
  });

  it("parses --profile value and strips it", () => {
    const res = parseCliProfileArgs(["node", "civitas", "--profile", "work", "status"]);
    if (!res.ok) {
      throw new Error(res.error);
    }
    expect(res.profile).toBe("work");
    expect(res.argv).toEqual(["node", "civitas", "status"]);
  });

  it("parses interleaved --profile after the command token", () => {
    const res = parseCliProfileArgs(["node", "civitas", "status", "--profile", "work", "--deep"]);
    if (!res.ok) {
      throw new Error(res.error);
    }
    expect(res.profile).toBe("work");
    expect(res.argv).toEqual(["node", "civitas", "status", "--deep"]);
  });

  it("parses interleaved --dev after the command token", () => {
    const res = parseCliProfileArgs(["node", "civitas", "status", "--dev"]);
    if (!res.ok) {
      throw new Error(res.error);
    }
    expect(res.profile).toBe("dev");
  ...
  it("rejects missing profile value", () => {
    const res = parseCliProfileArgs(["node", "civitas", "--profile"]);
    expect(res.ok).toBe(false);
  });

  it.each([
    ["--dev first", ["node", "civitas", "--dev", "--profile", "work", "status"]],
    ["--profile first", ["node", "civitas", "--profile", "work", "--dev", "status"]],
    ["interleaved after command", ["node", "civitas", "status", "--profile", "work", "--dev"]],
