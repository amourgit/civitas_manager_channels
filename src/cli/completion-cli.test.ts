...
describe("completion-cli", () => {
  it("generates zsh functions for nested subcommands", () => {
    ...
    expect(script).toContain("_civitas_gateway()");
    expect(script).toContain("(status) _civitas_gateway_status ;;");
    expect(script).toContain("(restart) _civitas_gateway_restart ;;");
  ...
  it("defers zsh registration until compinit is available", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-zsh-completion-"));
    try {
      const scriptPath = path.join(tempDir, "civitas.zsh");
      ...
      const result = spawnSync(
        ...
        [
          ...
          `
            ...
            [[ -z "\${_comps[civitas]-}" ]] || exit 10
            [[ "\${precmd_functions[(r)_civitas_register_completion]}" = "_civitas_register_completion" ]] || exit 11
            autoload -Uz compinit
            compinit -C
            _civitas_register_completion
            [[ -z "\${precmd_functions[(r)_civitas_register_completion]}" ]] || exit 12
            [[ "\${_comps[civitas]-}" = "_civitas_root_completion" ]]
          `,
        ],
        {
          encoding: "utf8",
  ...
  it("generates PowerShell command paths without the executable prefix", () => {
    ...
    expect(script).not.toContain("if ($commandPath -eq 'civitas gateway') {");
  ...
  it("generates fish completions for root and nested command contexts", () => {
    ...
    expect(script).toContain(
      'complete -c civitas -n "__fish_use_subcommand" -a "gateway" -d \'Gateway commands\'',
    );
    expect(script).toContain(
      'complete -c civitas -n "__fish_seen_subcommand_from gateway" -a "status" -d \'Show gateway status\'',
    );
    expect(script).toContain(
      "complete -c civitas -n \"__fish_seen_subcommand_from gateway\" -l force -d 'Force the action'",
    );
  });
});
