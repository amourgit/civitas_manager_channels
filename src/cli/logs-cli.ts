...
function createLogWriters() {
  const writer = createSafeStreamWriter({
    ...
    onBrokenPipe: (err, stream) => {
      ...
      const message = `civitas logs: output ${target} closed (${code}). Stopping tail.`;
      try {
...
async function emitGatewayError(
  ...
  const hint = `Hint: run \`${formatCliCommand("civitas doctor")}\`.`;
...
export function registerLogsCli(program: Command) {
  const logs = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/logs", "docs.civitas.ai/cli/logs")}\n`,
    );
