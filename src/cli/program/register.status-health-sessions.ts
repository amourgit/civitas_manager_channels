...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["civitas status", "Show channel health + session summary."],
          ["civitas status --all", "Full diagnosis (read-only)."],
          ["civitas status --json", "Machine-readable output."],
          ["civitas status --usage", "Show model provider usage/quota snapshots."],
          [
            "civitas status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["civitas status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.civitas.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.civitas.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["civitas sessions", "List all sessions."],
          ["civitas sessions --agent work", "List sessions for one agent."],
          ["civitas sessions --all-agents", "Aggregate sessions across agents."],
          ["civitas sessions --active 120", "Only last 2 hours."],
          ["civitas sessions --json", "Machine-readable output."],
          ["civitas sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.civitas.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["civitas sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "civitas sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["civitas sessions cleanup --enforce", "Apply maintenance now."],
          ["civitas sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["civitas sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "civitas sessions cleanup --enforce --store ./tmp/sessions.json",
