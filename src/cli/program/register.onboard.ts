...
export function registerOnboardCommand(program: Command) {
  const command = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/onboard", "docs.civitas.ai/cli/onboard")}\n`,
    )
    .option("--workspace <dir>", "Agent workspace directory (default: ~/.civitas/workspace)")
    .option(
      "--reset",
  ...
  command
    ...
    .option(
      ...
      "Gateway token SecretRef env var name (token auth; e.g. CIVITAS_GATEWAY_TOKEN)",
    )
