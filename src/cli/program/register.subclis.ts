...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.CIVITAS_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.CIVITAS_DISABLE_LAZY_SUBCOMMANDS);
};

export const loadValidatedConfigForPluginRegistration =
  async (): Promise<CIVITASConfig | null> => {
...
const entries: SubCliEntry[] = [
  ...
  },
  {
    ...
    hasSubcommands: true,
    register: async (program) => {
      const mod = await import("../CIVITAS Channelbot-cli.js");
      mod.registerChannelbotCli(program);
    },
  },
  {
    name: "pairing",
