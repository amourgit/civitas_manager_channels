...
${formatHelpExamples([
  ['civitas agent --to +15555550123 --message "status update"', "Start a new session."],
  ['civitas agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'civitas agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'civitas agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['civitas agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'civitas agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.civitas.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.civitas.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "civitas agents set-identity --identity-file ~/.civitas/workspace/IDENTITY.md --agent main",
