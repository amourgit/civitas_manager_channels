...
const EXAMPLES = [
  ["civitas models --help", "Show detailed help for the models command."],
  [
    "civitas channels login --verbose",
    "Link personal WhatsApp Web and show QR + connection logs.",
  ],
  [
    'civitas message send --target +15555550123 --message "Hi" --json',
    "Send via your web session and print JSON result.",
  ],
  ["civitas gateway --port 18789", "Run the WebSocket Gateway locally."],
  ["civitas --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
  ["civitas gateway --force", "Kill anything bound to the default gateway port, then start it."],
  ["civitas gateway ...", "Gateway control via WebSocket."],
  [
    'civitas agent --to +15555550123 --message "Run summary" --deliver',
    "Talk directly to the agent using the Gateway; optionally send the WhatsApp reply.",
  ],
  [
    'civitas message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.civitas-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.civitas.ai/cli");
