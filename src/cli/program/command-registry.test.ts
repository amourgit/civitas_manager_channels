...
describe("command-registry", () => {
  ...
  it("registers doctor placeholder for doctor primary command", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "civitas", "doctor"]);
  ...
  it("does not narrow to the primary command when help is requested", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "civitas", "doctor", "--help"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "civitas", "vitest"]);
    program.exitOverride();
    await withProcessArgv(["node", "civitas", "status"], async () => {
      await program.parseAsync(["node", "civitas", "status"]);
    });
