...
describe("program routes", () => {
  ...
  it("matches status route and preloads plugins only for text output", () => {
    ...
    expect(shouldLoad(["node", "civitas", "status"])).toBe(true);
    expect(shouldLoad(["node", "civitas", "status", "--json"])).toBe(false);
  });
  ...
  it("matches health route and preloads plugins only for text output", () => {
    ...
    expect(typeof route?.loadPlugins).toBe("function");
    const shouldLoad = route?.loadPlugins as (argv: string[]) => boolean;
    expect(shouldLoad(["node", "civitas", "health"])).toBe(true);
    expect(shouldLoad(["node", "civitas", "health", "--json"])).toBe(false);
  });
  ...
  it("returns false for gateway status route when option values are missing", async () => {
    await expectRunFalse(["gateway", "status"], ["node", "civitas", "gateway", "status", "--url"]);
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "civitas", "gateway", "status", "--token"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "civitas", "gateway", "status", "--password"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "civitas", "gateway", "status", "--timeout"],
    );
  });
  ...
  it("returns false for gateway status route when probe-only flags are present", async () => {
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "civitas", "gateway", "status", "--ssh", "user@host"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "civitas", "gateway", "status", "--ssh-identity", "~/.ssh/id_test"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "civitas", "gateway", "status", "--ssh-auto"],
    );
  });
  ...
  it("passes --no-probe through to daemon status", async () => {
    const route = expectRoute(["gateway", "status"]);
    await expect(route?.run(["node", "civitas", "gateway", "status", "--no-probe"])).resolves.toBe(
      true,
    );
  ...
  it("returns false when status timeout flag value is missing", async () => {
    await expectRunFalse(["status"], ["node", "civitas", "status", "--timeout"]);
  });
  ...
  it("returns false for sessions route when --store value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "civitas", "sessions", "--store"]);
  });

  it("returns false for sessions route when --active value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "civitas", "sessions", "--active"]);
  });

  it("returns false for sessions route when --agent value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "civitas", "sessions", "--agent"]);
  });
  ...
  it("returns false for config get route when path argument is missing", async () => {
    await expectRunFalse(["config", "get"], ["node", "civitas", "config", "get", "--json"]);
  });

  it("returns false for config unset route when path argument is missing", async () => {
    await expectRunFalse(["config", "unset"], ["node", "civitas", "config", "unset"]);
  });
  ...
  it("passes config unset path correctly when root option values precede command", async () => {
    ...
    await expect(
      route?.run(["node", "civitas", "--profile", "work", "config", "unset", "update.channel"]),
    ).resolves.toBe(true);
  ...
  it("passes config unset path when root value options appear after subcommand", async () => {
    const route = expectRoute(["config", "unset"]);
    await expect(
      route?.run(["node", "civitas", "config", "unset", "--profile", "work", "update.channel"]),
    ).resolves.toBe(true);
    expect(runConfigUnsetMock).toHaveBeenCalledWith({ path: "update.channel" });
  });
  ...
  it("returns false for models list route when --provider value is missing", async () => {
    await expectRunFalse(["models", "list"], ["node", "civitas", "models", "list", "--provider"]);
  });

  it("returns false for models status route when probe flags are missing values", async () => {
    await expectRunFalse(
      ["models", "status"],
      ["node", "civitas", "models", "status", "--probe-provider"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "civitas", "models", "status", "--probe-timeout"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "civitas", "models", "status", "--probe-concurrency"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "civitas", "models", "status", "--probe-max-tokens"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "civitas", "models", "status", "--probe-provider", "openai", "--agent"],
    );
  });
  ...
  it("returns false for models status route when --probe-profile has no value", async () => {
    await expectRunFalse(
      ["models", "status"],
      ["node", "civitas", "models", "status", "--probe-profile"],
    );
  });
