...
function formatPluginLine(plugin: PluginRecord, verbose = false): string {
  ...
  const format = plugin.format ?? "civitas";
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.civitas.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action((id: string | undefined, opts: PluginInspectOptions) => {
      ...
      lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "civitas"}`);
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.civitas.ai/plugin");
      lines.push("");
