...
import { ensureCIVITASCliOnPath } from "../infra/path-env.js";
...
export function resolveMissingBrowserCommandMessage(config?: CIVITASConfig): string | null {
  const allow =
  ...
  if (allow.length > 0 && !allow.includes("browser")) {
    return (
      'The `civitas browser` command is unavailable because `plugins.allow` excludes "browser". ' +
      'Add "browser" to `plugins.allow` if you want the bundled browser CLI and tool.'
    );
  }
  if (config?.plugins?.entries?.browser?.enabled === false) {
    return (
      "The `civitas browser` command is unavailable because `plugins.entries.browser.enabled=false`. " +
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? process.env.CIVITAS_CONTAINER?.trim() ?? null;
