...
import type { ConfigFileSnapshot } from "../config/types.civitas.js";
import {
...
const hoisted = vi.hoisted(() => ({
  loadConfigMock: vi.fn<() => CIVITASConfig>(),
...
vi.mock("../commands/doctor/shared/channel-doctor.js", () => ({
  collectChannelDoctorStaleConfigMutations: (cfg: CIVITASConfig) =>
...
function makeSnapshot(overrides: Partial<ConfigFileSnapshot> = {}): ConfigFileSnapshot {
  return {
    ...
    resolved: { plugins: {} } as CIVITASConfig,
    valid: false,
    runtimeConfig: { plugins: {} } as ConfigFileSnapshot["runtimeConfig"],
    config: { plugins: {} } as CIVITASConfig,
    hash: "abc",
...
describe("loadConfigForInstall", () => {
  const matrixNpmRequest = {
    rawSpec: "@civitas/matrix",
    normalizedSpec: "@civitas/matrix",
  ...
  beforeEach(() => {
    ...
    collectChannelDoctorStaleConfigMutationsMock.mockImplementation(async (cfg: CIVITASConfig) => [
      {
        config: cfg,
        changes: [],
      },
  ...
  it("returns the config directly when loadConfig succeeds", async () => {
    const cfg = { plugins: { entries: { matrix: { enabled: true } } } } as CIVITASConfig;
  ...
  it("does not run stale Matrix cleanup on the happy path", async () => {
    const cfg = { plugins: {} } as CIVITASConfig;
    loadConfigMock.mockReturnValue(cfg);
  ...
  it("allows explicit repo-checkout bundled-plugin reinstall recovery", async () => {
    ...
    const snapshotCfg = { plugins: {} } as CIVITASConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        config: snapshotCfg,
  ...
  it("rejects non-Matrix install requests when config is invalid", async () => {
    ...
    ).rejects.toThrow("Config invalid; run `civitas doctor --fix` before installing plugins.");
    expect(readConfigFileSnapshotMock).not.toHaveBeenCalled();
  });
  ...
  it("throws when loadConfig fails with INVALID_CONFIG and snapshot parsed is empty", async () => {
    ...
    await expect(loadConfigForInstall(matrixNpmRequest)).rejects.toThrow(
      "Config file could not be parsed; run `civitas doctor` to repair it.",
    );
  });
