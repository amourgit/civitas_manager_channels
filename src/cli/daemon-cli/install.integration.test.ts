...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("civitas-daemon-install-int-");
    configPath = path.join(tempHome, "civitas.json");
    process.env.HOME = tempHome;
    process.env.CIVITAS_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.CIVITAS_GATEWAY_TOKEN).toBeUndefined();
  });
});
