...
const loadConfig = vi.fn<() => CIVITASConfig>(() => ({
  gateway: {
    auth: {
      token: "config-token",
    },
...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { CIVITAS_GATEWAY_TOKEN: "service-token" },
    });
    stubEmptyGatewayEnv();
  });
  ...
  it("prints the container restart hint when restart is requested for a not-loaded service", async () => {
    ...
    vi.stubEnv("CIVITAS_CONTAINER_HINT", "civitas-demo-container");
    ...
    expect(runtimeLogs).toContain(
      "Start with: Restart the container or the service that manages it for civitas-demo-container.",
    );
  });
  ...
  it("compares restart drift against config token even when caller env is set", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { CIVITAS_GATEWAY_TOKEN: "env-token" },
    });
    vi.stubEnv("CIVITAS_GATEWAY_TOKEN", "env-token");
  ...
  it("resolves config token SecretRefs using service command env before drift checks", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: {
        CIVITAS_GATEWAY_TOKEN: "service-token",
  ...
  it("falls back to not-loaded hints when start finds no install artifacts", async () => {
    ...
    await runServiceStart({
      serviceNoun: "Gateway",
      service,
      renderStartHints: () => ["civitas gateway install"],
      opts: { json: true },
    });
    ...
    expect(payload.hints).toEqual(expect.arrayContaining(["civitas gateway install"]));
    expect(payload.hintItems).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "install",
          text: "civitas gateway install",
        }),
      ]),
    );
    expect(service.restart).not.toHaveBeenCalled();
