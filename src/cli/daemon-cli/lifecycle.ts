...
async function assertUnmanagedGatewayRestartEnabled(port: number): Promise<void> {
  ...
  const probe = await probeGateway({
    ...
    auth: {
      token: process.env.CIVITAS_GATEWAY_TOKEN?.trim() || undefined,
      password: process.env.CIVITAS_GATEWAY_PASSWORD?.trim() || undefined,
    },
    timeoutMs: 1_000,
  }).catch(() => null);
...
async function restartGatewayWithoutServiceManager(port: number) {
  ...
  if (pids.length > 1) {
    throw new Error(
      `multiple gateway processes are listening on port ${port}: ${formatGatewayPidList(pids)}; use "civitas gateway status --deep" before retrying restart`,
    );
  }
    ...
    postRestartCheck: async ({ warnings, fail, stdout }) => {
      if (restartedWithoutServiceManager) {
        ...
        fail(`Gateway restart timed out after ${restartWaitSeconds}s waiting for health checks.`, [
          formatCliCommand("civitas gateway status --deep"),
