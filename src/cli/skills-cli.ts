...
import {
  installSkillFromChannelHub,
  readTrackedChannelHubSkillSlugs,
  searchSkillsFromChannelHub,
  updateSkillsFromChannelHub,
} from "../agents/skills-CIVITAS Channelhub.js";
...
export function registerSkillsCli(program: Command) {
  const skills = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/skills", "docs.civitas.ai/cli/skills")}\n`,
    );

  skills
    .command("search")
    .description("Search ChannelHub skills")
    .argument("[query...]", "Optional search query")
    .option("--limit <n>", "Max results", (value) => Number.parseInt(value, 10))
    .option("--json", "Output as JSON", false)
    .action(async (queryParts: string[], opts: { limit?: number; json?: boolean }) => {
      try {
        const results = await searchSkillsFromChannelHub({
        ...
        if (results.length === 0) {
          defaultRuntime.log("No ChannelHub skills found.");
          return;
        }
  ...
  skills
    .command("install")
    .description("Install a skill from ChannelHub into the active workspace")
    .argument("<slug>", "ChannelHub skill slug")
  ...
  skills
    .command("update")
    .description("Update ChannelHub-installed skills in the active workspace")
    .argument("[slug]", "Single skill slug")
    .option("--all", "Update all tracked ChannelHub skills", false)
    .action(async (slug: string | undefined, opts: { all?: boolean }) => {
      try {
        ...
        const workspaceDir = resolveActiveWorkspaceDir();
        const tracked = await readTrackedChannelHubSkillSlugs(workspaceDir);
        if (opts.all && tracked.length === 0) {
          defaultRuntime.log("No tracked ChannelHub skills to update.");
          return;
        }
