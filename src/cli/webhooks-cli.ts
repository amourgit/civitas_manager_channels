...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.civitas.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + CIVITAS hooks")
    ...
    .option("--hook-url <url>", "CIVITAS hook URL")
    .option("--hook-token <token>", "CIVITAS hook token")
