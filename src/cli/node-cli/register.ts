    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "civitas node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["civitas node status", "Check node host service status."],
          ["civitas node install", "Install the node host service."],
          ["civitas node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.civitas.ai/cli/node")}\n`,
    );
