...
function createConfigureInteractiveResult(options?: {
  ...
  return {
    plan: {
      ...
      protocolVersion: 1,
      generatedAt: "2026-02-26T00:00:00.000Z",
      generatedBy: "civitas secrets configure",
      targets: options?.targets ?? [],
    },
    preflight: {
      mode: "dry-run" as const,
      changed: options?.changed ?? false,
      changedFiles: options?.changed ? ["/tmp/civitas.json"] : [],
      checks: {
        resolvability: true,
...
async function withPlanFile(run: (planPath: string) => Promise<void>) {
  const planPath = path.join(
    os.tmpdir(),
    `civitas-secrets-cli-test-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
  );
