...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["civitas security audit", "Run a local security audit."],
          ["civitas security audit --deep", "Include best-effort live Gateway probe checks."],
          ["civitas security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "civitas security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["civitas security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["civitas security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.civitas.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("CIVITAS security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("civitas security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("civitas security audit --fix")}`));
        if (!fixResult) {
