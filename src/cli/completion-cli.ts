...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(civitas completion --shell zsh)
  return (
 ...
 * Returns true if profile has `source <(civitas completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.civitas.ai/cli/completion")}\n`,
    )
    .addOption(
...
export async function installCompletion(shell: string, yes: boolean, binName = "civitas") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
