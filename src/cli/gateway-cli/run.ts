...
function getGatewayStartGuardErrors(params: {
  ...
  if (!params.configExists) {
    return [
      `Missing config. Run \`${formatCliCommand("civitas setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
    ];
  }
  if (params.mode === undefined) {
    return [
      [
        ...
        `Re-run \`${formatCliCommand("civitas onboard --mode local")}\` or \`${formatCliCommand("civitas setup")}\`, set gateway.mode=local manually, or pass --allow-unconfigured.`,
      ].join(" "),
...
async function runGatewayCommand(opts: GatewayRunOpts) {
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("civitas gateway stop")}`,
      );
      try {
