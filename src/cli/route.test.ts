...
describe("tryRouteCli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    originalDisableRouteFirst = process.env.CIVITAS_DISABLE_ROUTE_FIRST;
    delete process.env.CIVITAS_DISABLE_ROUTE_FIRST;
    vi.resetModules();
  ...
  afterEach(() => {
    ...
    if (originalDisableRouteFirst === undefined) {
      ...
      process.env.CIVITAS_DISABLE_ROUTE_FIRST = originalDisableRouteFirst;
    }
  });

  it("skips config guard for routed status --json commands", async () => {
    await expect(tryRouteCli(["node", "civitas", "status", "--json"])).resolves.toBe(true);
  ...
  it("does not pass suppressDoctorStdout for routed non-json commands", async () => {
    await expect(tryRouteCli(["node", "civitas", "status"])).resolves.toBe(true);
  ...
  it("routes logs to stderr during plugin loading in --json mode and restores after", async () => {
    ...
    await tryRouteCli(["node", "civitas", "agents", "--json"]);
  ...
  it("does not route logs to stderr during plugin loading without --json", async () => {
    ...
    await tryRouteCli(["node", "civitas", "agents"]);
  ...
  it("routes status when root options precede the command", async () => {
    await expect(tryRouteCli(["node", "civitas", "--log-level", "debug", "status"])).resolves.toBe(
      true,
    );
