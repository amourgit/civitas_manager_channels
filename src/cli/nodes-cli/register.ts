    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["civitas nodes status", "List known nodes with live status."],
          ["civitas nodes pairing pending", "Show pending node pairing requests."],
          [
            'civitas nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["civitas nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.civitas.ai/cli/nodes")}\n`,
    );
