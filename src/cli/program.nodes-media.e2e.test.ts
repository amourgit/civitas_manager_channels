...
describe("cli program (nodes media)", () => {
  ...
  it("runs nodes camera snap and prints two MEDIA paths", async () => {
    ...
    expect(mediaPaths[0]).toContain("civitas-camera-snap-");
    expect(mediaPaths[1]).toContain("civitas-camera-snap-");
  ...
  it("runs nodes camera clip and prints one MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /civitas-camera-clip-front-.*\.mp4$/,
    });
  });
  ...
  it("runs nodes canvas snapshot and prints MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /civitas-canvas-snapshot-.*\.png$/,
    });
  });
    ...
    it.each([
      {
        ...
        expectedPathPattern: /civitas-camera-snap-front-.*\.jpg$/,
      },
      {
