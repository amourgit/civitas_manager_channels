...
async function resolvePackageRuntimePreflightError(params: {
  ...
  return [
    `Node ${process.versions.node ?? "unknown"} is too old for civitas@${targetLabel}.`,
    `The requested package requires ${status.nodeEngine}.`,
    "Upgrade Node to 22.14+ or Node 24, then rerun `civitas update`.",
    "Bare `npm i -g civitas` can silently install an older compatible release.",
    "After upgrading Node, use `npm i -g civitas@latest`.",
  ].join("\n");
}
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("civitas completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
async function maybeRestartService(params: {
    ...
    try {
      ...
      if (!params.opts.json && restartInitiated) {
        ...
        if (health.healthy) {
          ...
          defaultRuntime.log(
            theme.muted(
              `Run \`${replaceCliName(formatCliCommand("civitas gateway status --deep"), CLI_NAME)}\` for details.`,
            ),
          );
        }
        defaultRuntime.log("");
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("civitas gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("civitas doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("civitas gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("civitas gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      ...
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g civitas@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g civitas@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
