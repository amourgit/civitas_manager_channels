  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        CIVITAS_PROFILE: "default",
      });
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("systemctl --user restart 'civitas-gateway.service'");
      // Script should self-cleanup
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses CIVITAS_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        CIVITAS_PROFILE: "default",
        CIVITAS_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.civitas.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.civitas.gateway'");
      expect(content).toContain("launchctl bootstrap 'gui/501'");
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses CIVITAS_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        CIVITAS_PROFILE: "default",
        CIVITAS_LAUNCHD_LABEL: "com.custom.civitas",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.civitas'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "CIVITAS Gateway"');
      expect(content).toContain('schtasks /Run /TN "CIVITAS Gateway"');
    ...
    it("uses CIVITAS_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        CIVITAS_PROFILE: "default",
        CIVITAS_WINDOWS_TASK_NAME: "CIVITAS Gateway (custom)",
      });
      expect(content).toContain('schtasks /End /TN "CIVITAS Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "CIVITAS Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        CIVITAS_PROFILE: "production",
      });
      expect(content).toContain("civitas-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        CIVITAS_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.civitas.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "CIVITAS Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        CIVITAS_LAUNCHD_LABEL: "ai.civitas.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.civitas.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
