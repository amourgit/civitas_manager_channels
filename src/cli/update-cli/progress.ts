...
const STEP_LABELS: Record<string, string> = {
  ...
  "civitas doctor entry": "Checking doctor entrypoint",
  "civitas doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g civitas@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g civitas@latest --omit=optional");
  }
