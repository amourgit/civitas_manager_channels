...
const SANDBOX_EXAMPLES = {
  main: [
    ["civitas sandbox list", "List all sandbox containers."],
    ["civitas sandbox list --browser", "List only browser containers."],
    ["civitas sandbox recreate --all", "Recreate all containers."],
    ["civitas sandbox recreate --session main", "Recreate a specific session."],
    ["civitas sandbox recreate --agent mybot", "Recreate agent containers."],
    ["civitas sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["civitas sandbox list", "List all sandbox containers."],
    ["civitas sandbox list --browser", "List only browser containers."],
    ["civitas sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["civitas sandbox recreate --all", "Recreate all containers."],
    ["civitas sandbox recreate --session main", "Recreate a specific session."],
    ["civitas sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["civitas sandbox recreate --browser --all", "Recreate only browser containers."],
    ["civitas sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["civitas sandbox explain", "Show effective sandbox config."],
    ["civitas sandbox explain --session agent:main:main", "Explain a specific session."],
    ["civitas sandbox explain --agent work", "Explain an agent sandbox."],
    ["civitas sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.civitas.ai/cli/sandbox")}\n`,
    )
    .action(() => {
