...
vi.mock("../plugins/loader.js", () => ({
  loadCIVITASPlugins: mocks.loadCIVITASPlugins,
}));
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("uses the auto-enabled config snapshot for configured channel scope", async () => {
    ...
    expect(mocks.loadCIVITASPlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        config: autoEnabledConfig,
  ...
  it("reloads when escalating from configured-channels to channels", async () => {
    ...
    expect(mocks.loadCIVITASPlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadCIVITASPlugins).toHaveBeenNthCalledWith(
      1,
  ...
  it("does not treat a pre-seeded partial registry as all scope", async () => {
    ...
    expect(mocks.loadCIVITASPlugins).toHaveBeenCalledTimes(1);
    expect(mocks.loadCIVITASPlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        config,
        throwOnLoadError: true,
