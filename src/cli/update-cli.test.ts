...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/civitas-update-tests";
  let fixtureCount = 0;
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("civitas-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("civitas-updated-root");
  ...
  beforeEach(() => {
    ...
    prepareRestartScript.mockResolvedValue("/tmp/civitas-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "civitas-gateway" }],
      hints: [],
    });
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "civitas", "update", "status", "--json"]);
  ...
  it.each([
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "civitas@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
  ...
  it.each([
    ...
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("civitas-update"));
        await updateCommand({ yes: true, tag: "github:civitas/civitas#main" });
      },
      expectedSpec: "github:civitas/civitas#main",
    },
    {
      ...
      expectedSpec: "http://10.211.55.2:8138/civitas-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "civitas");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "civitas", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("uses the owning npm binary for package updates when PATH npm points elsewhere", async () => {
    ...
    const pkgRoot = path.join(brewRoot, "civitas");
