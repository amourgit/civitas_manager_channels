...
const shouldPreferSourceGraph =
  ...
  (process.env.NODE_ENV !== "production" ||
    ...
    process.env.CIVITAS_PLUGIN_SDK_SOURCE_IN_TESTS === "1");
...
function onDiagnosticEvent(listener) {
  ...
  if (!diagnosticEvents || typeof diagnosticEvents.onDiagnosticEvent !== "function") {
    throw new Error("civitas/plugin-sdk root alias could not resolve onDiagnosticEvent");
  }
...
function buildPluginSdkAliasMap(useDist) {
  ...
  const aliasMap = {
    "civitas/plugin-sdk": __filename,
  };

  for (const subpath of listPluginSdkExportedSubpaths()) {
    const candidate = path.join(pluginSdkDir, `${subpath}${ext}`);
    if (fs.existsSync(candidate)) {
      aliasMap[`civitas/plugin-sdk/${subpath}`] = candidate;
    }
  }
...
function getJiti(tryNative) {
  ...
  const jitiLoader = createJiti(__filename, {
    ...
    // so local plugins do not create a second transpiled CIVITAS core graph.
    tryNative,
