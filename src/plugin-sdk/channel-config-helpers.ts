...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`civitas pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`civitas pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends CIVITASConfig = CIVITASConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends CIVITASConfig = CIVITASConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends CIVITASConfig = CIVITASConfig>(
...
> {
  const base = {
    resolveAllowFrom({ cfg, accountId }: { cfg: CIVITASConfig; accountId?: string | null }) {
...
function setTopLevelChannelEnabledInConfigSection<Config extends CIVITASConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends CIVITASConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends CIVITASConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
