...
describe("backupCreateCommand atomic archive write", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("civitas-backup-atomic-test-");
  });
  ...
  it("does not leave a partial final archive behind when tar creation fails", async () => {
    ...
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-backup-failure-"));
    try {
  ...
  it("does not overwrite an archive created after readiness checks complete", async () => {
    const stateDir = path.join(tempHome.home, ".civitas");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-backup-race-"));
  ...
  it("falls back to exclusive copy when hard-link publication is unsupported", async () => {
    const stateDir = path.join(tempHome.home, ".civitas");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-backup-copy-fallback-"));
    const linkSpy = vi.spyOn(fs, "link");
    try {
      await fs.writeFile(path.join(stateDir, "civitas.json"), JSON.stringify({}), "utf8");
      await fs.writeFile(path.join(stateDir, "state.txt"), "state\n", "utf8");
