...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("civitas doctor --fix")} or ${formatCliCommand("civitas memory status --fix")}`
    : `Verify: ${formatCliCommand("civitas memory status --deep")}`;
  return [
 ...
 * Runs as part of `civitas doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("civitas config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("civitas config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("civitas memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`civitas config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("civitas configure --section model")}`,
        `- To disable: ${formatCliCommand("civitas config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("civitas memory status --deep")}`,
      ].join("\n"),
      "Memory search",
