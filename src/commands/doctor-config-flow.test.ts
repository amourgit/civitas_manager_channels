...
describe("doctor config flow", () => {
  ...
  it("previews Matrix legacy sync-store migration in read-only mode", async () => {
    ...
    try {
      ...
      expect(warning?.[0]).toContain(
        'Run "civitas doctor --fix" to migrate this Matrix state now.',
      );
    } finally {
      noteSpy.mockRestore();
    }
  ...
  it("creates a Matrix migration snapshot before doctor repair mutates Matrix state", async () => {
    await withTempHome(async (home) => {
      ...
      const snapshotDir = path.join(home, "Backups", "civitas-migrations");
      ...
      expect(marker.archivePath).toContain(path.join("Backups", "civitas-migrations"));
    });
  });
  ...
  it("warns when Matrix is installed from a stale custom path", async () => {
    const doctorWarnings = await collectDoctorWarnings({
      ...
      plugins: {
        installs: {
          matrix: {
            source: "path",
            sourcePath: "/tmp/civitas-matrix-missing",
            installPath: "/tmp/civitas-matrix-missing",
          },
        },
      },
    });
    ...
    expect(
      doctorWarnings.some(
        (line) => line.includes("custom path") && line.includes("/tmp/civitas-matrix-missing"),
      ),
    ).toBe(true);
  });
    ...
    try {
      ...
      expect(
        noteSpy.mock.calls.some(
          ([message, title]) =>
            title === "Doctor" &&
            String(message).includes('Run "civitas doctor --fix" to migrate legacy config keys.'),
        ),
      ).toBe(true);
    } finally {
      noteSpy.mockRestore();
  ...
  it("converts numeric discord ids to strings on repair", async () => {
    await withTempHome(async (home) => {
      const configDir = path.join(home, ".civitas");
