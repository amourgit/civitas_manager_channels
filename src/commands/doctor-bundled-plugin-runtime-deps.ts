...
export async function maybeRepairBundledPluginRuntimeDeps(params: {
  ...
  if (conflicts.length > 0) {
    ...
    note(
      [
        ...
        ...conflictLines,
        `Update bundled plugins and rerun ${formatCliCommand("civitas doctor")}.`,
      ].join("\n"),
      "Bundled plugins",
    );
  }
  ...
  note(
    [
      ...
      `Fix: run ${formatCliCommand("civitas doctor --fix")} to install them.`,
    ].join("\n"),
    "Bundled plugins",
  );
