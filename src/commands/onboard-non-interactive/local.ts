...
export async function runNonInteractiveLocalSetup(params: {
        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `civitas gateway run` or `civitas agent --local`.",
              ]
            : [`Run \`${formatCliCommand("civitas gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              ...
              `Fix: start \`${formatCliCommand("civitas gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
  ...
  if (!opts.json) {
    runtime.log(
      `Tip: run \`${formatCliCommand("civitas configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.civitas.ai/tools/web`,
    );
  }
}
