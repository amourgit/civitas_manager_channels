...
export function resolveProviderAuthLoginCommand(params: {
  ...
  return formatCliCommand(`civitas models auth login --provider ${provider.id}`);
}
...
export function buildProviderAuthRecoveryHint(params: {
  ...
  if (params.includeConfigure !== false) {
    parts.push(`\`${formatCliCommand("civitas configure")}\``);
  }
  if (params.includeEnvVar) {
    parts.push("set an API key env var");
  }
  if (parts.length === 0) {
    return `Run \`${formatCliCommand("civitas configure")}\`.`;
  }
  if (parts.length === 1) {
    return `${parts[0]}.`;
  }
