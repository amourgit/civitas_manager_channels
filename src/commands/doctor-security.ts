...
import type { CIVITASConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: CIVITASConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: CIVITASConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "CIVITAS default (full)";
  const defaultRequestedAskSource = "CIVITAS default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("civitas approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: CIVITASConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: CIVITASConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("civitas security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.civitas/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("civitas approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.civitas.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("civitas configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("civitas config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("civitas doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "civitas config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("civitas config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('civitas config set session.dmScope "per-channel-peer"') +
