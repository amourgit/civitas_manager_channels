    ...
    createTestRegistry([
      {
        ...
        plugin: {
          ...
          setup: {
            applyAccountConfig: ({ cfg }: { cfg: CIVITASConfig }) => cfg,
          },
          setupWizard: {
            channel: "telegram",
            status: {
...
describe("setupChannels post-write hooks", () => {
  ...
  it("collects onboarding post-write hooks and runs them against the final config", async () => {
    ...
    const restore = patchChannelOnboardingAdapterForTest({
      configureInteractive,
      afterConfigWritten,
      getStatus: vi.fn(async ({ cfg }: { cfg: CIVITASConfig }) => ({
        channel: "telegram",
    ...
    try {
      const cfg = await setupChannels({} as CIVITASConfig, runtime, prompter, {
