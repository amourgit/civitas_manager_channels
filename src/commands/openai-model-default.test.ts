...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: CIVITASConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: CIVITASConfig) => { changed: boolean; next: CIVITASConfig };
  defaultModel: string;
  overrideConfig: CIVITASConfig;
  alreadyDefaultConfig: CIVITASConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: CIVITASConfig) => config,
      applyDefaultConfig: (config: CIVITASConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: CIVITASConfig = { agents: { defaults: {} } };
