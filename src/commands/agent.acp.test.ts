...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "civitas-agent-acp-" });
}

function createAcpEnabledConfig(home: string, storePath: string): CIVITASConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function mockConfigWithAcpOverrides(
  home: string,
  storePath: string,
  acpOverrides: Partial<NonNullable<CIVITASConfig["acp"]>>,
) {
  ...
  it.each([
    {
      ...
      acpOverrides: { enabled: false } satisfies Partial<NonNullable<CIVITASConfig["acp"]>>,
    },
    {
      name: "blocks ACP turns when ACP dispatch is disabled by policy",
      acpOverrides: {
        dispatch: { enabled: false },
      } satisfies Partial<NonNullable<CIVITASConfig["acp"]>>,
    },
