...
function resolveAgentName(cfg: CIVITASConfig, agentId: string) {
...
function resolveAgentModel(cfg: CIVITASConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: CIVITASConfig): AgentSummary[] {
