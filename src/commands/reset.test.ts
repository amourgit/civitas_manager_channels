...
describe("resetCommand", () => {
  ...
  beforeEach(() => {
    ...
    resolveCleanupPlanFromDisk.mockReturnValue({
      stateDir: "/tmp/.civitas",
      configPath: "/tmp/.civitas/civitas.json",
      oauthDir: "/tmp/.civitas/credentials",
      configInsideState: true,
      oauthInsideState: true,
      workspaceDirs: ["/tmp/.civitas/workspace"],
    });
    removePath.mockResolvedValue({ ok: true });
    listAgentSessionDirs.mockResolvedValue(["/tmp/.civitas/agents/main/sessions"]);
  ...
  it("recommends creating a backup before state-destructive reset scopes", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(expect.stringContaining("civitas backup create"));
  });
  ...
  it("does not recommend backup for config-only reset", async () => {
    ...
    expect(runtime.log).not.toHaveBeenCalledWith(expect.stringContaining("civitas backup create"));
  });
});
