...
export async function buildGatewayInstallPlan(params: {
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      process.platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(params.env.CIVITAS_PROFILE)
        : undefined,
  ...
  //   1. ~/.civitas/.env file vars  (lowest — user secrets / fallback keys)
  //   2. Config env vars              (civitas.json env.vars + inline keys)
  //   3. Auth-profile env refs        (credential store → env var lookups)
  //   4. Service environment          (HOME, PATH, CIVITAS_* — highest)
  return {
    programArguments,
    workingDirectory,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("civitas gateway install")}\` after fixing the error.`;
}
