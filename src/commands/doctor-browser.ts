...
function collectChromeMcpProfiles(cfg: CIVITASConfig): ExistingSessionProfile[] {
...
export async function noteChromeMcpBrowserReadiness(
  ...
  if (autoConnectProfiles.length === 0) {
    noteFn(
      [
        ...
        "- Keep the browser running and accept the attach consent prompt the first time CIVITAS connects.",
      ].join("\n"),
      "Browser",
    );
    return;
  ...
  if (!chrome) {
    const lines = [
      `- Chrome MCP existing-session is configured for profile(s): ${profileLabel}.`,
      `- Google Chrome was not found on this host for auto-connect profile(s): ${autoProfileLabel}. CIVITAS does not bundle Chrome.`,
