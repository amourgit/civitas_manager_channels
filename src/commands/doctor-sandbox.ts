...
function resolveSandboxDockerImage(cfg: CIVITASConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.docker?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_IMAGE;
}

function resolveSandboxBackend(cfg: CIVITASConfig): string {
  const backend = cfg.agents?.defaults?.sandbox?.backend?.trim();
  return backend || "docker";
}

function resolveSandboxBrowserImage(cfg: CIVITASConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.browser?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_BROWSER_IMAGE;
}

function updateSandboxDockerImage(cfg: CIVITASConfig, image: string): CIVITASConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
function updateSandboxBrowserImage(cfg: CIVITASConfig, image: string): CIVITASConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
export async function maybeRepairSandboxImages(
  ...
  if (!dockerAvailable) {
    const lines = [
      ...
      "- Disable sandbox mode: civitas config set agents.defaults.sandbox.mode off",
    ];
    note(lines.join("\n"), "Sandbox");
    return cfg;
  }
...
export function noteSandboxScopeWarnings(cfg: CIVITASConfig) {
