...
beforeEach(() => {
  envSnapshot = captureEnv(["CIVITAS_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  tempAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-auth-"));
  process.env.CIVITAS_AGENT_DIR = tempAgentDir;
...
describe("maybeRemoveDeprecatedCliAuthProfiles", () => {
  it("removes deprecated CLI auth profiles from store + config", async () => {
    ...
    const next = await maybeRemoveDeprecatedCliAuthProfiles(
      cfg as unknown as CIVITASConfig,
      makePrompter(true),
    );
