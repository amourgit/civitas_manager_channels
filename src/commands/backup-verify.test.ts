...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-civitas-backup";
...
describe("backupVerifyCommand", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("civitas-backup-verify-test-");
  });
  ...
  it("verifies an archive created by backup create", async () => {
    const stateDir = path.join(tempHome.home, ".civitas");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-backup-verify-out-"));
    try {
      await fs.writeFile(path.join(stateDir, "civitas.json"), JSON.stringify({}), "utf8");
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-civitas-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.civitas",
            archivePath: `${rootName}/payload/posix/tmp/.civitas`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "civitas-backup-traversal-",
  ...
  it("fails when archive paths contain backslashes", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "civitas-backup-backslash-",
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    const stateDir = path.join(tempHome.home, ".civitas");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-workspace-"));
    const configPath = path.join(tempHome.home, "custom-config.json");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-backup-verify-out-"));
    try {
      process.env.CIVITAS_CONFIG_PATH = configPath;
      await fs.writeFile(
        configPath,
        JSON.stringify({
          agents: {
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.civitas/payload.txt`;
    await withBrokenArchiveFixture(
      {
        tempPrefix: "civitas-backup-duplicate-manifest-",
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "civitas-backup-duplicate-payload-",
        manifestAssetArchivePath: payloadArchivePath,
        payloads: [
