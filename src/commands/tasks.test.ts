...
const ORIGINAL_STATE_DIR = process.env.CIVITAS_STATE_DIR;
...
async function withTaskCommandStateDir(run: () => Promise<void>): Promise<void> {
  await withTempDir({ prefix: "civitas-tasks-command-" }, async (root) => {
...
describe("tasks commands", () => {
  ...
  afterEach(() => {
    ...
    if (ORIGINAL_STATE_DIR === undefined) {
      ...
      process.env.CIVITAS_STATE_DIR = ORIGINAL_STATE_DIR;
    }
    resetTaskRegistryDeliveryRuntimeForTests();
    resetTaskRegistryForTests();
    resetTaskFlowRegistryForTests();
