...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.CIVITAS_DEBUG_HEALTH)) {
...
async function inspectHealthAccount(plugin: ChannelPlugin, cfg: CIVITASConfig, accountId: string) {
  return (
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: CIVITASConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.CIVITAS_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
