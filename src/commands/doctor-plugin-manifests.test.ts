...
function makeTempDir() {
  return makeTrackedTempDir("civitas-doctor-plugin-manifests", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(
    path.join(dir, "civitas.plugin.json"),
...
describe("doctor plugin manifest legacy contract repair", () => {
  ...
  it("collects legacy top-level capability keys for migration", () => {
    ...
    const migrations = collectLegacyPluginManifestContractMigrations({
      env: {
        ...process.env,
        CIVITAS_BUNDLED_PLUGINS_DIR: pluginsRoot,
      },
    });
  ...
  it("rewrites legacy top-level capability keys into contracts", async () => {
    ...
    const next = JSON.parse(fs.readFileSync(path.join(root, "civitas.plugin.json"), "utf-8")) as {
