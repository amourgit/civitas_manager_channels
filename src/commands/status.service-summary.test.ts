...
describe("readServiceStatusSummary", () => {
  it("marks CIVITAS-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["civitas", "gateway", "run"] })),
    ...
    expect(summary.managedByCIVITAS).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByCIVITAS).toBe(false);
  ...
  it("passes command environment to runtime and loaded checks", async () => {
    const isLoaded = vi.fn(async ({ env }: GatewayServiceEnvArgs) => {
      return env?.CIVITAS_GATEWAY_PORT === "18789";
    });
    const readRuntime = vi.fn(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.CIVITAS_GATEWAY_PORT === "18789" ? ("running" as const) : ("unknown" as const),
    }));
    ...
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({
          programArguments: ["civitas", "gateway", "run", "--port", "18789"],
          environment: { CIVITAS_GATEWAY_PORT: "18789" },
        })),
        readRuntime,
      }),
      "Daemon",
