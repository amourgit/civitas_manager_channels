...
async function withOnboardEnv(
  ...
  const configPath = path.join(tempHome, "civitas.json");
  ...
  try {
    await withEnvAsync(
      {
        HOME: tempHome,
        CIVITAS_STATE_DIR: tempHome,
        CIVITAS_CONFIG_PATH: configPath,
        CIVITAS_SKIP_CHANNELS: "1",
        CIVITAS_SKIP_GMAIL_WATCHER: "1",
        CIVITAS_SKIP_CRON: "1",
        CIVITAS_SKIP_CANVAS_HOST: "1",
        CIVITAS_GATEWAY_TOKEN: undefined,
        CIVITAS_GATEWAY_PASSWORD: undefined,
        CUSTOM_API_KEY: undefined,
        CIVITAS_DISABLE_CONFIG_CACHE: "1",
        CIVITAS_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
        CIVITAS_DISABLE_PLUGIN_MANIFEST_CACHE: "1",
      },
      async () => {
...
describe("onboard (non-interactive): provider auth", () => {
  ...
  it("stores MiniMax API key in the global auth profile", async () => {
    await withOnboardEnv("civitas-onboard-minimax-", async (env) => {
  ...
  it("supports MiniMax CN API endpoint auth choice", async () => {
    await withOnboardEnv("civitas-onboard-minimax-cn-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Z.AI API key after probing the global endpoint", async () => {
    await withZaiProbeFetch(
      ...
      async (fetchMock) =>
        await withOnboardEnv("civitas-onboard-zai-", async (env) => {
          const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("supports Z.AI CN coding endpoint auth choice", async () => {
    await withZaiProbeFetch(
      ...
      async (fetchMock) =>
        await withOnboardEnv("civitas-onboard-zai-cn-", async (env) => {
          const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("supports Z.AI Coding Plan global endpoint detection", async () => {
    await withZaiProbeFetch(
      ...
      async (fetchMock) =>
        await withOnboardEnv("civitas-onboard-zai-coding-global-", async (env) => {
          const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores xAI API key in the default auth profile", async () => {
    await withOnboardEnv("civitas-onboard-xai-", async (env) => {
  ...
  it("infers Mistral auth choice from --mistral-api-key", async () => {
    await withOnboardEnv("civitas-onboard-mistral-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Volcano Engine API key and sets default model", async () => {
    await withOnboardEnv("civitas-onboard-volcengine-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers BytePlus auth choice from --byteplus-api-key and sets default model", async () => {
    await withOnboardEnv("civitas-onboard-byteplus-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("civitas-onboard-ai-gateway-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores legacy Anthropic setup-token onboarding again when explicitly selected", async () => {
    await withOnboardEnv("civitas-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("civitas-onboard-openai-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it.each([
    {
      name: "anthropic",
      prefix: "civitas-onboard-ref-flag-anthropic-",
      authChoice: "apiKey",
      optionKey: "anthropicApiKey",
      flagName: "--anthropic-api-key",
      envVar: "ANTHROPIC_API_KEY",
    ...
    {
      name: "openai",
      prefix: "civitas-onboard-ref-flag-openai-",
      authChoice: "openai-api-key",
      optionKey: "openaiApiKey",
      flagName: "--openai-api-key",
      envVar: "OPENAI_API_KEY",
    ...
    {
      name: "openrouter",
      prefix: "civitas-onboard-ref-flag-openrouter-",
    ...
    {
      name: "xai",
      prefix: "civitas-onboard-ref-flag-xai-",
      authChoice: "xai-api-key",
      optionKey: "xaiApiKey",
      flagName: "--xai-api-key",
      envVar: "XAI_API_KEY",
    ...
    {
      name: "volcengine",
      prefix: "civitas-onboard-ref-flag-volcengine-",
      authChoice: "volcengine-api-key",
      optionKey: "volcengineApiKey",
      flagName: "--volcengine-api-key",
      envVar: "VOLCANO_ENGINE_API_KEY",
    ...
    {
      name: "byteplus",
      prefix: "civitas-onboard-ref-flag-byteplus-",
  ...
  it("stores the detected env alias as keyRef for both OpenCode runtime providers", async () => {
    await withOnboardEnv("civitas-onboard-ref-opencode-alias-", async ({ runtime }) => {
      await withEnvAsync(
        {
  ...
  it("configures vLLM via the provider plugin in non-interactive mode", async () => {
    await withOnboardEnv("civitas-onboard-vllm-non-interactive-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
        authChoice: "vllm",
  ...
  it("configures SGLang via the provider plugin in non-interactive mode", async () => {
    await withOnboardEnv("civitas-onboard-sglang-non-interactive-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
        authChoice: "sglang",
  ...
  it("stores LiteLLM API key in the default auth profile", async () => {
    await withOnboardEnv("civitas-onboard-litellm-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it.each([
    {
      ...
      prefix: "civitas-onboard-cf-gateway-",
      options: {
    ...
    },
    {
      ...
      prefix: "civitas-onboard-cf-gateway-infer-",
      options: {},
    },
  ...
  it("infers Together auth choice from --together-api-key and sets default model", async () => {
    await withOnboardEnv("civitas-onboard-together-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers QIANFAN auth choice from --qianfan-api-key and sets default model", async () => {
    await withOnboardEnv("civitas-onboard-qianfan-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers Qwen auth choice from --modelstudio-api-key and sets default model", async () => {
    await withOnboardEnv("civitas-onboard-modelstudio-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("configures a custom provider from non-interactive flags", async () => {
    await withOnboardEnv("civitas-onboard-custom-provider-", async ({ configPath, runtime }) => {
      await runNonInteractiveSetupWithDefaults(runtime, {
        authChoice: "custom-api-key",
  ...
  it("infers custom provider auth choice from custom flags", async () => {
    await withOnboardEnv(
      "civitas-onboard-custom-provider-infer-",
  ...
  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "civitas-onboard-custom-provider-env-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("stores CUSTOM_API_KEY env ref for non-interactive custom provider auth in ref mode", async () => {
    await withOnboardEnv(
      "civitas-onboard-custom-provider-env-ref-",
      async ({ configPath, runtime }) => {
        process.env.CUSTOM_API_KEY = "custom-env-key"; // pragma: allowlist secret
        await runCustomLocalNonInteractive(runtime, {
          secretInputMode: "ref", // pragma: allowlist secret
  ...
  it("fails fast for custom provider ref mode when --custom-api-key is set but CUSTOM_API_KEY env is missing", async () => {
    await withOnboardEnv("civitas-onboard-custom-provider-ref-flag-", async ({ runtime }) => {
  ...
  it("uses matching profile fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "civitas-onboard-custom-provider-profile-fallback-",
      async ({ configPath, runtime }) => {
        upsertAuthProfile({
  ...
  it("fails custom provider auth when compatibility is invalid", async () => {
    await withOnboardEnv(
      "civitas-onboard-custom-provider-invalid-compat-",
      async ({ runtime }) => {
        await expect(
  ...
  it("fails custom provider auth when explicit provider id is invalid", async () => {
    await withOnboardEnv("civitas-onboard-custom-provider-invalid-id-", async ({ runtime }) => {
      await expect(
        runNonInteractiveSetupWithDefaults(runtime, {
          authChoice: "custom-api-key",
          customBaseUrl: "https://models.custom.local/v1",
  ...
  it("fails inferred custom auth when required flags are incomplete", async () => {
    await withOnboardEnv(
      "civitas-onboard-custom-provider-missing-required-",
      async ({ runtime }) => {
        await expect(
          runNonInteractiveSetupWithDefaults(runtime, {
            customApiKey: "custom-test-key", // pragma: allowlist secret
