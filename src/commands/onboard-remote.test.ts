...
describe("promptRemoteGatewayConfig", () => {
  const envSnapshot = captureEnv(["CIVITAS_ALLOW_INSECURE_PRIVATE_WS"]);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    envSnapshot.restore();
    delete process.env.CIVITAS_ALLOW_INSECURE_PRIVATE_WS;
  ...
  it("falls back to manual URL entry when discovery trust is declined", async () => {
    ...
    const next = await promptRemoteGatewayConfig({} as CIVITASConfig, prompter);
  ...
  it("allows ws:// hostname remote URLs when CIVITAS_ALLOW_INSECURE_PRIVATE_WS=1", async () => {
    process.env.CIVITAS_ALLOW_INSECURE_PRIVATE_WS = "1";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        expect(params.validate?.("ws://civitas-gateway.ai:18789")).toBeUndefined();
        expect(params.validate?.("ws://1.1.1.1:18789")).toContain("Use wss://");
        return "ws://civitas-gateway.ai:18789";
      }
      return "";
    }) as WizardPrompter["text"];
    ...
    expect(next.gateway?.mode).toBe("remote");
    expect(next.gateway?.remote?.url).toBe("ws://civitas-gateway.ai:18789");
  });

  it("supports storing remote auth as an external env secret ref", async () => {
    process.env.CIVITAS_GATEWAY_TOKEN = "remote-token-value";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        return "wss://remote.example.com:18789";
      }
      if (params.message === "Environment variable name") {
        return "CIVITAS_GATEWAY_TOKEN";
      }
      return "";
    }) as WizardPrompter["text"];
