...
describe("resolveDefaultChannelAccountContext", () => {
  it("uses enabled/configured defaults when hooks are missing", async () => {
    ...
    const result = await resolveDefaultChannelAccountContext(plugin, {} as CIVITASConfig);
  ...
  it("keeps strict mode fail-closed when resolveAccount throws", async () => {
    ...
    await expect(resolveDefaultChannelAccountContext(plugin, {} as CIVITASConfig)).rejects.toThrow(
      /missing secret/i,
    );
  });
