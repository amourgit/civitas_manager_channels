...
describe("doctor state integrity oauth dir checks", () => {
  ...
  it("prints civitas-only verification hints when recent sessions are missing transcripts", async () => {
    ...
    expect(text).toMatch(/civitas sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/civitas sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /civitas sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
