...
import { writeConfigFile, type CIVITASConfig } from "../config/config.js";
...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      ...
      message:
        "Gateway service CIVITAS_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing CIVITAS gateway next.");
      }
    }
  }
