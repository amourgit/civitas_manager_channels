...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/civitas-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/civitas-dev/civitas.json",
    }),
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/civitas-agent");
    }
  }
}
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/civitas-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/civitas-agent-custom");
