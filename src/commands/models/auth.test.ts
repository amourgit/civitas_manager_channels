...
describe("modelsAuthLoginCommand", () => {
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/civitas/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/civitas/workspace");
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Tip: Codex-capable models can use native Codex web search. Enable it with civitas configure --section web (recommended mode: cached). Docs: https://docs.civitas.ai/tools/web",
    );
  });
  ...
  it("loads lockout state from the agent-scoped store", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/civitas/agents/main");
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `civitas plugins list --json`.',
    );
  });
