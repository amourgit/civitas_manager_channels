...
export async function loadValidConfigOrThrow(): Promise<CIVITASConfig> {
...
export async function updateConfig(
  mutator: (cfg: CIVITASConfig) => CIVITASConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: CIVITASConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: CIVITASConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("civitas agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
