...
const secretRefKeyMock = vi.hoisted(() => vi.fn(() => "env:default:CIVITAS_GATEWAY_TOKEN"));
...
describe("resolveGatewayInstallToken", () => {
  ...
  it("validates SecretRef token but does not persist resolved plaintext", async () => {
    const tokenRef = { source: "env", provider: "default", id: "CIVITAS_GATEWAY_TOKEN" };
    resolveSecretInputRefMock.mockReturnValue({ ref: tokenRef });
    resolveSecretRefValuesMock.mockResolvedValue(
      new Map([["env:default:CIVITAS_GATEWAY_TOKEN", "resolved-token"]]),
    );
    ...
    const result = await resolveGatewayInstallToken({
      ...
      env: { CIVITAS_GATEWAY_TOKEN: "resolved-token" } as NodeJS.ProcessEnv,
    });
  ...
  it("returns unavailable reason when token and password are both configured and mode is unset", async () => {
    ...
    expect(result.unavailableReason).toContain("civitas config set gateway.auth.mode token");
    expect(result.unavailableReason).toContain("civitas config set gateway.auth.mode password");
  ...
  it("drops generated plaintext when config changes to SecretRef before persist", async () => {
    ...
    resolveSecretInputRefMock.mockReturnValueOnce({ ref: undefined }).mockReturnValueOnce({
      ref: { source: "env", provider: "default", id: "CIVITAS_GATEWAY_TOKEN" },
    });
  ...
  it("passes the install env through to gateway auth resolution", async () => {
    const env = {
      CIVITAS_GATEWAY_PASSWORD: "dotenv-password", // pragma: allowlist secret
