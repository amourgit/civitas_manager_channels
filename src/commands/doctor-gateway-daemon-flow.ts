...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin" && params.cfg.gateway?.mode !== "remote") {
    ...
    await maybeRepairLaunchAgentBootstrap({
      env: {
        ...process.env,
        CIVITAS_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
      },
      title: "Node",
      runtime: params.runtime,
      prompter: params.prompter,
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.CIVITAS_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("civitas gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
