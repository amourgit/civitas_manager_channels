...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.CIVITAS_STATE_DIR = missingDir;
  ...
  it("skips gateway auth warning when CIVITAS_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.CIVITAS_GATEWAY_TOKEN = "env-token-1234567890";
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
  ...
  it("warns when token and password are both configured and gateway.auth.mode is unset", async () => {
    ...
    expect(String(gatewayAuthNote?.[0])).toContain("civitas config set gateway.auth.mode token");
    expect(String(gatewayAuthNote?.[0])).toContain(
      "civitas config set gateway.auth.mode password",
    );
  });
  ...
  it("keeps doctor read-only when gateway token is SecretRef-managed but unresolved", async () => {
    ...
    const previousToken = process.env.CIVITAS_GATEWAY_TOKEN;
    delete process.env.CIVITAS_GATEWAY_TOKEN;
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
      ...
      if (previousToken === undefined) {
        ...
        process.env.CIVITAS_GATEWAY_TOKEN = previousToken;
      }
    }
