...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "CIVITAS_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("CIVITAS_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv CIVITAS_GATEWAY_TOKEN");
    expect(message).not.toContain("CIVITAS_GATEWAY_PASSWORD");
  });
  ...
  it("treats SecretRef-backed credentials as configured", async () => {
    const noteFn = vi.fn();
    const getenv = vi.fn(async (name: string) =>
      name === "CIVITAS_GATEWAY_PASSWORD" ? "launchctl-password" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("CIVITAS_GATEWAY_PASSWORD");
  });
