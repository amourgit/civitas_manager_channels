...
const SEARCH_TOOL = "https://docs.civitas.ai/mcp.SearchOpenClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.civitas.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('civitas docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.civitas.ai/");
      runtime.log(`Search: ${formatCliCommand('civitas docs "your query"')}`);
    }
    return;
  }
