...
describe("noteStartupOptimizationHints", () => {
  it("does not warn when compile cache and no-respawn are configured", () => {
    ...
    noteStartupOptimizationHints(
      {
        NODE_COMPILE_CACHE: "/var/tmp/civitas-compile-cache",
        CIVITAS_NO_RESPAWN: "1",
      },
  ...
  it("warns when compile cache is under /tmp and no-respawn is not set", () => {
    ...
    noteStartupOptimizationHints(
      {
        NODE_COMPILE_CACHE: "/tmp/civitas-compile-cache",
      },
      { platform: "linux", arch: "arm64", totalMemBytes: 4 * 1024 ** 3, noteFn },
    );
    ...
    expect(message).toContain("CIVITAS_NO_RESPAWN is not set to 1");
    expect(message).toContain("export NODE_COMPILE_CACHE=/var/tmp/civitas-compile-cache");
    expect(message).toContain("export CIVITAS_NO_RESPAWN=1");
  });
