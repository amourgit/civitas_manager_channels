...
describe("ensureChannelSetupPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    expect(result.cfg.plugins?.installs?.zalo?.spec).toBe("@civitas/zalo");
    expect(result.cfg.plugins?.installs?.zalo?.installPath).toBe("/tmp/zalo");
    expect(installPluginFromNpmSpec).toHaveBeenCalledWith(
      expect.objectContaining({ spec: "@civitas/zalo" }),
    );
  });
  ...
  it("uses the catalog plugin id for local-path installs", async () => {
    ...
    const result = await ensureChannelSetupPluginInstalled({
      ...
      entry: {
        ...baseEntry,
        id: "teams",
        pluginId: "@civitas/msteams-plugin",
      },
      prompter,
      runtime,
    });
    ...
    expect(result.installed).toBe(true);
    expect(result.pluginId).toBe("@civitas/msteams-plugin");
    expect(result.cfg.plugins?.entries?.["@civitas/msteams-plugin"]?.enabled).toBe(true);
  });
    ...
    resolveBundledPluginSources.mockReturnValue(
      new Map([
        [
          ...
          {
            pluginId: "zalo",
            localPath: bundledPluginRootAt("/opt/civitas", "zalo"),
            npmSpec: "@civitas/zalo",
          },
        ],
      ]),
    ...
    expect(select).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: expect.arrayContaining([
          expect.objectContaining({
            value: "local",
            hint: bundledPluginRootAt("/opt/civitas", "zalo"),
          }),
        ]),
      }),
    );
    ...
    resolveBundledPluginSources.mockReturnValue(
      new Map([
        [
          ...
          {
            pluginId: "whatsapp",
            localPath: bundledPluginRootAt("/opt/civitas", "whatsapp"),
  ...
  it("scopes channel reloads when setup starts from an empty registry", () => {
    ...
    expect(loadCIVITASPlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        workspaceDir: "/tmp/civitas-workspace",
        cache: false,
        onlyPluginIds: ["@civitas/telegram-plugin"],
        includeSetupOnlyChannelPlugins: true,
      }),
    );
  });
  ...
  it("scopes snapshots by plugin id when channel and plugin ids differ", () => {
    ...
    expect(loadCIVITASPlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        workspaceDir: "/tmp/civitas-workspace",
        cache: false,
        onlyPluginIds: ["@civitas/msteams-plugin"],
        includeSetupOnlyChannelPlugins: true,
        activate: false,
      }),
    );
