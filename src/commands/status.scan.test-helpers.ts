...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/civitas-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = CIVITASConfig>(
  overrides: T = {} as T,
): CIVITASConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): CIVITASConfig {
