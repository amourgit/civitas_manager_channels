...
describe("resolveGatewayAuthTokenForService", () => {
  ...
  it("falls back to CIVITAS_GATEWAY_TOKEN when SecretRef is unresolved", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
      ...
      {
        CIVITAS_GATEWAY_TOKEN: "env-fallback-token",
      } as NodeJS.ProcessEnv,
    );
  ...
  it("falls back to CIVITAS_GATEWAY_TOKEN when SecretRef resolves to empty", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
...
describe("shouldRequireGatewayTokenForInstall", () => {
  ...
  it("requires token in inferred mode when password env exists only in shell", async () => {
    await withEnvAsync(
      { [envVar("CIVITAS", "GATEWAY", "PASSWORD")]: "password-from-env" },
      async () => {
    ...
    const required = shouldRequireGatewayTokenForInstall(
      {
        ...
        env: {
          vars: {
            CIVITAS_GATEWAY_PASSWORD: "configured-password", // pragma: allowlist secret
          },
        },
      } as CIVITASConfig,
      {} as NodeJS.ProcessEnv,
  ...
  it("does not require token in inferred mode when password env exists in state-dir .env", async () => {
    await withTempHome(async (_home) => {
      await writeStateDirDotEnv("CIVITAS_GATEWAY_PASSWORD=dotenv-password\n", {
        env: process.env,
      });
