...
export function hasExplicitMemorySearchConfig(cfg: CIVITASConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: CIVITASConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: CIVITASConfig, agentId: string) => { store: { path: string } } | null;
