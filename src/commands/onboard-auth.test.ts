...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under CIVITAS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("civitas-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-oauth-sync-"));
    ...
    process.env.CIVITAS_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-oauth-nosync-"));
    process.env.CIVITAS_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside CIVITAS_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-oauth-external-"));
    process.env.CIVITAS_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* CIVITAS_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to CIVITAS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("civitas-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
