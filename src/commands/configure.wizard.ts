...
async function runGatewayHealthCheck(params: {
  ...
  try {
    ...
    note(
      [
        "Docs:",
        "https://docs.civitas.ai/gateway/health",
        "https://docs.civitas.ai/gateway/troubleshooting",
      ].join("\n"),
      "Health check help",
    );
  }
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from civitas.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.civitas.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("civitas doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    note(
      [
        ...
        gatewayStatusLine,
        "Docs: https://docs.civitas.ai/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
