...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    note.mockClear();
    pluginRegistry.list = [];
    prevToken = process.env.CIVITAS_GATEWAY_TOKEN;
    prevPassword = process.env.CIVITAS_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    ...
    if (prevPassword === undefined) {
      ...
      process.env.CIVITAS_GATEWAY_PASSWORD = prevPassword;
    }
  ...
  async function withExecApprovalsFile(
    ...
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-doctor-security-"));
    process.env.HOME = home;
    await fs.mkdir(path.join(home, ".civitas"), { recursive: true });
    await fs.writeFile(
      path.join(home, ".civitas", "exec-approvals.json"),
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as CIVITASConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.CIVITAS_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as CIVITASConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats SecretRef token config as authenticated for exposure warning level", async () => {
    const cfg = {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: { source: "env", provider: "default", id: "CIVITAS_GATEWAY_TOKEN" },
        },
      },
    } as CIVITASConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as CIVITASConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as CIVITASConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("civitas approvals get --gateway");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    await noteSecurityWarnings({} as CIVITASConfig);
    const message = lastMessage();
    expect(message).toContain("[secrets]");
    expect(message).toContain("failed to resolve account");
    expect(message).toContain("Run: civitas security audit --deep");
  });
