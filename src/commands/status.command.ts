...
export async function statusCommand(
  ...
  if (summary.taskAudit.errors > 0) {
    runtime.log("");
    runtime.log(
      theme.muted(`Task maintenance: ${formatCliCommand("civitas tasks maintenance --apply")}`),
    );
  }
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`civitas devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("civitas devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("civitas devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("civitas security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("civitas security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.civitas.ai/faq");
  runtime.log("Troubleshooting: https://docs.civitas.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("civitas status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("civitas logs --follow")}`);
  if (nodeOnlyGateway) {
    runtime.log(`  Need node service?  ${formatCliCommand("civitas node status")}`);
  } else if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("civitas status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("civitas gateway probe")}`);
  }
}
