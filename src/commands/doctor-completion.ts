...
async function generateCompletionCache(): Promise<boolean> {
  ...
  const binPath = path.join(root, "civitas.mjs");
...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(civitas completion ...)` */
 ...
 */
export async function ensureCompletionCacheExists(binName = "civitas"): Promise<boolean> {
  const shell = resolveShellFromEnv() as CompletionShell;
  const cacheExists = await completionCacheExists(shell, binName);
