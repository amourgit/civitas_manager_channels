...
function createPerplexityConfig(apiKey: string, enabled?: boolean): CIVITASConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: CIVITASConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): CIVITASConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: CIVITASConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: CIVITASConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
