...
function validateGatewayWebSocketUrl(value: string): string | undefined {
  ...
  if (
    !isSecureWebSocketUrl(trimmed, {
      allowPrivateWs: process.env.CIVITAS_ALLOW_INSECURE_PRIVATE_WS === "1",
    })
  ) {
    return (
      "Use wss:// for remote hosts, or ws://127.0.0.1/localhost via SSH tunnel. " +
      "Break-glass: CIVITAS_ALLOW_INSECURE_PRIVATE_WS=1 for trusted private networks."
    );
  }
  return undefined;
}
  ...
  if (!hasBonjourTool) {
    await prompter.note(
      [
        ...
        "Docs: https://docs.civitas.ai/gateway/discovery",
      ].join("\n"),
      "Discovery",
    );
  }
  ...
  if (authChoice === "token") {
    const selectedMode = await resolveSecretInputModeForEnvSelection({
      ...
      copy: {
        ...
        plaintextHint: "Stores the token directly in CIVITAS config",
      },
    });
    if (selectedMode === "ref") {
      const resolved = await promptSecretRefForSetup({
        ...
        config: cfg,
        prompter,
        preferredEnvVar: "CIVITAS_GATEWAY_TOKEN",
        copy: {
          sourceMessage: "Where is this gateway token stored?",
          envVarPlaceholder: "CIVITAS_GATEWAY_TOKEN",
        },
      });
      token = resolved.ref;
    } else {
    ...
    const selectedMode = await resolveSecretInputModeForEnvSelection({
      ...
      copy: {
        ...
        plaintextHint: "Stores the password directly in CIVITAS config",
      },
    });
    if (selectedMode === "ref") {
      const resolved = await promptSecretRefForSetup({
        ...
        config: cfg,
        prompter,
        preferredEnvVar: "CIVITAS_GATEWAY_PASSWORD",
        copy: {
          sourceMessage: "Where is this gateway password stored?",
          envVarPlaceholder: "CIVITAS_GATEWAY_PASSWORD",
        },
      });
