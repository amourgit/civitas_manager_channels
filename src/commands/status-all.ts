...
export async function statusAllCommand(
  ...
  await withProgress({ label: "Scanning status --all…", total: 11 }, async (progress) => {
    ...
    const root = await resolveCIVITASPackageRoot({
    ...
    const readServiceSummary = async (service: GatewayService) => {
      try {
        ...
        return {
          ...
          managedByCIVITAS: summary.managedByCIVITAS,
    ...
    const overviewRows = [
      ...
      ...(probeAuthResolution.warning
        ...
        : []),
      { Item: "Security", Value: `Run: ${formatCliCommand("civitas security audit --deep")}` },
      gatewaySelfLine
      ...
      daemon
        ? {
            ...
            Value: !daemon.installed
              ...
              : `${daemon.label} ${daemon.managedByCIVITAS ? "installed · " : ""}${daemon.loadedText}${daemon.runtime?.status ? ` · ${daemon.runtime.status}` : ""}${daemon.runtime?.pid ? ` (pid ${daemon.runtime.pid})` : ""}`,
          }
      ...
      nodeService
        ? {
            ...
            Value: !nodeService.installed
              ...
              : `${nodeService.label} ${nodeService.managedByCIVITAS ? "installed · " : ""}${nodeService.loadedText}${nodeService.runtime?.status ? ` · ${nodeService.runtime.status}` : ""}${nodeService.runtime?.pid ? ` (pid ${nodeService.runtime.pid})` : ""}`,
          }
