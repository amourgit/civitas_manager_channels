...
vi.mock("../agents/workspace.js", () => {
  const resolveDefaultAgentWorkspaceDir = () => "/tmp/civitas-workspace";
  return {
    DEFAULT_AGENT_WORKSPACE_DIR: "/tmp/civitas-workspace",
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "civitas-agent-" });
}
...
function mockConfig(
  ...
  telegramOverrides?: Partial<NonNullable<NonNullable<CIVITASConfig["channels"]>["telegram"]>>,
  ...
  const cfg = {
    agents: {
      defaults: {
        ...
        workspace: path.join(home, "civitas"),
        ...agentOverrides,
      },
      list: agentsList,
    },
...
async function withCrossAgentResumeFixture(
  run: (params: { sessionId: string; sessionKey: string; cfg: CIVITASConfig }) => Promise<void>,
): Promise<void> {
