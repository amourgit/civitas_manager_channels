...
import { CIVITASSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: CIVITASConfig): {
  config: CIVITASConfig;
  removed: string[];
} {
  const parsed = CIVITASSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: CIVITASConfig): void {
