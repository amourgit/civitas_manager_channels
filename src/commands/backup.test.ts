...
describe("backup commands", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("civitas-backup-test-");
  });
  ...
  it("orders coverage checks by canonical path so symlinked workspaces do not duplicate state", async () => {
    ...
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-workspace-link-"));
    ...
    try {
      ...
      await fs.writeFile(
        path.join(stateDir, "civitas.json"),
        JSON.stringify({
          agents: {
            defaults: {
  ...
  it("creates an archive with a manifest and external workspace payload", async () => {
    ...
    const configPath = path.join(tempHome.home, "custom-config.json");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-backups-"));
    try {
      process.env.CIVITAS_CONFIG_PATH = configPath;
      await fs.writeFile(
        configPath,
      ...
      const extractDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-backup-extract-"));
      try {
  ...
  it("optionally verifies the archive after writing it", async () => {
    ...
    const archiveDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "civitas-backup-verify-on-create-"),
    );
    try {
      await fs.writeFile(path.join(stateDir, "civitas.json"), JSON.stringify({}), "utf8");
      await fs.writeFile(path.join(stateDir, "state.txt"), "state\n", "utf8");
  ...
  it("rejects symlinked output paths even when intermediate directories do not exist yet", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".civitas");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-backup-link-"));
  ...
  it("falls back to the home directory when cwd is a symlink into a backed-up source tree", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".civitas");
    const workspaceDir = path.join(stateDir, "workspace");
    const linkParent = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-backup-cwd-link-"));
