...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    expect(resolveGatewayDevMode(["node", "/Users/me/civitas/src/cli/index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\civitas\\src\\cli\\index.ts"])).toBe(
      true,
    );
    expect(resolveGatewayDevMode(["node", "/Users/me/civitas/dist/cli/index.js"])).toBe(false);
  });
});
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { CIVITAS_PORT: "3000" },
  } = params;
...
describe("buildGatewayInstallPlan", () => {
  // Prevent tests from reading the developer's real ~/.civitas/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ CIVITAS_PORT: "3000" });
  ...
  it("merges config env vars into the environment", async () => {
    ...
    expect(plan.environment.CIVITAS_PORT).toBe("3000");
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("merges .env file vars into the install plan", async () => {
    await writeStateDirDotEnv("BRAVE_API_KEY=BSA-from-env\nOPENROUTER_API_KEY=or-key\n", {
      stateDir: path.join(tmpDir, ".civitas"),
    });
    mockNodeGatewayPlanFixture({ serviceEnvironment: { CIVITAS_PORT: "3000" } });
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:civitas|civitas)( --profile isolated)? gateway install/,
    );
  });
});
