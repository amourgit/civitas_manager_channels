...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("civitas-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.CIVITAS_STATE_DIR).toBe(path.join(tempHome.home, ".civitas"));
    ...
    expect(process.env.CIVITAS_STATE_DIR).toBe(previousStateDir);
