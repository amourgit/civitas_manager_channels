...
describe("shared/assistant-identity-values", () => {
  ...
  it("trims values and preserves strings within the limit", () => {
    expect(coerceIdentityValue("  CIVITAS  ", 20)).toBe("CIVITAS");
    expect(coerceIdentityValue("  CIVITAS  ", 8)).toBe("CIVITAS");
  });

  it("truncates overlong trimmed values at the exact limit", () => {
    expect(coerceIdentityValue("  CIVITAS Assistant  ", 8)).toBe("CIVITAS");
  });

  it("returns an empty string when truncating to a zero-length limit", () => {
    expect(coerceIdentityValue("  CIVITAS  ", 0)).toBe("");
    expect(coerceIdentityValue("  CIVITAS  ", -1)).toBe("OpenCla");
  });
});
