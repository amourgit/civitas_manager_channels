...
describe("avatar policy", () => {
  it("classifies avatar URI and path helpers directly", () => {
    ...
    expect(isWindowsAbsolutePath("C:\\\\avatars\\\\civitas.png")).toBe(true);
  });

  it("accepts workspace-relative avatar paths and rejects URI schemes", () => {
    expect(isWorkspaceRelativeAvatarPath("avatars/civitas.png")).toBe(true);
    expect(isWorkspaceRelativeAvatarPath("C:\\\\avatars\\\\civitas.png")).toBe(true);
  ...
  it("detects avatar-like path strings", () => {
    expect(looksLikeAvatarPath("avatars/civitas.svg")).toBe(true);
    expect(looksLikeAvatarPath("civitas.webp")).toBe(true);
