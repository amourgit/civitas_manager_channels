...
async function runInstallMetadataAudit(
  ...
  return runSecurityAudit({
    ...
    includeChannelSecurity: false,
    stateDir,
    configPath: path.join(stateDir, "civitas.json"),
...
describe("security audit", () => {
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "civitas.json");
  ...
  const runSharedExtensionsAudit = async (config: OpenClawConfig) => {
    return runSecurityAudit({
      ...
      configPath: path.join(sharedExtensionsStateDir, "civitas.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  };
  beforeAll(async () => {
  ...
  it("evaluates safeBinTrustedDirs risk findings", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/civitas-safe-bins"];
    const cases = [
      {
    ...
    const cases = [
      {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from(
                    "civitas-sbx-browser-old\ncivitas-sbx-browser-missing-hash\n",
                  ),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "civitas-sbx-browser-old") {
                return {
              ...
              if (args[0] === "inspect" && args.at(-1) === "civitas-sbx-browser-missing-hash") {
                return {
        ...
        assert: (res: SecurityAuditReport) => {
          ...
          expect(staleEpoch?.detail).toContain("civitas-sbx-browser-old");
        },
      },
      {
        ...
        run: async () => {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from("civitas-sbx-browser-exposed\n"),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "civitas-sbx-browser-exposed") {
                return {
              ...
              if (args[0] === "port" && args.at(-1) === "civitas-sbx-browser-exposed") {
                return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-civitas.json");
      ...
      cases
        ...
        .map((testCase) => ({
          run: async () => {
            ...
            const configPath = path.join(fixture.stateDir, "civitas.json");
            await fs.writeFile(configPath, "{}\n", "utf-8");
            if (!isWindows) {
              await fs.chmod(configPath, 0o600);
            }
  ...
  it.each([
    {
      ...
      detailIncludes: [
        ...
        "~/.civitas/credentials/discord-allowFrom.json:team.owner",
      ],
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      ...
      configPath: "/Users/test/Dropbox/.civitas/civitas.json",
    });
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            ...
            JSON.stringify({ name: "@civitas/voice-call", version: "9.9.9" }),
            "utf-8",
          );
          await fs.writeFile(
            path.join(hookDir, "package.json"),
            JSON.stringify({ name: "@civitas/test-hooks", version: "8.8.8" }),
            "utf-8",
          );
  ...
  it("skips plugin code safety findings when deep audit is disabled", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        civitas: { extensions: [".hidden/index.js"] },
      }),
      "utf-8",
    );
    await fs.writeFile(
