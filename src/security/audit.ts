...
import type { ConfigFileSnapshot, CIVITASConfig } from "../config/config.js";
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your CIVITAS state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your CIVITAS state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.CIVITAS_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.CIVITAS_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("civitas security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.CIVITAS_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(env.CIVITAS_GATEWAY_PASSWORD) ||
...
function collectLoggingFindings(cfg: CIVITASConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: CIVITASConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: CIVITASConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: CIVITASConfig): string[] {
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("civitas status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("civitas security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set CIVITAS_GATEWAY_TOKEN/CIVITAS_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("civitas security audit --deep")}".`,
    });
  }
