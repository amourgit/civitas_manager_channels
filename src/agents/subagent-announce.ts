    ...
    lines.push(
      ...
      ...(acpEnabled
        ? [
            ...
            '`agents_list` and `subagents` apply to CIVITAS sub-agents (`runtime: "subagent"`); ACP harness ids are controlled by `acp.allowedAgents`.',
            "Do not ask users to run slash commands or CLI when `sessions_spawn` can do it directly.",
            "Do not use `exec` (`civitas ...`, `acpx ...`) to spawn ACP sessions.",
            'Use `subagents` only for CIVITAS subagents (`runtime: "subagent"`).',
