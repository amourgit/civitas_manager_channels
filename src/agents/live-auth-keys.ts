...
const GOOGLE_LIVE_SINGLE_KEY = "CIVITAS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "CIVITAS_LIVE_ANTHROPIC_KEY",
    listVar: "CIVITAS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "CIVITAS_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `CIVITAS_LIVE_${base}_KEY`;
