...
function createCIVITASModelRegistry(
  authStorage: PiAuthStorage,
  modelsJsonPath: string,
  agentDir: string,
): PiModelRegistry {
...
function scrubLegacyStaticAuthJsonEntries(pathname: string): void {
  if (process.env.CIVITAS_AUTH_STORE_READONLY === "1") {
    return;
  }
...
export function discoverModels(authStorage: PiAuthStorage, agentDir: string): PiModelRegistry {
  return createCIVITASModelRegistry(authStorage, path.join(agentDir, "models.json"), agentDir);
}
