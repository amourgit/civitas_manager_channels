...
function resolveExecConfig(params: { cfg?: CIVITASConfig; agentId?: string }) {
...
export function createCIVITASCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createCIVITASReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
