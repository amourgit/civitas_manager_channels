...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real civitas startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "civitas", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "civitas", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "civitas", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
