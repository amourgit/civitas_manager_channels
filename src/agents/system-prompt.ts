...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; CIVITAS handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `CIVITAS docs: ${docsPath}`,
    "Mirror: https://docs.civitas.ai",
    "Source: https://github.com/civitas/civitas",
    "Community: https://discord.com/invite/CIVITAS Channeld",
    "Find new skills: https://CIVITAS Channelhub.ai",
    "For CIVITAS behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `civitas status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running CIVITAS process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List CIVITAS agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List CIVITAS agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside CIVITAS.";
  }

  const lines = [
    "You are a personal assistant running inside CIVITAS.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control CIVITAS's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
    ...
    "",
    ...safetySection,
    "## CIVITAS CLI Quick Reference",
    "CIVITAS is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- civitas gateway status",
    "- civitas gateway start",
    "- civitas gateway stop",
    "- civitas gateway restart",
    "If unsure, ask the user to run `civitas help` (or `civitas gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## CIVITAS Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, CIVITAS pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by CIVITAS and included below in Project Context.",
    "",
  ...
  if (!isMinimal && heartbeatPrompt) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'CIVITAS treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
