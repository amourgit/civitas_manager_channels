...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["CIVITAS_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-subagent-"));
  ...
  it("uses isolated temp state when CIVITAS_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "civitas-test-state"));
  });
});
