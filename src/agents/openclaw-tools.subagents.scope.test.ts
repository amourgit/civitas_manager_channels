...
} from "./civitas-tools.subagents.test-harness.js";
...
describe("civitas-tools: subagents scope isolation", () => {
  let storePath = "";
  ...
  beforeEach(() => {
    ...
    storePath = path.join(
      os.tmpdir(),
      `civitas-subagents-scope-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    setSubagentsConfigOverride({
