...
async function createTempWorkspaceDir() {
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-"));
...
async function setupWorkspaceWithProsePlugin() {
  ...
  const pluginRoot = path.join(workspaceDir, ".civitas", "extensions", "open-prose");
...
async function setupWorkspaceWithDiffsPlugin() {
  ...
  const managedDir = path.join(workspaceDir, ".managed");
  const bundledDir = path.join(workspaceDir, ".bundled");
  const pluginRoot = path.join(workspaceDir, ".civitas", "extensions", "diffs");
...
describe("loadWorkspaceSkillEntries", () => {
  ...
  it("keeps remote-eligible skills when agent filtering is active", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"civitas":{"requires":{"anyBins":["missingbin","sandboxbin"]}}}',
    });
