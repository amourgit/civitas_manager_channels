...
function isLikelyCIVITASCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = path
    .basename(argv[1] ?? "")
    .trim()
    .toLowerCase();
  return (
    entryBasename === "civitas" ||
    entryBasename === "civitas.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real CIVITAS CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureCIVITASModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyCIVITASCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): CIVITASConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureCIVITASModelsJson(cfg);
    } catch {
