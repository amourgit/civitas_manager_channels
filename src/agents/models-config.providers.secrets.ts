...
type ModelsConfig = NonNullable<CIVITASConfig["models"]>;
...
function resolveConfigBackedProviderAuth(params: { provider: string; config?: CIVITASConfig }):
  | {
      apiKey: string;
      discoveryApiKey?: string;
      mode: "api_key";
...
function resolveXaiConfigFallbackAuth(params: { provider: string; config?: CIVITASConfig }):
  | {
      apiKey: string;
      source: string;
      mode: "api-key";
