...
const originalBundledDir = process.env.CIVITAS_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("civitas-bundled-");
  const workspaceDir = await createTempDir("civitas-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: CIVITASConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.CIVITAS_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
...
afterEach(async () => {
  process.env.CIVITAS_BUNDLED_PLUGINS_DIR = originalBundledDir;
  clearPluginManifestRegistryCache();
  await Promise.all(
