...
[media attached 1/2: /Users/tyleryust/.civitas/media/IMG_6430.jpeg (image/jpeg)]
[media attached 2/2: /Users/tyleryust/.civitas/media/IMG_6431.jpeg (image/jpeg)]
...
what is this?`);
  ...
  it("handles paths with spaces in filename", () => {
    ...
    const ref =
      expectSingleImageReference(`[media attached: /Users/test/.civitas/media/ChatGPT Image Apr 21, 2025.png (image/png) | https://example.com/same.png]
what is this?`);
...
describe("loadImageFromRef", () => {
  it("allows sandbox-validated host paths outside default media roots", async () => {
    ...
    const sandboxParent = await fs.mkdtemp(path.join(homeDir, "civitas-sandbox-image-"));
    try {
...
describe("detectAndLoadPromptImages", () => {
  ...
  it("blocks prompt image refs outside workspace when sandbox workspaceOnly is enabled", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-native-image-sandbox-"));
