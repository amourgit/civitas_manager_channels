...
vi.mock("../../docs-path.js", () => ({
  resolveCIVITASDocsPath: async () => undefined,
}));
...
vi.mock("../../pi-tools.js", () => ({
  createCIVITASCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) => [
    {
      name: "sessions_spawn",
      execute: async (
        _callId: string,
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("civitas.cache-ttl", data),
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-ctx-engine-agent-"));
