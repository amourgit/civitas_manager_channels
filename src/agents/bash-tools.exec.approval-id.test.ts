...
function getTestConfigPath() {
  return path.join(process.env.HOME ?? "", ".civitas", "civitas.json");
}

async function writeCIVITASConfig(config: Record<string, unknown>, pretty = false) {
...
async function writeExecApprovalsConfig(config: Record<string, unknown>) {
  const approvalsPath = path.join(process.env.HOME ?? "", ".civitas", "exec-approvals.json");
...
describe("exec approvals", () => {
  ...
  beforeEach(async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-test-"));
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-test-bin-"));
  ...
  it("auto-continues the same Discord session after approval resolves without a second user turn", async () => {
    const agentCalls: Array<Record<string, unknown>> = [];
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-exec-followup-discord-"));
  ...
  it("executes approved commands and emits a session-only followup in webchat-only mode", async () => {
    const agentCalls: Array<Record<string, unknown>> = [];
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-exec-followup-sidefx-"));
    const markerPath = path.join(tempDir, "marker.txt");
  ...
  it("runs a skill wrapper chain without prompting when the wrapper is allowlisted", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-skill-wrapper-"));
    try {
      const skillDir = path.join(tempDir, ".civitas", "skills", "gog");
  ...
  it("denies gateway obfuscated command when approval request times out", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-test-obf-"));
