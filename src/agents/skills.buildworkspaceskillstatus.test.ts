...
function makeEntry(params: {
  ...
  return {
    skill: createFixtureSkill({
      ...
      filePath,
      baseDir,
      source: params.source ?? "civitas-workspace",
    }),
    frontmatter: {},
    metadata: {
...
describe("buildWorkspaceSkillStatus", () => {
  ...
  it("does not mark an overridden workspace skill as bundled by bundled name alone", async () => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-bundled-"));
    ...
    await withEnvAsync({ CIVITAS_BUNDLED_SKILLS_DIR: bundledDir }, async () => {
      const report = buildWorkspaceSkillStatus("/tmp/ws", {
        entries: [
          makeEntry({
            name: "peekaboo",
      ...
      expect(skill).toBeDefined();
      expect(skill?.source).toBe("civitas-workspace");
