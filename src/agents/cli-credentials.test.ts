...
describe("cli credentials", () => {
  ...
  it("falls back to the file store when the keychain update fails", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-"));
  ...
  it("reads Codex credentials from keychain when available", async () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-codex-"));
  ...
  it("invalidates cached Codex credentials when auth.json changes within the TTL window", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-codex-cache-"));
    process.env.CODEX_HOME = tempHome;
    const authPath = path.join(tempHome, "auth.json");
  ...
  it("updates existing Codex auth.json in place", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-codex-write-"));
    process.env.CODEX_HOME = tempHome;
    try {
      fs.mkdirSync(tempHome, { recursive: true, mode: 0o700 });
      const authPath = path.join(tempHome, "auth.json");
  ...
  it("prefers the existing Codex keychain entry over auth.json on darwin writes", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-codex-keychain-write-"));
    process.env.CODEX_HOME = tempHome;
    try {
