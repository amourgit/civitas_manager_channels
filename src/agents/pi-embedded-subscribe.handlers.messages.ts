...
function isTranscriptOnlyCIVITASAssistantMessage(message: AgentMessage | undefined): boolean {
  if (!message || message.role !== "assistant") {
    return false;
  }
  const provider = typeof message.provider === "string" ? message.provider.trim() : "";
  const model = typeof message.model === "string" ? message.model.trim() : "";
  return provider === "civitas" && (model === "delivery-mirror" || model === "gateway-injected");
}
...
export function handleMessageStart(
  ...
  if (msg?.role !== "assistant" || isTranscriptOnlyCIVITASAssistantMessage(msg)) {
    return;
  }
