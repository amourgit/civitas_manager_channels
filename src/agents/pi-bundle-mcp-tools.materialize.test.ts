...
describe("createBundleMcpToolRuntime", () => {
  it("loads bundle MCP tools and executes them", async () => {
    const workspaceDir = await makeTempDir("civitas-bundle-mcp-tools-");
    const pluginRoot = path.join(workspaceDir, ".civitas", "extensions", "bundle-probe");
  ...
  it("loads configured SSE MCP tools via url", async () => {
    ...
    try {
      const workspaceDir = await makeTempDir("civitas-bundle-mcp-sse-");
      const runtime = await createBundleMcpToolRuntime({
        workspaceDir,
        cfg: {
          mcp: {
