...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("civitas-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("civitas-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("civitas-sbx-");
