...
let configOverride: CIVITASConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: CIVITASConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/civitas/civitas/issues/31298
    previousFastTestEnv = process.env.CIVITAS_TEST_FAST;
  ...
  afterAll(() => {
    ...
    if (previousFastTestEnv === undefined) {
      delete process.env.CIVITAS_TEST_FAST;
      return;
    }
    process.env.CIVITAS_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // CIVITAS_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("CIVITAS runtime context (internal):");
