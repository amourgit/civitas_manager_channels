...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "civitas-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "civitas-managed",
  });
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`civitas skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`civitas skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
