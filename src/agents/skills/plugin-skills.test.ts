    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "civitas.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "civitas.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "civitas.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("civitas-");
  const acpxRoot = await tempDirs.make("civitas-acpx-plugin-");
  const helperRoot = await tempDirs.make("civitas-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("civitas-");
  const pluginRoot = await tempDirs.make("civitas-plugin-");
  const outsideDir = await tempDirs.make("civitas-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("civitas-");
    const pluginRoot = await tempDirs.make("civitas-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("civitas-");
    const pluginRoot = await tempDirs.make("civitas-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
