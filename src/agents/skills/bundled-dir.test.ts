...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["CIVITAS_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns CIVITAS_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-bundled-override-"));
    process.env.CIVITAS_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.CIVITAS_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "civitas" }));
