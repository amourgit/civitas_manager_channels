...
function resolveAttachmentLimits(config: CIVITASConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".civitas", "attachments");
  const relDir = path.posix.join(".civitas", "attachments", attachmentId);
