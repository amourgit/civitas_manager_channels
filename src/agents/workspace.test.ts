...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses CIVITAS_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      CIVITAS_HOME: "/srv/civitas-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/civitas-home"), ".civitas", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".civitas", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("civitas-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("civitas-workspace-");
    await fs.mkdir(path.join(tempDir, ".civitas"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-workspace-hardlink-"));
    try {
