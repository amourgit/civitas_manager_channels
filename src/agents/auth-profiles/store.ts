 ...
 * A common mistake — caused by the similarity with the `civitas.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (civitas.json uses "mode"; auth-profiles.json uses "type")
...
function shouldLogAuthStoreTiming(): boolean {
  return process.env.CIVITAS_DEBUG_INGRESS_TIMING === "1";
}
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.CIVITAS_AUTH_STORE_READONLY === "1";
