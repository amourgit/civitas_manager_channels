...
describe("resolveApiKeyForProfile openai-codex refresh fallback", () => {
  ...
  beforeEach(async () => {
    ...
    clearRuntimeAuthProfileStoreSnapshots();
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-codex-refresh-fallback-"));
    agentDir = path.join(tempRoot, "agents", "main", "agent");
    await fs.mkdir(agentDir, { recursive: true });
    process.env.CIVITAS_STATE_DIR = tempRoot;
  ...
  it("re-reads externally managed Codex CLI credentials instead of refreshing them", async () => {
    const profileId = "openai-codex:default";
    saveAuthProfileStore(
      {
        version: 1,
