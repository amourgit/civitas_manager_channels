...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /civitas\.json|civitas gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumed = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify civitas.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
