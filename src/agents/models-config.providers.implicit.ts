...
function resolveLiveProviderCatalogTimeoutMs(env: NodeJS.ProcessEnv): number | null {
  const live =
    env.CIVITAS_LIVE_TEST === "1" || env.CIVITAS_LIVE_GATEWAY === "1" || env.LIVE === "1";
  if (!live) {
    return null;
  }
  const raw = env.CIVITAS_LIVE_PROVIDER_DISCOVERY_TIMEOUT_MS?.trim();
  if (!raw) {
    return 15_000;
  }
...
function resolveLiveProviderDiscoveryFilter(env: NodeJS.ProcessEnv): string[] | undefined {
  ...
  const raw = env.CIVITAS_LIVE_PROVIDERS?.trim();
  if (!raw || raw === "all") {
    return undefined;
  }
  const ids = raw
...
function buildPluginCatalogConfig(ctx: ImplicitProviderContext): CIVITASConfig {
...
): Promise<NonNullable<CIVITASConfig["models"]>["providers"]> {
