...
export function buildDockerExecArgs(params: {
  ...
  for (const [key, value] of Object.entries(params.env)) {
    ...
    // PATH is handled separately via CIVITAS_PREPEND_PATH below.
    if (key === "PATH") {
      continue;
    }
  ...
  if (hasCustomPath) {
    ...
    args.push("-e", `CIVITAS_PREPEND_PATH=${params.env.PATH}`);
  }
  ...
  const pathExport = hasCustomPath
    ? 'export PATH="${CIVITAS_PREPEND_PATH}:$PATH"; unset CIVITAS_PREPEND_PATH; '
    : "";
