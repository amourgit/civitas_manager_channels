...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-test-"));
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: civitas --profile isolated doctor --non-interactive",
          );
  ...
  it("rejects config.apply when protected exec settings are omitted", async () => {
    ...
    await expect(
      tool.execute("call-missing-protected", {
        action: "config.apply",
        raw: '{ agents: { defaults: { workspace: "~/civitas" } } }',
      }),
    ).rejects.toThrow(
