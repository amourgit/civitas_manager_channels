...
function createOpenAiConfigWithResolvedApiKey(mergeMode = false): CIVITASConfig {
  return {
    models: {
...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureCIVITASModelsJson(validated.config);
  ...
  it("fills anthropic-vertex apiKey with the ADC sentinel when models exist", async () => {
    await withTempHome(async () => {
      const adcDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-adc-"));
