...
type CodingToolsInput = NonNullable<Parameters<typeof createCIVITASCodingTools>[0]>;
...
function resolveApplyPatchTool(
  params: Pick<CodingToolsInput, "sandbox" | "workspaceDir"> & { config: CIVITASConfig },
): ToolWithExecute {
...
describe("tools.fs.workspaceOnly", () => {
  it("defaults to allowing sandbox mounts outside the workspace root", async () => {
    await withUnsafeMountedSandboxHarness(async ({ sandboxRoot, agentRoot, sandbox }) => {
      ...
      const tools = createCIVITASCodingTools({ sandbox, workspaceDir: sandboxRoot });
  ...
  it("rejects sandbox mounts outside the workspace root when enabled", async () => {
    await withUnsafeMountedSandboxHarness(async ({ sandboxRoot, agentRoot, sandbox }) => {
      ...
      const cfg = { tools: { fs: { workspaceOnly: true } } } as unknown as CIVITASConfig;
      const tools = createCIVITASCodingTools({ sandbox, workspaceDir: sandboxRoot, config: cfg });
