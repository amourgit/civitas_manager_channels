...
describe("openai transport stream", () => {
  ...
  it("prepares a custom simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("civitas-openai-responses-transport");
    expect(prepared).toMatchObject({
      api: "civitas-openai-responses-transport",
      provider: "openai",
      id: "gpt-5.4",
    });
  ...
  it("prepares an Anthropic simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("civitas-anthropic-messages-transport");
    expect(prepared).toMatchObject({
      api: "civitas-anthropic-messages-transport",
      provider: "anthropic",
      id: "claude-sonnet-4-6",
    });
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
  ...
  it("prepares a Google simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe(
      "civitas-google-generative-ai-transport",
    );
    expect(prepared).toMatchObject({
      api: "civitas-google-generative-ai-transport",
      provider: "google",
      id: "gemini-3.1-pro-preview",
    });
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    ...
    void streamFn!(
      {
        id: "anthropic/claude-sonnet-4",
        name: "Claude Sonnet 4",
        api: "civitas-openai-completions-transport",
        provider: "openrouter",
        baseUrl: "https://proxy.example.com/v1",
        reasoning: true,
        input: ["text"],
        ...
        contextWindow: 200000,
        maxTokens: 8192,
      } as Model<"civitas-openai-completions-transport">,
      {
        systemPrompt: "system",
        messages: [],
        tools: [],
  ...
  it("adds native OpenAI turn metadata on direct Responses routes", () => {
    const params = buildOpenAIResponsesParams(
      ...
      {
        civitas_session_id: "session-123",
        civitas_turn_id: "turn-123",
        civitas_turn_attempt: "1",
        civitas_transport: "stream",
      },
