...
function buildConfig(enableNoVnc: boolean): SandboxConfig {
  return {
    ...
    workspaceRoot: "/tmp/civitas-sandboxes",
    docker: {
    ...
    browser: {
      enabled: true,
      image: "civitas-sandbox-browser:bookworm-slim",
      containerPrefix: "civitas-sbx-browser-",
      network: "civitas-sandbox-browser",
      cdpPort: 9222,
      vncPort: 5900,
      noVncPort: 6080,
      headless: false,
...
describe("ensureSandboxBrowser create args", () => {
  ...
  it("publishes noVNC on loopback and injects noVNC password env", async () => {
    ...
    expect(envEntries).toContain("CIVITAS_BROWSER_NO_SANDBOX=1");
    const passwordEntry = envEntries.find((entry) =>
      entry.startsWith("CIVITAS_BROWSER_NOVNC_PASSWORD="),
    );
    expect(passwordEntry).toMatch(/^CIVITAS_BROWSER_NOVNC_PASSWORD=[A-Za-z0-9]{8}$/);
  ...
  it("does not inject noVNC password env when noVNC is disabled", async () => {
    ...
    const createArgs = findDockerArgsCall(dockerMocks.execDocker.mock.calls, "create");
    const envEntries = collectDockerFlagValues(createArgs ?? [], "-e");
    expect(envEntries.some((entry) => entry.startsWith("CIVITAS_BROWSER_NOVNC_PASSWORD="))).toBe(
      false,
    );
  ...
  it("stamps the mount format version label on browser containers", async () => {
    ...
    expect(labels).toContain(`civitas.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  });
});
