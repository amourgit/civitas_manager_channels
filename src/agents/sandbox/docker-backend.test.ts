...
describe("docker sandbox backend manager", () => {
  ...
  it("matches ordinary sandbox runtimes against sandbox.docker.image", async () => {
    dockerMocks.execDocker.mockResolvedValueOnce({
      code: 0,
      stdout: "civitas-sandbox:bookworm-slim\n",
      stderr: "",
    });
    ...
    expect(result).toEqual({
      running: true,
      actualConfigLabel: "civitas-sandbox:bookworm-slim",
  ...
  it("matches browser runtimes against sandbox.browser.image", async () => {
    dockerMocks.execDocker.mockResolvedValueOnce({
      code: 0,
      stdout: "civitas-sandbox-browser:bookworm-slim\n",
      stderr: "",
    });
    ...
    expect(result).toEqual({
      running: true,
      actualConfigLabel: "civitas-sandbox-browser:bookworm-slim",
      configLabelMatch: true,
    });
  });
