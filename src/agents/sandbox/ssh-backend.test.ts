...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "civitas-test-ssh-config"),
    host: "civitas-sandbox",
  };
}
...
function createBackendSandboxConfig(params?: { binds?: string[]; target?: string }): SandboxConfig {
  return {
    ...
    workspaceRoot: "~/.civitas/sandboxes",
    docker: {
      image: "img",
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "civitas-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "civitas-ssh-worker-abcd1234",
    ...
    expect(sshMocks.runSshSandboxCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        remoteCommand: expect.stringContaining("/remote/civitas/civitas-ssh-agent-worker"),
      }),
    );
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const backend = await createSshSandboxBackend({
      ...
      cfg: {
        ...
        browser: {
          enabled: false,
          image: "civitas-browser",
          containerPrefix: "civitas-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/civitas/civitas-ssh-agent-worker");
