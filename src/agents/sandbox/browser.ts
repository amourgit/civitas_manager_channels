...
const CDP_SOURCE_RANGE_ENV_KEY = "CIVITAS_BROWSER_CDP_SOURCE_RANGE";
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_CIVITAS_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_CIVITAS_BROWSER_PROFILE_NAME,
    extraArgs: [],
    profiles: {
      [DEFAULT_CIVITAS_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "civitas.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `civitas sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `civitas sandbox recreate --browser --agent ${agentId}`;
          }
          return "civitas sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: {
        ...
        "civitas.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
      },
    ...
    args.push("-e", `CIVITAS_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `CIVITAS_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `CIVITAS_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    if (cdpSourceRange) {
      args.push("-e", `${CDP_SOURCE_RANGE_ENV_KEY}=${cdpSourceRange}`);
    }
    args.push("-e", `CIVITAS_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `CIVITAS_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
    ...
    args.push("-e", "CIVITAS_BROWSER_NO_SANDBOX=1");
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_CIVITAS_BROWSER_PROFILE_NAME)
    : null;
