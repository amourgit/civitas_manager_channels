...
import { markCIVITASExecEnv } from "../../infra/civitas-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "civitas.sandbox=1");
  args.push("--label", `civitas.sessionKey=${params.scopeKey}`);
  args.push("--label", `civitas.createdAtMs=${createdAtMs}`);
  args.push("--label", `civitas.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `civitas.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markCIVITASExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "civitas.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`civitas sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`civitas sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("civitas sandbox recreate --all");
}
