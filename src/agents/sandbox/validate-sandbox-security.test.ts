...
describe("getBlockedBindReason", () => {
  ...
  it("still blocks OS-home credential paths when CIVITAS_HOME points elsewhere", () => {
    vi.stubEnv("HOME", "/home/tester");
    vi.stubEnv("CIVITAS_HOME", "/srv/civitas-home");
  ...
  it("blocks canonical OS-home aliases for credential paths", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "civitas-home-"));
...
describe("validateBindMounts", () => {
  ...
  it("blocks symlink escapes into blocked directories", () => {
    if (process.platform === "win32") {
      ...
      const dir = mkdtempSync(join(tmpdir(), "civitas-sbx-"));
  ...
  it("blocks symlink-parent escapes into blocked paths when leaf does not exist", () => {
    ...
    const missingLeaf = join(link, "civitas-not-created");
    expect(() =>
...
describe("validateApparmorProfile", () => {
  it("allows named profile/undefined", () => {
    expect(() => validateApparmorProfile("civitas-sandbox")).not.toThrow();
