...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: CIVITASConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<CIVITASConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<CIVITASConfig["agents"]>["list"]>[number]["tools"];
  }): CIVITASConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createCIVITASCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createCIVITASCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createCIVITASCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createCIVITASCodingTools({
      config: cfg,
    ...
    const bobTools = createCIVITASCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createCIVITASCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createCIVITASCodingTools({
      config: cfg,
