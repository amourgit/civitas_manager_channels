...
function createOpenAiApiKeySourceConfig(): CIVITASConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiApiKeyRuntimeConfig(): CIVITASConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderSourceConfig(): CIVITASConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderRuntimeConfig(): CIVITASConfig {
  return {
    models: {
      providers: {
        openai: {
...
function withGatewayTokenMode(config: CIVITASConfig): CIVITASConfig {
  return {
    ...config,
    gateway: {
      auth: {
  ...
  await withTempHome(async () => {
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      try {
        ...
        await ensureCIVITASModelsJson(params.candidateConfig ?? loadConfig());
        await runAssertions();
      } finally {
        clearRuntimeConfigSnapshot();
        clearConfigCache();
    ...
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        try {
          ...
          await ensureCIVITASModelsJson(loadConfig());
  ...
  it("projects cloned runtime configs onto source snapshot when preserving provider auth", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const clonedRuntimeConfig: CIVITASConfig = {
          ...runtimeConfig,
          agents: {
            defaults: {
        ...
        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureCIVITASModelsJson(clonedRuntimeConfig);
  ...
  it("invalidates cached readiness when projected config changes under the same runtime snapshot", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const sourceConfig = createOpenAiApiKeySourceConfig();
        const runtimeConfig = createOpenAiApiKeyRuntimeConfig();
        const firstCandidate: CIVITASConfig = {
          ...runtimeConfig,
          models: {
            providers: {
              openai: {
        ...
        const secondCandidate: CIVITASConfig = {
          ...runtimeConfig,
          models: {
            providers: {
              openai: {
        ...
        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureCIVITASModelsJson(firstCandidate);
          ...
          await ensureCIVITASModelsJson(secondCandidate);
  ...
  it("keeps source markers when runtime projection is skipped for incompatible top-level shape", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const incompatibleCandidate: CIVITASConfig = {
          ...createOpenAiApiKeyRuntimeConfig(),
        };

        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureCIVITASModelsJson(incompatibleCandidate);
          await expectGeneratedProviderApiKey("openai", "OPENAI_API_KEY"); // pragma: allowlist secret
        } finally {
          clearRuntimeConfigSnapshot();
          clearConfigCache();
