...
function createTempCommandDir(
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-shell-"));
  tempDirs.push(dir);
...
describe("resolveShellFromPath", () => {
  ...
  it("returns undefined when command does not exist", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-shell-empty-"));
    tempDirs.push(dir);
...
describe("resolvePowerShellPath", () => {
  ...
  it("prefers PowerShell 7 in ProgramFiles", () => {
    const base = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-pfiles-"));
    tempDirs.push(base);
  ...
  it("prefers ProgramW6432 PowerShell 7 when ProgramFiles lacks pwsh", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-pfiles-"));
    const programW6432 = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-pw6432-"));
  ...
  it("finds pwsh on PATH when not in standard install locations", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-pfiles-"));
    const binDir = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-bin-"));
  ...
  it("falls back to Windows PowerShell 5.1 path when pwsh is unavailable", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-pfiles-"));
    const sysRoot = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-sysroot-"));
