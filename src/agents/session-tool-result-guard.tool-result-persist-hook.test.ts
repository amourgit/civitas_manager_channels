...
describe("tool_result_persist hook", () => {
  ...
  it("loads tool_result_persist hooks without breaking persistence", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-toolpersist-"));
    process.env.CIVITAS_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  api.on("tool_result_persist", (event) => {
    ...
    const registry = loadCIVITASPlugins({
      cache: false,
      workspaceDir: tmp,
      config: {
        plugins: {
...
describe("before_message_write hook", () => {
  it("continues persistence when a before_message_write hook throws", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-before-write-"));
    process.env.CIVITAS_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
