...
function resolveNativeOpenAISessionHeaders(params: {
  ...
  return {
    ...
    "x-civitas-session-id": sessionId,
  };
}
...
function resolveNativeOpenAITransportTurnState(params: {
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-civitas-turn-id": turnId,
      "x-civitas-turn-attempt": attempt,
    },
    metadata: {
      civitas_session_id: sessionHeaders["x-civitas-session-id"] ?? "",
      civitas_turn_id: turnId,
      civitas_turn_attempt: attempt,
      civitas_transport: params.transport,
    },
  };
}
