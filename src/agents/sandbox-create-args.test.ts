...
import { CIVITAS_CLI_ENV_VALUE } from "../infra/civitas-exec-env.js";
...
describe("buildSandboxCreateArgs", () => {
  function createSandboxConfig(
    ...
    return {
      image: "civitas-sandbox:bookworm-slim",
      containerPrefix: "civitas-sbx-",
      workdir: "/workspace",
      readOnlyRoot: false,
      tmpfs: [],
      network: "none",
  ...
  it("includes hardening and resource flags", () => {
    const cfg: SandboxDockerConfig = {
      ...
      apparmorProfile: "civitas-sandbox",
      dns: ["1.1.1.1"],
      extraHosts: ["internal.service:10.0.0.5"],
    };

    const args = buildSandboxCreateArgs({
      name: "civitas-sbx-test",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
      labels: { "civitas.sandboxBrowser": "1" },
    });
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        ...
        "civitas.sandbox=1",
        "--label",
        "civitas.sessionKey=main",
        "--label",
        "civitas.createdAtMs=1700000000000",
        "--label",
        "civitas.sandboxBrowser=1",
        "--read-only",
        "--tmpfs",
        "/tmp",
        "--network",
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        ...
        "LANG=C.UTF-8",
        "--env",
        `CIVITAS_CLI=${CIVITAS_CLI_ENV_VALUE}`,
      ]),
    );
  ...
  it("preserves the CIVITAS exec marker when strict env sanitization is enabled", () => {
  ...
  it("blocks bind sources outside runtime allowlist roots", () => {
    ...
    expect(() =>
      buildSandboxCreateArgs({
        name: "civitas-sbx-outside-roots",
        cfg,
        scopeKey: "main",
        createdAtMs: 1700000000000,
  ...
  it("allows bind sources outside runtime allowlist with explicit override", () => {
    const cfg = createSandboxConfig({}, ["/opt/external:/data:rw"]);
    const args = buildSandboxCreateArgs({
      name: "civitas-sbx-outside-roots-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
      bindSourceRoots: ["/tmp/workspace", "/tmp/agent"],
  ...
  it("blocks reserved /workspace target bind mounts by default", () => {
    ...
    expectBuildToThrow("civitas-sbx-reserved-target", cfg, /reserved container path/);
  });

  it("allows reserved /workspace target bind mounts with explicit dangerous override", () => {
    const cfg = createSandboxConfig({}, ["/tmp/override:/workspace:rw"]);
    const args = buildSandboxCreateArgs({
      name: "civitas-sbx-reserved-target-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
  ...
  it("allows container namespace join with explicit dangerous override", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "civitas-sbx-container-network-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
