...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.CIVITAS_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".civitas", `workspace-${profile}`);
  }
  return path.join(home, ".civitas", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".civitas";
