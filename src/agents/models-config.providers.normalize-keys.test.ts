...
describe("normalizeProviders", () => {
  it("trims provider keys so image models remain discoverable for custom providers", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-agent-"));
    try {
      const providers: NonNullable<NonNullable<CIVITASConfig["models"]>["providers"]> = {
        " dashscope-vision ": {
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    } as unknown as NonNullable<NonNullable<CIVITASConfig["models"]>["providers"]>;

    const sourceProviders: NonNullable<NonNullable<CIVITASConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",
