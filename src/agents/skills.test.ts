...
const fixtureSuite = createFixtureSuite("civitas-skills-suite-");
...
async function writeEnvSkill(workspaceDir: string) {
  ...
  await writeSkill({
    ...
    metadata: '{"civitas":{"requires":{"env":["ENV_KEY"]},"primaryEnv":"ENV_KEY"}}',
  });
}

beforeAll(async () => {
  await fixtureSuite.setup();
  tempHome = await createTempHomeEnv("civitas-skills-home-");
  await fs.mkdir(path.join(tempHome.home, ".civitas", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("applySkillEnvOverrides", () => {
  ...
  it("blocks unsafe env overrides but allows declared secrets", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata:
        '{"civitas":{"requires":{"env":["OPENAI_API_KEY","NODE_OPTIONS"]},"primaryEnv":"OPENAI_API_KEY"}}',
    });
  ...
  it("blocks dangerous host env overrides even when declared", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"civitas":{"requires":{"env":["BASH_ENV","SHELL"]}}}',
    });
  ...
  it("blocks override-only host env overrides in skill config", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata:
        '{"civitas":{"requires":{"env":["HTTPS_PROXY","NODE_TLS_REJECT_UNAUTHORIZED","DOCKER_HOST"]}}}',
    });
  ...
  it("allows required env overrides from snapshots", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"civitas":{"requires":{"env":["OPENAI_API_KEY"]}}}',
    });
