...
describe("exec PATH login shell merge", () => {
  ...
  it("sets CIVITAS_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-civitas-shell", {
      command: 'printf "%s" "${CIVITAS_SHELL:-}"',
    });
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/civitas-ssl-keys.log";
    try {
      const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
      ...
      expect(output).not.toContain("/tmp/civitas-ssl-keys.log");
    } finally {
  ...
  it.each([
    ...
    "exec -a civitas /approve abc123 deny",
