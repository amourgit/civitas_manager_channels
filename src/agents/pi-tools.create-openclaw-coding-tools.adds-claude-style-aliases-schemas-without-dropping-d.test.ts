...
describe("createCIVITASCodingTools", () => {
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-read-"));
    try {
      ...
      const contents = "Hello from civitas read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "civitas-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "civitas-workspace"),
    ...
    const tools = createCIVITASCodingTools({ sandbox });
