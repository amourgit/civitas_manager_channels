...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool-call arguments", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "civitas" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [
              { title: "OpenClaw", url: "https://civitas.ai/docs", content: "docs" },
            ],
          }),
    ...
    const result = await tool?.execute?.("call-1", { query: "latest civitas release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({ q: "civitas" });
    ...
    expect(details.citations).toEqual(["https://civitas.ai/docs"]);
