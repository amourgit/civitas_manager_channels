...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    civitas: process.env.CIVITAS_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.civitas === undefined) {
      delete process.env.CIVITAS_GATEWAY_TOKEN;
    } else {
      process.env.CIVITAS_GATEWAY_TOKEN = envSnapshot.civitas;
    }
  });
  ...
  it("uses CIVITAS_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.CIVITAS_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
