...
async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-pdf-"));
  try {
...
function withDefaultModel(primary: string): CIVITASConfig {
  return {
    agents: { defaults: { model: { primary } } },
  } as CIVITASConfig;
}

function withPdfModel(primary: string): CIVITASConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  vi.spyOn(modelsConfig, "ensureCIVITASModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });
...
describe("createPdfTool", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempAgentDir(async (agentDir) => {
      vi.stubEnv("ANTHROPIC_API_KEY", "anthropic-test");
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-pdf-ws-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-pdf-out-"));
      try {
        const cfg = withDefaultModel(ANTHROPIC_PDF_MODEL);
