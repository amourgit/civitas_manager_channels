...
describe("createOpenAIWebSocketStreamFn", () => {
  ...
  it("keeps native turn metadata stable across websocket retries and increments attempt", async () => {
    ...
    expect(firstPayload.metadata?.civitas_session_id).toBe("sess-turn-metadata-retry");
    expect(firstPayload.metadata?.civitas_transport).toBe("websocket");
    expect(firstPayload.metadata?.civitas_turn_id).toBeTruthy();
    expect(secondPayload.metadata?.civitas_turn_id).toBe(firstPayload.metadata?.civitas_turn_id);
    expect(firstPayload.metadata?.civitas_turn_attempt).toBe("1");
    expect(secondPayload.metadata?.civitas_turn_attempt).toBe("2");
  });
