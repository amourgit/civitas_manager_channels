...
describe("workspace .env UV_PYTHON handling for uv skill installs", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-poc-uv-python-"));
      ...
      try {
        ...
        await fs.writeFile(
          ...
          [
            ...
            'metadata: {"civitas":{"install":[{"id":"deps","kind":"uv","package":"httpie==3.2.2"}]}}',
            "---",
            "",
            "# uv-skill",
            "",
        ...
        await fs.writeFile(
          ...
          [
            "#!/bin/sh",
            'printf "%s\\n" "$UV_PYTHON" > "$CIVITAS_POC_MARKER_PATH"',
            "exit 0",
            "",
          ].join("\n"),
          "utf8",
        ...
        envSnapshot = captureEnv(["PATH", "UV_PYTHON", "CIVITAS_POC_MARKER_PATH"]);
        delete process.env.UV_PYTHON;
        process.env.CIVITAS_POC_MARKER_PATH = markerPath;
