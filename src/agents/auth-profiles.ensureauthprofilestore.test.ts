...
describe("ensureAuthProfileStore", () => {
  ...
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "civitas-auth-profiles-"));
    try {
  ...
  it.each([
    ...
    ({ name, profile, expected }) => {
      withTempAgentDir("civitas-auth-alias-", (agentDir) => {
        const storeData = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it("normalizes mode/apiKey aliases while migrating legacy auth.json", () => {
    withTempAgentDir("civitas-auth-legacy-alias-", (agentDir) => {
      fs.writeFileSync(
  ...
  it("logs one warning with aggregated reasons for rejected auth-profiles entries", () => {
    ...
    try {
      withTempAgentDir("civitas-auth-invalid-", (agentDir) => {
        const invalidStore = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it.each([
    ...
    },
    {
      ...
      prefix: "civitas-nonstr-key-malformed-ref-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
    ...
    },
    {
      ...
      prefix: "civitas-nonstr-token-ref-",
    ...
    },
    {
      ...
      prefix: "civitas-nonstr-token-num-",
      profileId: "anthropic:default",
      profile: {
        type: "token",
        provider: "anthropic",
