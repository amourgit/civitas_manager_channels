...
async function createAgentDir(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "civitas-pi-auth-storage-"));
}
...
describe("discoverAuthStorage", () => {
  ...
  it("preserves legacy auth.json when auth store is forced read-only", async () => {
    await withAgentDir(async (agentDir) => {
      const previous = process.env.CIVITAS_AUTH_STORE_READONLY;
      ...
      try {
        ...
        if (previous === undefined) {
          delete process.env.CIVITAS_AUTH_STORE_READONLY;
        } else {
          process.env.CIVITAS_AUTH_STORE_READONLY = previous;
        }
      }
    });
  });
