...
const testLogPath = path.join(os.tmpdir(), "civitas-test-env-log-level.log");
const defaultMaxFileBytes = 500 * 1024 * 1024;

describe("CIVITAS_LOG_LEVEL", () => {
  let originalEnv: string | undefined;

  beforeEach(() => {
    originalEnv = process.env.CIVITAS_LOG_LEVEL;
    delete process.env.CIVITAS_LOG_LEVEL;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.CIVITAS_LOG_LEVEL = originalEnv;
    }
    loggingState.invalidEnvLogLevelValue = null;
    resetLogger();
    setLoggerOverride(null);
  ...
  it("applies a valid env override to both file and console levels", () => {
    ...
    process.env.CIVITAS_LOG_LEVEL = "debug";
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.CIVITAS_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("CIVITAS_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid CIVITAS_LOG_LEVEL="nope"');
  });
});
