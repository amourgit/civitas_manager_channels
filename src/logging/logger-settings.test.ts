...
let originalCIVITASLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.CIVITAS_TEST_FILE_LOG;
  originalCIVITASLogLevel = process.env.CIVITAS_LOG_LEVEL;
  delete process.env.CIVITAS_TEST_FILE_LOG;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.CIVITAS_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalCIVITASLogLevel === undefined) {
    delete process.env.CIVITAS_LOG_LEVEL;
  } else {
    process.env.CIVITAS_LOG_LEVEL = originalCIVITASLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
