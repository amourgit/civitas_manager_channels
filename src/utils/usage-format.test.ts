...
describe("usage-format", () => {
  const originalAgentDir = process.env.CIVITAS_AGENT_DIR;
  let agentDir: string;

  beforeEach(async () => {
    agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "civitas-usage-format-"));
  ...
  afterEach(async () => {
    if (originalAgentDir === undefined) {
      ...
      process.env.CIVITAS_AGENT_DIR = originalAgentDir;
    }
    __resetUsageFormatCachesForTest();
    __resetGatewayModelPricingCacheForTest();
  ...
  it("prefers models.json pricing over civitas config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          "demo-preferred": {
  ...
  it("falls back to civitas config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
